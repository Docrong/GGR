...
import com.boco.eoms.km.duty.model.Kmchangeduty;
...
public class KmchangedutyDAO extends DAO {

    public KmchangedutyDAO(com.boco.eoms.db.util.ConnectionPool ds) {
        super(ds);
    }
    ...
    public Vector managerQueryVector(int roomId) throws SQLException {
        ...
        try {
            ...
            sql = "select id, workserial1, workserial2,  hander, receiver, flag , reason  from km_changeduty where room_id = ? and flag in (1,2,4,5) order by id desc";
    ...
    public Vector sentQueryVector(int roomId, String user_id)
        ...
        try {
            conn = ds.getConnection();
            sql = "select id, workserial1, workserial2,  hander, receiver, flag , reason  from km_changeduty where room_id = ? and hander = ? order by id desc";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, roomId);
    ...
    public Vector receiveQueryVector(int roomId, String user_id)
        ...
        try {
            conn = ds.getConnection();
            String sql = "";
            sql = "select id, workserial1, workserial2,  hander, receiver, flag , reason  from km_changeduty where room_id = ? and receiver = ? order by id desc";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, roomId);
            pstmt.setString(2, user_id);
            rs = pstmt.executeQuery();
    ...
    public Vector allQueryVector(int roomId, String inputdate_from,
        ...
        try {
            conn = ds.getConnection();
            String sql = "";
            sql = "select inputdate, workserial1, workserial2,  hander, receiver, flag , reason,id  from km_changeduty where room_id = ? and inputdate >=  ? and inputdate <=  ? order by id desc";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, roomId);
    ...
    public Vector getApplyVector(int roomId, String user_id, String dutydate,
        ...
        try {
            ...
            String sql = "";
            if (flag == 0) {
                sql = "select a.id,a.starttime_defined,s.dutyman,u.username from km_assignwork a,km_assign_sub s,taw_system_user u where a.room_id = ? and a.id = s.workserial and s.dutyman = ? and a.dutydate = ? and s.dutyman = u.userid";
            }
            if (flag == 1) {
                sql = "select a.id,a.starttime_defined,s.dutyman,u.username from km_assignwork a,km_assign_sub s,taw_system_user u where a.room_id = ? and a.id = s.workserial and s.workserial not in (select distinct workserial from km_assign_sub where dutyman = ?) and a.dutydate = ? and s.dutyman = u.userid";
            }
    ...
    public Vector getApplyVector(int roomId, String user_id, String startdate,
        ...
        try {
            ...
            String sql = "";
            if (flag.equals("1")) {
                sql = "select a.id,a.starttime_defined,s.dutyman,u.username from km_assignwork a,"
                        + "km_assign_sub s,taw_system_user u where a.room_id = ? and a.id = s.workserial and "
                        + "s.workserial not in (select distinct workserial from km_assign_sub where dutyman = ?) and a.dutydate >? and a.dutydate <? "
                        + "and s.dutyman = u.userid";
            } else {
                sql = "select a.id,a.starttime_defined,s.dutyman,u.username from km_assignwork a,km_assign_sub s,"
    ...
    public int getApplyNum(String user_id) throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "";
            sql = "select count(*) from  km_changeduty where receiver = ? and flag = 0";
            pstmt = conn.prepareStatement(sql);
    ...
    public void update(int id, int transact_flag) throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "UPDATE km_changeduty SET  flag=? WHERE id=?";
            pstmt = conn.prepareStatement(sql);
    ...
    public void insert(int roomId, String inputdate, int workserial1,
        ...
        String sql;
        sql = "INSERT INTO km_changeduty (room_id,inputdate,workserial1, workserial2, expireddate,hander, receiver, flag,reason) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        com.boco.eoms.db.util.BocoConnection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
    ...
    public void insert(TawRmChangeduty tawRmChangeduty) throws SQLException {
        String sql;
        sql = "INSERT INTO km_changeduty ( room_id, inputdate, workserial1, workserial2, expireddate, hander, receiver, flag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        com.boco.eoms.db.util.BocoConnection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
    ...
    public void delete(String today) throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "DELETE FROM km_changeduty WHERE expireddate <=?";
            pstmt = conn.prepareStatement(sql);
    ...
    public void update(TawRmChangeduty tawRmChangeduty) throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "UPDATE km_changeduty SET room_id=?, inputdate=?, workserial1=?, workserial2=?, expireddate=?, hander=?, receiver=?, flag=? WHERE id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, tawRmChangeduty.getRoomId());
            pstmt.setString(2, tawRmChangeduty.getInputdate());
            pstmt.setInt(3, tawRmChangeduty.getWorkserial1());
    ...
    public void delete(int id) throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "DELETE FROM km_changeduty WHERE id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, id);
            pstmt.executeUpdate();
            close(pstmt);
     ...
     * @see 得到交换班记录对象
     */
    public Kmchangeduty retrieve(int id) throws SQLException {
        Kmchangeduty tawRmChangeduty = null;
        com.boco.eoms.db.util.BocoConnection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            conn = ds.getConnection();
            String sql = "SELECT * FROM km_changeduty WHERE id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, id);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                tawRmChangeduty = new Kmchangeduty();
    ...
    public List list() throws SQLException {
        ...
        try {
            conn = ds.getConnection();
            String sql = "SELECT room_id, inputdate, workserial1, workserial2, expireddate, hander, receiver, flag, id FROM km_changeduty";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    public List search(int offset, int limit, String strCondition)
        ...
        try {
            conn = ds.getConnection();
            String sql;
            sql = "SELECT * FROM km_changeduty " + strCondition;
            pstmt = conn.prepareStatement(sql);
