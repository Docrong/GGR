...
<script type="text/javascript">
    ...
    Ext.onReady(function () {
        ...
        userByDeptTree = new xbox({
            ...
            saveChkFldId: 'saveuser'
        });
    })
</script>


<form name="customstatremindForm" method="post" action="stSubscriptions.do?method=customstatremindsave"
    ...
    <table class="formTable">
        ...
        <tr>
            ...
            <td width="500" colspan="5">
                ...
                %>
                <option value="<%=subId+","+sheetName %>"><%=sheetName %>
                </option>
                <% }
                }
                %>
        ...
        <tr>
            ...
            <td width="500" colspan="5">
                <select size=1 name="stattype" class="select"
                        onChange="changetype(document.customstatremindForm.stattype.options[document.customstatremindForm.stattype.selectedIndex].value)">
        ...
        <tr>
            ...
            <td width="500" colspan="5">
                ...
                <div id="quartertime" style="display:none">
                    ...
                    <%
                        for (int q = 1; q <= 4; q++) {
                            if (q == quarte) {
                    %>
                    <option value="<%=q %>" selected>${eoms:a2u("第")}<%=q %>
                        ${eoms:a2u("季度")}</option>
                    <% } else { %>
                    <option value="<%=q %>">${eoms:a2u("第")}<%=q %>
                        ${eoms:a2u("季度")}</option>
                    <%
                            }
                        }
                ...
                <div id="datetime" style="display:none">
                    ${eoms:a2u("日期：")}
                    <input type="text" name="date" size="20" value="<%=StaticMethod.getCurrentDateTime("yyyy-MM-dd") %>"
                           onclick="popUpCalendar(this, this,null,null,null,false,-1);" readonly="readonly"
                           class="text">
                </div>
                ...
                <div id="weektime" style="display:none">
                    ...
                    <%
                        for (int w = 1; w <= 53; w++) {
                            if (w == week) {
                    %>
                    <option value="<%=w %>" selected>${eoms:a2u("第")}<%=w %>
                        ${eoms:a2u("周")}</option>
                    <% } else {%>
                    <option value="<%=w %>">${eoms:a2u("第")}<%=w %>
                        ${eoms:a2u("周")}</option>
                    <%
                            }
                        }
        ...
        <tr>
            ...
            <td width="500" colspan="5">
                <input type="text" name="acceptdate" size="20"
                       value="<%=StaticMethod.getCurrentDateTime("yyyy-MM-dd HH:mm:ss") %>"
                       onclick="popUpCalendar(this, this,null,null,null,true,'<%=StaticMethod.getCurrentDateTime("yyyy-MM-dd")%>');"
