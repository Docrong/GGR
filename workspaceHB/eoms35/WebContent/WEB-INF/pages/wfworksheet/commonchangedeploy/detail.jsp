		 		...
		 		<c:if test="${sheetMain.status != 1}">
		 			<c:if test="${taskStatus==2}">
			 			...
			 			<c:if test="${taskName == 'AuditTask' || taskName == 'PlanTask' }">
								<option value="${urlShowAcceptRejectPage}"><bean:message bundle="sheet" key="common.acceptReject"/> </option>
								<option value="${urlShowTransmitDealPage}"><bean:message bundle="sheet" key="common.acceptRejectReSend"/> </option>
					</c:if>
					    </c:if>
				...
				<!-- 流程步骤中的操作 -->
	 			<c:if test="${taskStatus==8}">
				 	  ...
				 	  <!-- 这个页面包括移交，阶段回复，分派公共操作 -->     
		  			  <%@ include file="/WEB-INF/pages/wfworksheet/common/sheet/dealPageSelectorOption.jsp"%>
