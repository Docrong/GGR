    ...
    <tr>
        ...
        <td valign="middle" colspan=3>
            <logic:iterate id="fileList" name="SchemeMgrForm" property="fileUrl">
                ...
                %>
                <a href="<%=request.getContextPath()%>/filemanager/fileUpload/download.jsp?url=<%=url%>&showName=<%=fileRealName%>"><%=fileRealName%>
                </a><br/>
            </logic:iterate>
        </td>
    </tr>
...
<logic:iterate id="reportFile" name="ReportList" type="com.boco.eoms.filemanager.form.ReportListBean">
    ...
    <table width="100%" class="formTable">
        <tr>
            ...
            <td>
                <bean:write name="reportFile" property="dealUserName" scope="page"/>[<bean:write name="reportFile"
                                                                                                 property="acceptDeptName"
                                                                                                 scope="page"/>]
            </td>
            <td width="15%" class="label"><b>联系方式</b></td>
            <td>
        ...
        <tr>
            ...
            <logic:equal value="1" name="reportFile" property="status">
                <logic:equal value="0" name="reportFile" property="isAudit">
                    ...
                    <td width="15%">
                        <input type="button" name="btnVote" value="驳回报表" class="button"
                               onclick="if(window.confirm('是否要驳回\'<bean:write name="reportFile"
                                                                              property="acceptDeptName"
                                                                              scope="page"/>\'的报表？'))  submitReject3('
                                   <bean:write scope="session" name="sessionform" property="userid"/>','<bean:write
                                       name="reportFile" property="flowId"/>','<%="reportFile" + i%>');"/>
                    </td>
                    <td>
                        <input type="button" name="btnVote" value="通过报表" class="button"
                               onclick="if(window.confirm('是否要通过\'<bean:write name="reportFile"
                                                                              property="acceptDeptName"
                                                                              scope="page"/>\'的报表？'))  submitPass3('
                                   <bean:write scope="session" name="sessionform" property="userid"/>','<bean:write
                                       name="reportFile" property="flowId"/>','<%="reportFile" + i%>');"/>
                    </td>
                </logic:equal>
...
<%--         	<html:checkbox property="fileList" value="<%=url2%>" <%if(reportFile2.getReject() == 1){ %>enabled=false<%} %>/>         	--%>
        ...
        <td nowrap>
         <bean:write name="reportFile2" property="dealUserName" scope="page"/>[<bean:write name="reportFile2"
                                                                                           property="acceptDeptName"
                                                                                           scope="page"/>]
        </td>
        <td nowrap>         
