...
import com.boco.eoms.km.configInfo.dao.KmuserRoomDAO;
...
import com.boco.eoms.km.duty.bo.KmrecordBO;
import com.boco.eoms.km.duty.bo.KmrecordPerBO;
import com.boco.eoms.km.duty.bo.KmreliefRecordBO;
import com.boco.eoms.km.duty.dao.KmassignworkDAO;
import com.boco.eoms.km.duty.dao.KmdutyfileDAO;
import com.boco.eoms.km.duty.dao.KmhangoverDAO;
import com.boco.eoms.km.duty.dao.KmrecordDAO;
import com.boco.eoms.km.duty.dao.KmrecordPerDAO;
import com.boco.eoms.km.duty.dao.KmrecordSubDAO;
...
import com.boco.eoms.km.duty.webapp.form.KmrecordForm;
...
public class KmrecordAction extends Action {
    ...
    private ActionForward performUpdateMain(ActionMapping mapping,
        ...
        KmrecordForm form = (KmrecordForm) actionForm;
        try {
    ...
    private ActionForward performUpdatePerEdit(ActionMapping mapping,
        ...
        try {
            KmrecordForm tawRmRecordForm = (KmrecordForm) actionForm;
    ...
    private ActionForward performUpdatePer(ActionMapping mapping,
        ...
        try {
            KmrecordForm kmrecordForm = (KmrecordForm) actionForm;
            if (kmrecordForm.getId() == 0) {
                int intWorkserial = Integer.parseInt(request
                        .getParameter("WORKSERIAL"));
                kmrecordForm.setId(intWorkserial);
            }
            int workserial = kmrecordForm.getId();
            // 定义数据库连接和DAO
            ...
            int complete_flag = Integer.parseInt(request
                    .getParameter(CompleteFlag));
            KmrecordPerDAO tawRmRecordPerDAO = new KmrecordPerDAO(ds);
            java.util.Date date = new java.util.Date();
            java.text.SimpleDateFormat dtFormat = new java.text.SimpleDateFormat(
                    "yyyy-MM-dd HH:mm:ss");
            String strDate = dtFormat.format(date);
    ...
    private ActionForward performSavePer(ActionMapping mapping,
        ...
        try {
            ...
            tawRmRecordPerDAO.insertPerRecord(workserial, strDate, DutyMan,
                    DutyManName, Dutyrecord, complete_flag, "", typename, "");
            KmrecordPerBO tawRmRecordPerBo = new KmrecordPerBO();
    ...
    private ActionForward performSearch(ActionMapping mapping,
        ...
        try {
            ...
            if (endtime != null && !(endtime.trim().equals(""))) {
                ...
                strCondition = strCondition + "and starttime<= '" + endtime + "'";
            }
            ...
            int intSize = tawRmRecordDAO.getSize("km_record", strCondition);
            url = request.getContextPath() + "/kmmanager" + mapping.getPath()
                    + ".do";
     ...
     * @param request
     * @param response
     * @return private ActionForward performSearchedit(ActionMapping mapping,
    ActionForm actionForm, HttpServletRequest request,
    HttpServletResponse response) {
    // edit by wangheqi 2.7 to 3.5
    TawSystemSessionForm saveSessionBeanForm = (TawSystemSessionForm) request
     ...
     * @param request
     * @param response
     * @return private ActionForward performList(ActionMapping mapping,
    ActionForm actionForm, HttpServletRequest request,
    HttpServletResponse response) {
    TawRmRecordDAO tawRmRecordDAO = null;
    String pageOffset = null;
    ...
    private ActionForward performRecord(ActionMapping mapping,
        ...
        try {
            ...
            String expertRoomName = "";
            int expertWorkSerial = 0;

            String userId = StaticMethod.null2String(saveSessionBeanForm
                    .getUserid());
            expertRoomId = KmassignworkBO.getRoomId(userId, ds);
            if (!expertRoomId.equals("-1")) {
                expertRoomName = KmassignworkBO.getRoomName(userId, ds);
                java.util.Date date = new java.util.Date();
                java.text.SimpleDateFormat dtFormat = new java.text.SimpleDateFormat(
                        "yyyy-MM-dd HH:mm:ss");
                String strDutyTime = dtFormat.format(date);
                int tempWorkSerial = kmrecordDAO.receiverWorkSerial(expertRoomId, strDutyTime, userId);

                if (tempWorkSerial == 0) {
                    return mapping.findForward("notonduty");
                } else {
                    expertWorkSerial = tempWorkSerial;
            ...
            request.setAttribute("workserial", String.valueOf(expertWorkSerial));
            request.setAttribute("roomName", expertRoomName); // 机房名称
            request.setAttribute("roomId", expertRoomId); // 机房名称


            boolean CheckMainRecordExist = kmrecordDAO
                    .receiverCheckMainRecordExist(expertWorkSerial);
            if (CheckMainRecordExist == false) {
                // 如果不存在创建值班主记录
                kmrecordDAO.getNewManiRecord(expertWorkSerial, expertRoomId + "");
            }
            // 判断值班子记录是否存在
            boolean CheckSubRecordExist = kmrecordDAO
                    .receiverCheckSubRecordExist(expertWorkSerial, userId);
            // 如果不存在生成值班子记录
            if (CheckSubRecordExist == false) {
                kmrecordDAO.getSaveSubRecord(expertWorkSerial, userId);
            }

            KmrecordForm kmrecordForm = (KmrecordForm) actionForm;
            kmrecord = kmrecordDAO.retrieve(expertWorkSerial);
            org.apache.commons.beanutils.BeanUtils.populate(kmrecordForm,
                    org.apache.commons.beanutils.BeanUtils
                            .describe(kmrecord));

            Vector vecPerRecords = new Vector();
            KmrecordPerDAO kmrecordPerDAO = new KmrecordPerDAO(ds);
            vecPerRecords = kmrecordPerDAO.getVecPerRecord(expertWorkSerial);
    ...
    private ActionForward performDevolve(ActionMapping mapping,
        ...
        KmassignworkDAO tawRmAssignworkDAO = null;
        ...
        KmdutyfileDAO tawRmDutyfileDAO = null;
        Vector vecDutyFile = null;
        if (saveSessionBeanForm == null) {
            return mapping.findForward("timeout");
        }
        // 判断是否值班
        String workserial2 = KmassignworkBO.getWorkSerial(saveSessionBeanForm.getUserid(), ds);
        ...
        try {
            ...
            tawRmAssignworkDAO = new KmassignworkDAO(ds);

            // 取得最大误差时间
            int roomid = Integer.parseInt(KmassignworkBO.getRoomId(saveSessionBeanForm.getUserid(), ds));
            KmsystemCptroomBo cptroomBO = KmsystemCptroomBo.getInstance();
            KmsystemCptroom kmsystemcptroom = cptroomBO.getKmsystemCptroomById(new Integer(
                    roomid), 0);
            int workserial = Integer.parseInt(workserial2);
            ...
            if (exchangeFlag.equals("0") || exchangeFlag.equals("1")) {
                // 判断是否值班班长
                if (!KmassignworkBO.isdutyMaster(userId, ds)) {
            ...
            roomName = kmsystemcptroom.getRoomname();
            ...
            org.apache.commons.beanutils.BeanUtils.populate(tawRmRecordForm,
                    org.apache.commons.beanutils.BeanUtils
                            .describe(tawRmRecord));
            // 附件
            tawRmDutyfileDAO = new KmdutyfileDAO(ds);
            ...
            if (tawRmRecord == null) {
                ...
                request
                        .setAttribute("TAWRMRECORD_DUTYMAN",
                                tawRmRecord_dutyman);
                // 遗留问题
                KmhangoverDAO tawRmHangoverDAO = new KmhangoverDAO(ds);
                Kmhangover tawRmHangover = tawRmHangoverDAO.retrieve(id, 0);
                String strHangQuestion = "";
                if (tawRmHangover != null) {
                    strHangQuestion = tawRmHangover.getHangQuestion();
                }
    ...
    private ActionForward performView(ActionMapping mapping,
		...
		/*	Vector bocoVector = new Vector();
            ...
            tawRmRecordForm.setRealCount(0);
            request.setAttribute("FILLEDSHEETLIST", new Vector());
		...
		/*	Vector cycleVectot = new Vector();
			...
			cycleVectot = tawRmCycleTableSub
					.getSheetListByWorkSerial(workserial);;*/
            tawRmRecordForm.setCycleCount(0);
            request.setAttribute("CYCLETABLELIST", new Vector());
    ...
    private ActionForward performExchange(ActionMapping mapping,
        ...
        KmrecordSub tawRmRecordSub = null;
        Kmsysteminfo tawRmSysteminfo = null;
        ...
        int workserial = Integer.parseInt(KmassignworkBO.getWorkSerial(userId, ds));
        int roomid = Integer.parseInt(KmassignworkBO.getRoomId(userId, ds));
        KmrecordForm form = (KmrecordForm) actionForm;
        try {
            ...
            if (CheckMainRecordExist == false) {
                ...
                // 保存值班纪录
                tawRmRecordSub = new KmrecordSub();
    ...
    private ActionForward performSubmit(ActionMapping mapping,
        ...
        String date = null;
        KmrecordSubDAO tawRmRecordSubDAO = null;
        KmrecordSub tawRmRecordSub = null;
        KmsysteminfoDAO tawRmSysteminfoDAO = null;
        Kmsysteminfo tawRmSysteminfo = null;
        GregorianCalendar cal_start = null;
        String time_start = null;
        KmreliefRecordBO tawRmReliefRecordBO = null;
        int int_stagger = 0;
        ...
        KmrecordForm form = (KmrecordForm) actionForm;
        form.setStrutsAction(KmrecordForm.EDIT);
        try {

            tawRmReliefRecordBO = new KmreliefRecordBO(ds);
            tawRmRecordDAO = new KmrecordDAO(ds);
            // 判断密码是否正确
            ...
            if (psSubmit && !boolNextWorkSerial) {
                ...
                // 改自己的子记录
                // 取得自己的值班纪录
                tawRmRecordSubDAO = new KmrecordSubDAO(ds);
                ...
                if (intWorkSerialTemp >= 0) {
                    KmrecordPerDAO tawRmRecordPerDAO = new KmrecordPerDAO(
                            ds);
		...
		/*		if (actionForm != null) {
					if (actionForm instanceof KmrecordForm) {
						KmrecordForm trrf = (KmrecordForm) actionForm;
    ...
    private ActionForward performSubmit2(ActionMapping mapping,
        ...
        form.setStrutsAction(KmrecordForm.EDIT);
        try {
            KmreliefRecordBO tawRmReliefRecordBO = new KmreliefRecordBO(ds);
            tawRmRecordDAO = new KmrecordDAO(ds);
...
*/
    ...
    private ActionForward performSubmit3(ActionMapping mapping,
        ...
        // 判断是否值班
        userId = saveSessionBeanForm.getUserid();
        if (KmassignworkBO.getWorkSerial(userId, ds).equals("0")) {
            return mapping.findForward("notonduty");
        }
        ...
        try {
            KmreliefRecordBO tawRmReliefRecordBO = new KmreliefRecordBO(ds);
            tawRmRecordDAO = new KmrecordDAO(ds);
            roomId = Integer.parseInt(KmassignworkBO.getRoomId(userId, ds));
            tawRmSysteminfoDAO = new KmsysteminfoDAO(ds);
            tawRmSysteminfo = tawRmSysteminfoDAO.retrieve(roomId + "");
            if (tawRmSysteminfo != null) {
     ...
     * @param request
     * @param response
     * @return private ActionForward performSearchdelete(ActionMapping mapping,
    ActionForm actionForm, HttpServletRequest request,
    HttpServletResponse response) {
    // edit by wangheqi 2.7 to 3.5
    TawSystemSessionForm saveSessionBeanForm = (TawSystemSessionForm) request
    ...
    private ActionForward performRecordMain(ActionMapping mapping,
        ...
        KmhangoverDAO tawRmHangoverDAO = null;
        ...
        try {
            ...
            roomid = KmassignworkBO.getRoomId(userId, ds);
            roomName = KmassignworkBO.getRoomName(userId, ds);
            ...
            // 遗留问题
            tawRmHangoverDAO = new KmhangoverDAO(ds);
    ...
    private ActionForward performAdd(ActionMapping mapping,
                                     ...
                                     HttpServletResponse response) {
        KmrecordForm form = (KmrecordForm) actionForm;
        form.setStrutsAction(KmrecordForm.ADD);
        return mapping.findForward("success");
    }
    ...
    private ActionForward performExport(ActionMapping mapping,
        ...
        try {
            tawRmRecordBO = new KmrecordBO(ds);
    ...
    private ActionForward performUpLoad(ActionMapping mapping,
        ...
        KmaddonsTableForm tawRmAddonsTableForm = (KmaddonsTableForm) actionForm;
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        response.setContentType("text/html; charset=GBK");
        String roomid = request.getParameter("typeId");
        KmrecordBO tawRmRecordBO = new KmrecordBO(ds);
