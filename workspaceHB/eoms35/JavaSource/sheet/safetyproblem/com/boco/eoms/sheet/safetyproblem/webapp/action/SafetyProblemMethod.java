...
// Source File Name:   SafetyProblemMethod.java
...
import com.boco.eoms.safetyproblemgm.IHbGmVulEmoseService;
import com.boco.eoms.safetyproblemgm.IHbGmVulEmoseServiceServiceLocator;
import com.boco.eoms.sheet.base.task.ITask;
import com.boco.eoms.sheet.base.util.UUIDHexGenerator;
import com.boco.eoms.sheet.base.webapp.action.BaseSheet;
import com.boco.eoms.sheet.safetyproblem.model.*;
...
public class SafetyProblemMethod extends BaseSheet
{

	public SafetyProblemMethod()
	{
	}
...
//			抄送短信派发end
		...
		//调用广目接口将 完成情况 传过去 start
		try {
			System.out.println("taskName===="+taskName+"====operateType="+operateType);
			Map main = (HashMap) sheetMap.get("main");
			String sheetId = StaticMethod.nullObject2String(main.get("sheetId"));
			String linkTaskComplete = StaticMethod.nullObject2String(request.getParameter("linkTaskComplete"));
			System.out.println("linkTaskComplete==="+linkTaskComplete);
			String opt = "{\"sheetNo\":\""+sheetId+"\",\"statusMsg\":\""+linkTaskComplete+"\"}";
			System.out.println("lyg==opt=="+opt);
			if(taskName.equals("ExcuteHumTask") && (operateType.equals("205") || operateType.equals("206"))){
				IHbGmVulEmoseServiceServiceLocator iservice = new IHbGmVulEmoseServiceServiceLocator();
				IHbGmVulEmoseService service= iservice.getIHbGmVulEmoseServicePort();
				System.out.println("get GM reviewByGmScan===");
				service.reviewByGmScan(opt);
			}
		} catch (Exception e) {
	...
	private static final String CONFIG_FILEPATH = "classpath:config/worksheet-sms-service-info.xml";//短信服务信息配置文件	
	   ...
	   public void workSM_NON_T (String sheetId,String receiverId,String title) throws Exception {	   
	     try {
	    	 ...
	    	 String nodeInstantName="worksheet.SafetyProblemMainFlowProcess.smsServiceId.instant";
	    	 ...
	    	 //派发通知（即时提醒）
	    	   ...
	    	   //拼写发送信息
	    	   String sendContent= "提醒您收取安全问题工单:"+ sheetId + ",主题名:" + title + ",请查阅!";
		...
		throws Exception
	{
		...
		{
			SafetyProblemLink link = (SafetyProblemLink)request.getAttribute("sheetLink");
			HashMap sessionMap = new HashMap();
			TawSystemSessionForm sessionform = (TawSystemSessionForm)request.getSession().getAttribute("sessionform");
			sessionMap.put("userId", sessionform.getUserid());
			sessionMap.put("password", sessionform.getPassword());
		...
		throws Exception
	{
		...
		SafetyProblemMain mainObject = (SafetyProblemMain)request.getAttribute("sheetMain");
		SafetyProblemTask task = (SafetyProblemTask)request.getAttribute("task");
	...
	public static String showAtomDetail(SafetyProblemMain mainObject, ITask task, String isAccept, HttpServletRequest request)
		throws DictServiceException
	{
		...
		value10.setText("com.boco.eoms.sheet.safetyproblem.model.SafetyProblemMain");
		...
		value11.setText("com.boco.eoms.sheet.safetyproblem.model.SafetyProblemLink");
