...
<logic:notPresent name="recordTotal">
    ...
    <display:table name="taskList" cellspacing="0" cellpadding="0"
        ...
        <display:column property="siUplinkUrl" sortable="true"
                        headerClass="sortable" title="${eoms:a2u('所属小区名称')}"/>

        <display:column property="sendTime" sortable="true"
                        headerClass="sortable" title="${eoms:a2u('派单时间')}"
                        format="{0,date,yyyy-MM-dd HH:mm:ss}"/>

        <display:column sortable="true" headerClass="sortable" title="${eoms:a2u('归档时间')}">
            <c:if test="${taskList.status!=0}">
                <fmt:formatDate value="${taskList.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
            </c:if>
        </display:column>
