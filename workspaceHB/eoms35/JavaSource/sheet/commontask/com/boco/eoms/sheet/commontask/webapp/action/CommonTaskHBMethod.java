...
// Source File Name:   CommonTaskHBMethod.java
...
public class CommonTaskHBMethod extends CommonTaskMethod
{

	public CommonTaskHBMethod()
	{
	}
	...
	public void showListUndoAll(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		throws Exception
	{
		ICommonTaskMainManager mainService = (ICommonTaskMainManager)ApplicationContextHolder.getInstance().getBean("iCommonTaskMainManager");
		ICommonTaskTaskManager taskService = (ICommonTaskTaskManager)ApplicationContextHolder.getInstance().getBean("icommontaskTaskManager");
		ITask commonTaskTask = (ITask)ApplicationContextHolder.getInstance().getBean("CommonTaskTask");
		HashMap filterMap = new HashMap();
		...
		ITask taskObject = commonTaskTask;
		Map condition = new HashMap();
		condition.put("mainObject", mainObject);
		condition.put("taskObject", taskObject);
		condition.put("orderCondition", orderCondition);
		...
		ArrayList phaseIdList = new ArrayList();
		Map phaseIdMap = new HashMap();
		FlowDefineExplain flowDefineExplain = new FlowDefineExplain(workflowName, "classpath:config/commontask-config.xml");
		FlowDefine flowDefine = flowDefineExplain.getFlowDefine();
		if (flowDefine != null)
		{
			PhaseId phaseIds[] = flowDefine.getPhaseId();
				...
				for (int i = 0; i < taskOvertimeList.size(); i++)
				{
					...
					if (columnMap.size() > 0)
					{
						...
						for (; it.hasNext(); tmptaskMap.put(columnMap.get(elementKey), tempcolumn))
						{
							...
							conditionMap.put(elementKey, tempcolumn);
							j++;
			System.out.println("---showListUndoPersonal---elementKey=" + elementKey + "elementvalue=" + columnMap.get(elementKey) + "tempcolumn=" + tempcolumn);
			}
             		} else
					{
						tmptask = (ITask)taskOvertimeList.get(i);
