...
<%
    ...
    String flag5 = StaticMethod.nullObject2String(request.getAttribute("flag5"));
    ...
    if (flag4 != null && flag4.equals("flag4"))
        audit = "2";
    if (flag5 != null && flag5.equals("flag5"))
        audit = "4";
...
<form name="form1" method="post">
    ...
    <table border="0" width="100%" cellspacing="1" cellpadding="1" class="table_show" align=center>
        ...
        <logic:iterate id="tawInformation" name="TAWINFORMATIONS" type="com.boco.eoms.kbs.model.TawInformation">
            <tr class="tr_show">
                <td width="30%">
                    <input type="checkbox" name="chk_sel_id" id="<%=tawInformation.getId()%>"
                           value="<bean:write name="tawInformation" property="id" scope="page"/>">
                    <%
                        out.print("<input type=\"hidden\" name=\"chk_sel_pid\" id=\"" + tawInformation.getId() + "\" value=\"" + 1 + "\">");


                    %>
                    <a href="view.do?id=<%=tawInformation.getId()%>&boardId=<%=tawInformation.getBoardId()%>"
                       title="<%=tawInformation.getName()%>">
                ...
                <td width="20%">
                    <bean:write name="tawInformation" property="publicer" scope="page"/>
                </td>
                <td width="30%">
                    <bean:write name="tawInformation" property="publicDept" scope="page"/>
                </td>
                <td width="20%">
        ...
        <tr>
            <td align="right">
                ...
                <%
                    ...
                    if (audit.equalsIgnoreCase("1")) {
                %>
                <input type="button" Class="clsbtn2" value="<bean:message key="label.remove" />"
                       onclick="return onRemove(<%=listNum%>);">
                <input type="button" Class="clsbtn2" value="<bean:message key="label.add"/>" onclick="onAdd();">
                <input type="button" Class="clsbtn2" value="待审核" onclick="onCheck();">
                <input type="button" Class="clsbtn2" value="待修改" onclick="onModi();">
                <input type="button" Class="clsbtn2" value="未采纳" onclick="onDeny();">
                <input type="button" Class="clsbtn2" value="<bean:message key="label.classify"/>"
                       onclick="return onClassify(<%=listNum%>,<%=boardId%>);">
                <input type="button" Class="clsbtn2" value="<bean:message key="TawInformation.query"/>"
                       onclick="onQuery();">
                <%} else if (audit.equalsIgnoreCase("0")) {%>
                <input type="button" Class="clsbtn2" value="<bean:message key="label.remove"/>"
                       onclick="return onRemove(<%=listNum%>);">
                ...
                <%
                } else if (audit.equalsIgnoreCase("2")) {%>
                <input type="button" Class="clsbtn2" value="<bean:message key="label.remove"/>"
                       onclick="return onRemove(<%=listNum%>);">
