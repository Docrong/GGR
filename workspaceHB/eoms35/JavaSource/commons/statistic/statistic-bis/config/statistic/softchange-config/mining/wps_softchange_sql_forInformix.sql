...
from softchange_main

select *
from softchange_link select *
from softchange_task
...
select case when '2008-09-16 06:41:53' - current >0 then 1 else 0 end
from dual

select *
from common create view v_test
select TOORGROLEID, ACCEPTFLAG, ACCEPTTIME
from softchange_link TOORGROLEID VARCHAR2(510) Y
...
------------------------------VIEW----------------------------------------
select *
from v_softchange select distinct *
from v_softchange
...
--bussines main
    , MAINNETTYPEONE, MAINISSECURITY, MAINISCONNECT, MAINFACTORY, MAINSOFTCDKEY, MAINSOFTPATCHKEY, MAINISALLOW,
                         ...
                         MAINREJECTTIMES, MAINNETTYPETWO, MAINNETTYPETHREE, SENDYEAR, SENDMONTH, SENDDAY, MAINIFRECORD,
                         MAINEXECUTEENDDATE
--bussines link
    , LINKCOMPLETELIMITTIME, LINKINVOLVEDPROVINCE, LINKINVOLVEDCITY, LINKDESIGNKEY, LINKISCHECK, LINKPERMITRESULT,
                         ...
                         LINKISNEEDPROJECT, OPERATERCONTACT, OPERATEDAY, OPERATEYEAR, OPERATEMONTH, LINKFAILEDREASON
    ) as
    (
--已处理
        select
...
values (43, '2007-07-07 00:00:00', '软件变更工单基础表采集的上一次操作的时间');
...
from softchange_link link

SELECT count(distinct mainid)
FROM softchange_link
...
from softchange_link delete from est_softchange
select *
from est_softchange drop table est_softchange
--中间表
