...
<caption><div class="header center"><%if(!isShowLanguage.equals("yes")){ %>语音专线
								<%}else { %>传输专线<%} %>	
</div></caption>
...
<table class="formTable">
	...
	<tr>  
	    ...
	    <td colspan="3">
			<c:forTokens items="${languagespeciallineForm.callUserOpenBrand}" delims="," var="callUserOpenBrand" varStatus="status"><eoms:id2nameDB id="${callUserOpenBrand}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
    </tr>
	<tr>  
	    <td class="label">主叫用户归属地</td>
	    <td colspan="3">
	    	<c:forTokens items="${languagespeciallineForm.callUserLocalZone}" delims="," var="callUserLocalZone" varStatus="status"><eoms:id2nameDB id="${callUserLocalZone}" beanId="tawSystemAreaDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
	...
	<tr>  
	    ...
	    <td colspan="3">
			...
			<c:forTokens items="${languagespeciallineForm.businessOpenZone}" delims="," var="businessOpenZone" varStatus="status"><eoms:id2nameDB id="${businessOpenZone}" beanId="tawSystemAreaDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
    </tr>
...
<tr>  
	...
	<td colspan="3">
	<c:forTokens items="${languagespeciallineForm.userCallScope}" delims="," var="userCallScope" varStatus="status"><eoms:id2nameDB id="${userCallScope}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
	</td>
</tr>  
	<tr>
