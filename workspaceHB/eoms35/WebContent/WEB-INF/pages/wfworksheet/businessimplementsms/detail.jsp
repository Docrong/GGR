...
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&taskStatus=${taskStatus}&specialtyType=<%=specialtyType %>&sheetType=businessImplementSms&taskName=${taskName}" width="100%" height="300px"></iframe>
<%}%>
    </logic:present>
  </div>
...
<div class="sheet-deal">
	<div class='sheet-deal-header'>
		...
		<%}else if(taskName.equals("DraftTask")){%>	
			<select id="dealSelector">
			  <option value="${urlShowDraftDeal}"><bean:message bundle="businessimplementsms" key="operateType.draft"/></option>
			</select>
		...
		<%}else if(taskName.equals("HoldTask")){ %>
			<select id="dealSelector">
			  <option value="${urlShowHoldDealPage}"><bean:message bundle="businessimplementsms" key="operateType.hold"/></option>
			</select>		
		<%} %>
		</td>
		</tr>
