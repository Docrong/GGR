...
//out.println("disVec.size()::"+disVec.size());
        ...
        String strSql = "";
        try {
            strSql = Entity.getIPQuerySql(colVec, typeid, manufid, cityid, "", 1);
    ...
    %>
    <td align=center class=ttTable><b><font size=2 color=#000000
                                            face='Verdana, Arial, Helvetica, sans-serif'>查询出的设备数量为<%=rs.getRowCount()%>
    </font></b>
    </td>
    <br><br>
    <table bgcolor=#dddddd width='100%' border=0 cellspacing=1 id="resultRs">
        <tr>
            <%
                ...
                out.println("<td align=center class=ttTable><b><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'>" + disName + "</font></b></td>");
                ...
                for (int Col = 0; Col < disVec.size(); Col++) {
                    ...
                    out.println("<td align=center class=ttTable><b><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'>" + SysColIndex.getCc_name() + "</font></b>");
                    out.println("</td>");
                }
            %>
        </tr>
        <%
            ...
            if (rs.getRowCount() >= 1) {
                for (int i = 0; i < rs.getCurrentPageRowNum(); i++) {
                    ...
                    out.println("<td align=center class=td><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'>" + "<a href=../edit/editDetail.jsp?id=" + rs.getString(1) + "&tabname=" + typeid + ">" + rs.getString(2) + "" + "</font></td>");
                    out.println("<td align=center class=td><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'>" + rs.getString(3) + "</font></td>");
                    for (int col = 0; col < disVec.size(); col++) {
                        String colValue = rs.getString(col + 4);
                        if (colValue == null)
                            colValue = "";
        ...
        <table width=100%>
            <tr>
                ...
                <td height=20 align=right><font size=2 color=#000000
                    ...
                    页 / 共<%=rs.getPageCount()%>页 / 共<%=rs.getRowCount()%>条记录 </font></td>
                    <%}%>
