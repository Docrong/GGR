        ...
        <table border='0' cellpadding='0' cellspacing='0' width='100%'>
            <c:forEach var="todoList" items="${requestScope.todoLists}">
                <tr>
                    ...
                    <td class='portlet-table-td-left' width='80%' style="text-decoration: line-through;">
                        <input TYPE='checkbox' name='<c:out value="${todoList.name}"/>' checked='yes' value='1'
                               onClick="javascript:changeStatus('<%= responseId %>',this.name);"><c:out
                                value="${todoList.name}"/></input>
                        </c:if>
                        <c:if test="${todoList.value =='0'}">
                    <td class='portlet-table-td-left' width='80%' style="color: rgb(0, 0, 255);">
                        <input TYPE='checkbox' name='<c:out value="${todoList.name}"/>' value='1'
                               onClick="javascript:changeStatus('<%= responseId %>',this.name);"><c:out
                                value="${todoList.name}"/></input>
                        </c:if>
                    </td>
                    <td class='portlet-table-td-right'>
                        <input type="image" src="portal/light/images/edit.gif" name="<c:out value='${todoList.name}'/>"
                        ...
                        <input type="image" src="portal/light/images/deleteLink.gif"
                               name="<c:out value='${todoList.name}'/>"
            ...
            <tr>
                <td class='portlet-table-td-left'>
                    <a href='javascript:void(0);' onClick="javascript:newToDO('<%= responseId %>');"><fmt:message
                            key="portlet.button.addToDo"/></a>
                </td>
                <td></td>
            </tr>
        </table>
