...
<script language="JavaScript">
    ...
    function Transfer() {
        ...
        callerWindowObj.document
    .<%=formName%>.<%=name%>.
        value = '<%=resulturl%>';
        callerWindowObj.document
    .<%=formName%>.<%=numname%>.
        value = tawRmCycleTableForm.num.value;
        callerWindowObj.document
    .<%=formName%>.<%=executeContentUserIdName%>.
        value = tawRmCycleTableForm.executecontentuserid.value;
...
<fmt:bundle basename="config/ApplicationResources-workplan">
    ...
    <table class="formTable">
        ...
        <tr>
            <td width="30%" class="label"><%if (!action.equals("read")) {%><fmt:message
                    key="uploadfile.tawwpexecute.labFileRemove"/><%} else {%>><fmt:message
                    key="uploadfile.tawwpexecute.labFileDown"/> <%}%></td>
            <td>
                <%
                    String[] n = null;
        ...
        <tr>
            <td colspan="2" align=center>
                ...
                <input type="button" name="submitBut" value="<fmt:message key='uploadfile.tawwpexecute.btnComplete' />"
                       class="button" onclick="Transfer();WinClose();">
            </td>
        </tr>
        <%}%>
