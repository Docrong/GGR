...
<div id="sheetinfo"><logic:present name="sheetMain"
	...
	<table id="sheet" class="formTable">
		...
		<tr>
			...
			<td colspan="3"><pre>${sheetMain.mainDescRemark}</pre></td>		
		</tr>
		<tr>
		...
		<tr>
		    ...
		    <td class="label" colspan="4" width="120"> 
		   		<iframe height="400"  width="100%" src="mofficedata.do?method=showProducers&mainId=${sheetMain.id}&preLink=${task.correlationKey}&taskName=OfficeMadeTask"></iframe> 
		    </td>
		</tr>	
		</c:when>
		<c:otherwise>
		<tr>
					...
					<tr>
		    <td class="label" colspan="4" width="120"> 
		   		<iframe height="400"  width="100%" src="mofficedata.do?method=showProducers&mainId=${sheetMain.id}&preLink=${preLinkId}&taskName=${taskName}"></iframe> 
		    </td>
		</tr>	
		</c:otherwise>
		</c:choose> 
...
<table>
	<tr>
		...
		if (taskName.equals("AuditDataTask")) {%> 
			...
			<%} else if (taskStatus.equals(Constants.TASK_STATUS_CLAIMED)) {%>
				...
				<%} else {%>
					...
					<!-- 不需要等待回复 -->
					<%if (ifwaitfor.equals("false")) {%>
					<option value="${urlShowDispatchPage}"><bean:message bundle="mofficedata" key="operateType.subTaskReply" /></option>
					<%}%>
				<%}%>
	  ...
	  <%} else if (taskName.equals("HoldTask")) {%> 
	  	<select id="dealSelector">
			<option value="${urlShowHoldTaskPage}"><bean:message bundle="mofficedata" key="operateType.holdTask" /></option>
		</select> 
		...
		<%} else if (taskName.equals("DraftTask")) { %> 
 		<select id="dealSelector">
			<option value="${urlShowDraftPage}"><bean:message bundle="mofficedata" key="operateType.DraftTask" /></option>
		</select> 
		<%}%>
		</td>
	</tr>
