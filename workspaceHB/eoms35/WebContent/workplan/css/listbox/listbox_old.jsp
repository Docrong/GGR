    ...
    <SCRIPT language="jscript">
        ...
        function fnEOMSListboxAddRow(elName) {
            ...
            if (alertinfo == "") {
                with (window.document.all(elName)) {
                    ...
                    //addItem("C", listBox.text, listBox.value, -1, "", pageSelected.ID);
                }
            } else {
                alert(alertinfo);
            }
        ...
        function load(divName, divID) {
            ...
            if (targetTR.style.display != "block") {
                ...
                bufferFrame.document.location = "loadtree.jsp?divID=" + divID + "&user=<%=strUseroff%>";
...
<table width="100%" height="100%-32" border="0" cellspacing="1" cellpadding="0" bgcolor="#BEDEF8">
	...
	<SPAN class="EOMSListbox" id=listboxRoot onchange="jscript: fnEOMSListboxOnChange('listboxRoot');"
          style="margin:5px;">
      <TABLE cellSpacing=0 cellPadding=0>
        <TBODY id=rootTbody>
        <TR class=EOMSListboxItem id="tr_parentid_1">
          <TD class=EOMSListboxImage>
		  <IMG id="img_parentid_1" name="img_parentid_1" src="images/false.gif" onclick="load('',1)"
               style="cursor:hand"></TD>
          <TD class=EOMSListboxCaption>省公司</TD>
          <TD class=EOMSListboxValue>1</TD>
          <TD class=EOMSListboxTag></TD></TR>

			<tr style="display:none" id="tr_treeid_1">
			<td width=5><IMG height=5 src="" width=5></td>
			<td id="td_treeid_1" colspan="3">
...
<% for (int b = 0; b < arrSort.length; b++) { %>
	...
	<SPAN class="EOMSListbox" id="listboxSort<%=b+1%>"
          style=" WIDTH: 100%; HEIGHT: expression( (document.body.offsetHeight - 95 - <%=arrSort.length%> * 3) / <%=arrSort.length%>)"
