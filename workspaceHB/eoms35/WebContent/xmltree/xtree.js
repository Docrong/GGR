...
if (rootPath == 'report' || rootPath == 'filemanager' || rootPath == 'template') {     //����ϵͳ�����ڸ�Ŀ¼�µ����
    rootPath = "";
} else {
    rootPath = "/" + rootPath;
}
...
function reportParseError(errObj)//��ʽ����������
{
    var s = "";
...
function reportRuntimeError(errObj)//��ʽ�����д���
{
...
//Post xml to server
function PostOrder(xmldoc, xmlManage) {    //alert(xmlManage);
    xmlHttp = new ActiveXObject("MSXML2.XMLHTTP");
...
function FinishSave() {
    if (xmlHttp != null && xmlHttp.readyState == 4) {
        ...
        if (xmlHttp.status != 200) {
            var returnXml = "<return><returnInfo result='0' errInfo='xmlhttp״̬" + xmlHttp.status + " " + xmlHttp.statusText.replace(/\\'/g, "\'") + "'/><treeInfo/></return>"
        } else {
