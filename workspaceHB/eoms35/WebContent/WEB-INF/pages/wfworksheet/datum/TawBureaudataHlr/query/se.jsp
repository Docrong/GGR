...
%>
        <td align="center"><html:link page="/TawBureaudataHlr/recover.do" name="map" scope="page" onclick="return confirm('是否确定恢复该数据?');"><img src="<%=request.getContextPath()%>/images/bottom/an_hf.gif" border="0" alt="<bean:message key="label.edit"/>"></html:link></td>
        <td align="center"><html:link page="/TawBureaudataHlr/truncate.do" name="map" scope="page" onclick="return confirm('注意:这是不可恢复的操作!是否确定永久删除此数据?');"><img src="<%=request.getContextPath()%>/images/bottom/an_yj.gif" border="0" alt="<bean:message key="label.remove"/>"></html:link></td>
  </tr>
              </logic:iterate>
