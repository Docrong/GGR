...
<%@ page import="com.boco.eoms.commons.system.role.service.ITawSystemSubRoleManager,
           com.boco.eoms.commons.system.session.form.TawSystemSessionForm,java.util.List,
...
List list=roleManager.getSubRoles(sessionform.getUserid(),"324");
if(list == null || list.size() == 0) response.sendRedirect("/eoms35/sheetPriv.jsp");
 %>
<script type="text/javascript">
	...
	tabs.addTab('sheetform', "<bean:message bundle="securityevaluate" key="securityevaluate.header"/>");
	...
	if ('${templateId}'!= null && '${templateId}' != "") {
		mgr.update("${app}/sheet/securityevaluate/securityevaluate.do?method=showTemplateInputSheetPage&templateId=${templateId}&parentCorrelation=${parentCorrelation}&parentSheetId=${parentSheetId}&parentSheetName=${parentSheetName}&parentPhaseName=${parentPhaseName}");
	} else {
		mgr.update("${app}/sheet/securityevaluate/securityevaluate.do?method=showNewInputSheetPage&processname=securityevaluateMainProcess&parentCorrelation=${parentCorrelation}&parentSheetId=${parentSheetId}&parentSheetName=${parentSheetName}&parentPhaseName=${parentPhaseName}");
	}
	   	...
	   	if (templateManage == "templateManage") {
		   	...
		   	if (type == "new"){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "securityevaluate.do?method=saveTemplate&templateId=${templateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
  ...
  function removeTemplate() {
   if (confirm('<bean:message bundle="sheet" key="worksheet.delete.confirm" />')) {
		...
		thisform.action = "${app}/sheet/securityevaluate/securityevaluate.do?method=removeTemplate&templateId=${templateId}&type=${type}";
		thisform.submit();
	}
 }
</script>
...
<html:form action="/securityevaluate.do?method=newPerformAdd" styleId="theform">
	...
	<logic:notPresent name="templateManage">
		...
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./securityevaluate.do?method=getTemplatesByUserId',null,'left=300,top=150,height=400,width=500,scrollbars=yes')">
