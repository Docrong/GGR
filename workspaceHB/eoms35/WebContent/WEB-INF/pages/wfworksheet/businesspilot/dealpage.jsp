	...
	<table>
	  ...
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61' && taskName !='reject'}">
	   <c:if test="${taskName != 'draft'}">  
	   	<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./businesspilot.do?method=getDealTemplatesByUserId&sheetKey=${sheetMain.id}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskStatus=${taskStatus}&operateType=${operateType}',null,'left=300,top=150,height=400,width=500,scrollbars=yes')">
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/business/businesspilot.do?method=performClaimTask'" 
    ...
    <c:if test="${taskName=='pilot'&&operateType!='11'&&operateType!='4'}">
	    ...
	    <div class="form-btns hide" id="btns">
	      ...
	      <input type="submit" class="submit" onclick="saveAlert();v.passing=true;this.form.action='${app}/sheet/business/businesspilot.do?method=performSaveInfo'" value="<bean:message bundle='sheet' key='button.save'/>" />
	      <input type="submit" class="submit" onclick="saveAlert();v.passing=true;this.form.action='${app}/sheet/business/businesspilot.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	       </c:if>
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/business/businesspilot.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	    </div>    
    </c:otherwise>    	   
   </c:choose>
