...
<!-- added by liufuyuan -->
   <tr>
     ...
     <td class="content"><eoms:id2nameDB id="${orderSheet.customLevel}" beanId="ItawSystemDictTypeDao"/></td>
   </tr>
   <tr>
...
<%if("yes".equals(isCalledByLangugage)){ System.out.println("1----"+orderSheetId+isCalledByLangugage+startType);%>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&taskStatus=${taskStatus}&specialtyType=yuyin&sheetType=businessImplement&taskName=${taskName}" width="100%" height="300px"></iframe>

<%}else if(("startTranfer").equals(startType)){ System.out.println("-2---"+orderSheetId+isCalledByLangugage+startType);%>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&taskStatus=${taskStatus}&specialtyType=<%=CaiXinOrDuanXin %>&sheetType=businessImplement&taskName=${taskName}" width="100%" height="300px"></iframe>
<%}else if(("startIp").equals(startType)){ System.out.println("-3---"+orderSheetId+isCalledByLangugage+startType);%>

<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&taskStatus=${taskStatus}&specialtyType=<%=CaiXinOrDuanXin %>&sheetType=businessImplement&taskName=${taskName}" width="100%" height="300px"></iframe>

<%}else { System.out.println("--4--"+orderSheetId+isCalledByLangugage+startType);%>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&taskStatus=${taskStatus}&specialtyType=${sheetMain.mainSpecifyType}&sheetType=businessImplement&taskName=${taskName}" width="100%" height="300px"></iframe>
<%} %>
<%}%>
...
<div class="sheet-deal">
	<div class='sheet-deal-header'>
		...
		<%}else if(taskName.equals("DraftTask")){%>	
			<select id="dealSelector">
			  <option value="${urlShowCompleteDraftPage}"><bean:message bundle="businessimplement" key="operateType.draft"/></option>
			</select>
		...
		<%}else if(taskName.equals("HoldTask")){ %>
			<select id="dealSelector">
			  <option value="${urlShowHoldDealPage}"><bean:message bundle="businessimplement" key="operateType.hold"/></option>
