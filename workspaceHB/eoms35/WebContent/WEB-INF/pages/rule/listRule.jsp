    ...
    <div id="rulegroups" style="padding:10px">
        ...
        <table class="member-table" cellspacing="0">
            <form action="<html:rewrite page='/RuleWebToolAction.do?method=detailRuleGroup'/>" method="post">
                ...
                <tr>
                    <td colpsan="2"><input type="submit" value="<bean:message key='rule.table.button.query'/>"/></td>
                </tr>
            </form>
        </table>
        <table class="member-table" cellspacing="0">
            ...
            <c:forEach items="${ruleList}" var="item">
                <tr>
                    <td>
                        <a href="<html:rewrite page='/RuleWebToolAction.do?method=detailRuleGroup&id=${item.id}'/>">${item.name}</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
    ...
    <div id="rules" style="padding:10px">
        ...
        <table class="member-table" cellspacing="0">
            ...
            <c:forEach items="${ruleList}" var="item">
                <tr>
                    <td>
                        <a href="<html:rewrite page='/RuleWebToolAction.do?method=detailRule&id=${item.id}'/>">${item.name}</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
