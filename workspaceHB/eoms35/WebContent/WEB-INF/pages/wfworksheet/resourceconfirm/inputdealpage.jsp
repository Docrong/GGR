	...
	<c:when test="${task.subTaskFlag == 'true'}">
     ...
     <logic:present name="serviceList" scope="request">	
      ...
      <logic:iterate id="toServiceList" name="serviceList" type="java.util.Map"><%=++i%>
			  	    <input type="radio" name="${sheetPageName}checkTaskName"  id ="${sheetPageName}checkTaskName" value="${toServiceList.taskName}"  onclick='support()'>
                    ...
                    <input type="hidden" name="${sheetPageName}specialtyId" id="${sheetPageName}specialtyId" value="${toServiceList.specialtyId}" /> 			  	          
      </logic:iterate>
			  	    <input type="button" class="btn" value="派发任务单" onclick="javascript:openwin()"> <font color="RED"> 请按先后顺序派发任务单</font>   			  	    
 			  	    </td>    
                    </tr>
     </logic:present>
		...
		<tr>
			...
			%>
				<input type="radio" name="${sheetPageName}supportId" id="${sheetPageName}supportId"	value=<%=business.getId() %> />
      ...
      <tr>
	      ...
	      ${eoms:date2String(toprocessTasksList.endtime)}
				...
				<td class="label">客户端工程能力确认*</td>
				  <td colspan="3">
	                 <textarea name="${sheetPageName}mainBusinessDesc" alt="allowBlank:false,maxLength:255,vtext:'最多输入255字符'" id="${sheetPageName}mainBusinessDesc"  class="textarea max">${sheetMain.mainBusinessDesc}</textarea>
				  </td>	
				 </tr>
			  <tr>
			  	<td class="label"><bean:message bundle="resourceconfirm" key="resourceconfirm.linkSendSheetDesc"/>*</td>
			    <td colspan="3">
			      <textarea name="${sheetPageName}linkSendSheetDesc" alt="allowBlank:false,maxLength:255,vtext:'最多输入255字符'" id="${sheetPageName}linkTestDesc"  class="textarea max">${sheetLink.linkSendSheetDesc}</textarea>
			    </td>
			  </tr>			         
         <%}} %>
      <!-- 用户端资源勘查反馈 -->
...
<c:if test="${sheetMain.mainspecialtyType==101220101}">
				...
				<tr>
				  <td class="label"><bean:message bundle="resourceconfirm" key="resourceconfirm.linkPreInvestment"/>（单位：万元）*</td>
				  <td class="content">
				  ...
				  <td class="label"><bean:message bundle="resourceconfirm" key="resourceconfirm.linkPreCompleteDays"/>（单位：天）*</td>
				  <td class="content">
...
<c:if test="${sheetMain.mainspecialtyType==101220102}">
				<tr>				
				  ...
				  <td class="label"><bean:message bundle="resourceconfirm" key="resourceconfirm.linkPreCompleteDaysA"/>（单位：天）*</td>
				  <td class="content">
...
<c:if test="${sheetMain.mainSpecifyType== '101230104'}">
			<tr>
			    ...
			    <td colspan="3">
			      <textarea name="${sheetPageName}mainProductsNumber" alt="allowBlank:true,maxLength:255,vtext:'请填描述，最多输入255字符'" id="${sheetPageName}mainProductsNumber"  class="textarea max">${sheetMain.mainProductsNumber}</textarea>
			    </td>
			  </tr>	
			  <tr>
		      <td class="label">
		    ...
		    <eoms:attachment name="tawSheetAccess" property="accesss" 
     		...
     		<!-- 组网方案拓扑图  -->
	     		...
	     		<td colspan="3">
	     			<eoms:attachment name="sheetLink" property="linkProgramTopology" scope="request" idField="${sheetPageName}linkProgramTopology" appCode="resourceconfirm" alt="allowBlank:false"/>
				 </td>
		     </tr> 
</c:if>
<c:if test="${sheetMain.mainSpecifyType != '101230104'}">
		...
		<td colspan="2">
		 ...
		 <html:button styleClass="btn" property="" styleId="method.save"
			...
			<tr>
			  	<td class="label">预计投资</td>
			    <td class="content">
			      <input type="text"  class="text" name="${sheetPageName}mainProductsNumber" id="${sheetPageName}mainProductsNumber"  value="${sheetMain.mainProductsNumber}" alt="allowBlank:false"/>
			    </td>
			    <td class="label">预计完成天数</td>
				  <td class="content">
	                 <input type="text"  class="text" name="${sheetPageName}mainProductsSerialNum" id="${sheetPageName}mainProductsSerialNum"  value="${sheetMain.mainProductsSerialNum}" alt="allowBlank:false"/>
				  </td>	
			  </tr>	
     		...
     		<!-- 组网方案拓扑图  -->
	     		...
	     		<td colspan="3">
	              ...
	              scope="request" idField="${sheetPageName}accessories" appCode="resourceconfirm" alt="allowBlank:false"/>
				 </td>
		     </tr>  
		     </c:if>    
         ...
         <%if(taskName.equals("AuditTask")){ %>
             ...
             <input type="hidden" name="${sheetPageName}phaseId" id="${sheetPageName}phaseId" value=HandleTask />
				<tr>
				  ...
				  <td colspan="3">
				  	 <eoms:comboBox name="${sheetPageName}linkDealResult" id="${sheetPageName}linkDealResult" 
				  	       initDicId="1012212" defaultValue="${sheetLink.linkDealResult}" alt="allowBlank:false" onchange="ifAuditPass(this.value)"/>
				  </td>
		        </tr>			  
		  <tr>
