/*******************************************************************************
 ��ҳ���ؼ����� ��BOCO�����2003��12��-2004��1�£���������Ȩ����
 *********************************************************************************/
function TableControl(tbl, caption, dataType, tcStyle, tblStyle, rowStyle, colStyle) {
    try {
        var tc = this;
        tc.err = "";
        tc.col = [];
        tc.tbl = tbl;
        tc.copywrite = "Copyright by BOCO (c)2003��All right reserved!"
        if (!document.getElementById) {
            throw new Error(999, "Your browser dosen't support TableControl function,IE5.0+ is recommended.");
        }
        ...
        tc.setCaption = function (c) {
            ...
            for (var i = 0; i < tc.col.length; i++) {
                tc.col[i].innerHTML = c[i] == null ? "&nbsp;" : c[i];
            }
        }
        ...
        tc.setTcStyle = function (style) {
            var defCss = "border:1 outset;height:21;text-align:center;white-space:nowrap;overflow:hidden;background: #9CB8F4;cursor:hand;";
            //var defCss="font-size:9pt;color=#04308C;padding:1px;border-width: 1px;background: #9CB8F4;bordercolor: #9BB7EB;cursor: hand";
            tcStyle = style == null ? "" : style;
            for (var i = 0; i < tc.col.length; i++) tc.col[i].style.cssText = defCss + tcStyle;
        }
        tc.setTblStyle = function (style) {
            if (style != null) {
                tc.tbl.style.cssText = style;
                tblStyle = style;
            }
        }
        ...
        var tblSort = function (tbl, colIndex, By, DataType) {
            ...
            for (i = 0; i < arySort.length; i++) {
                tbl.lastChild.appendChild(arySort[i][1]);
            }
        }
        var load_js = function (src) {
            ...
            for (var i = 0; i < script.length; i++) {
                ...
                if (s.indexOf("TableControl.js") != -1) {
                    jsPath = s.replace("TableControl.js", "")
                }
            }
        ...
        Array.prototype.indexOf = function (elm) {
            for (var i = 0; i < this.length; i++) if (elm == this[i]) return i;
            return -1;
        }

        if (tbl == null) {
            throw new Error(999, "Miss table object.The first argument is required!");
            ...
            if (tbls.length > i) {
                ...
                throw new Error(999, "Table index value out of range.");
            }
        } else if (typeof (tbl) != "object" || tbl.tagName != "TABLE") {
            throw new Error(999, "The first argument isn't a table object.");
        }
        tbl = tc.tbl
        var tr = tbl.rows
        ...
        with (tc.ctr) {
            style.cssText = "border:0;font-size:9pt;color:black;margin:0;padding:0;border-collapse:collapse;background-color:buttonface;table-layout:fixed;cursor:hand;"
        ...
        tc.contextMenu = function () {
            var menu = function () {
                ...
                mnu.add = function (Text) {
                    ...
                    item.style.cssText = "padding-top:2;padding-left:14;font-size:12px;height:21;cursor:default;color:black;background-color:buttonface;width:100%;"
                    item.innerText = Text
                ...
                mnu.remove = function (idx) {
                    mnu.item[idx].removeNode(true);
                }
                mnu.show = function () {
            ...
            tc.tblMenu = tblMenu
            tc.ctrMenu = ctrMenu
            tblMenu.add("����һ��").onclick = function () {
                if (ri == -1 || ri == 0) return;
                tr[ri].swapNode(tr[ri - 1]);
                tc.setRowStyle(rowStyle);
            }
            tblMenu.add("����һ��").onclick = function () {
                if (ri == -1 || ri == tr.lenght - 1) return;
                tr[ri].swapNode(tr[ri + 1]);
                tc.setRowStyle(rowStyle);
            }
            tblMenu.add("ɾ����ǰ��").onclick = function () {
                if (ri == -1) return;
                tc.tbl.deleteRow(ri);
                tc.setRowStyle(rowStyle);
            }
            ctrMenu.add("��������").onclick = function () {
                if (ci == -1) return;
            ...
            ctrMenu.add("ɾ����ǰ��").onclick = function () {
                if (ci == -1) return;
            ...
            ctrMenu.add("����").onclick = find
            tblMenu.add("����").onclick = find
        }
        ...
        tc.doResize = function () {
            ...
            for (var i = 0; i < tc.col.length; i++) {
                tc.col[i].onmouseover = function () {
                    if (!isSize) ctrCol = this;
                }
                ...
                tc.col[i].onmousemove = function () {
                    if (isSize) {
                        ...
                        for (var i = 0; i < tr.length; i++) {
                            if (tr[i].cells[idx]) tr[i].cells[idx].setAttribute("width", tdW + iSpan);
                        }
                        return;
                    }
                    isSort = this.offsetWidth - event.offsetX > 8 && event.offsetX > 5;
        ...
        tc.err = "TableControl cause run time error!\nError number:" + e.number + ".\nError description:" + e.description;
        //alert(tc.err);
        return tc;
    }
}
