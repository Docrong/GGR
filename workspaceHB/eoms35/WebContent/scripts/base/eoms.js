...
eoms.Element = {
     ...
     */
    hasClass: function (c) {
        return this.className.hasSubString(c, " ");
    },
    ...
    select: function (selector) {
        return eoms.DomQuery.select(selector, this);
    },
...
eoms.DomQuery = function () {
    ...
    function byAttribute(cs, attr, value, op, custom) {
        ...
        var f = eoms.DomQuery.operators[op];
        for (var i = 0, ci; ci = cs[i]; i++) {
            var a;
            if (st) {
                a = eoms.DomQuery.getStyle(ci, attr)
            } else {
                if (attr == "class" || attr == "className") {
                    a = ci.className
                } else {
    ...
    function byPseudo(cs, name, value) {
        return eoms.DomQuery.pseudos[name](cs, value)
    }
    ...
    return {
        getStyle: function (el, name) {
            return eoms.$(el).getStyle(name)
        },
        compile: function (path, type) {
            type = type || "select";
            var fn = ["var f = function(root){\n var mode; ++batch; var n = root || document;\n"];
            var q = path, mode, lq;
            var tk = eoms.DomQuery.matchers;
            var tklen = tk.length;
            var mm;
        ...
        select: function (path, root, type) {
            ...
            for (var i = 0, len = paths.length; i < len; i++) {
                ...
                if (!cache[p]) {
                    cache[p] = eoms.DomQuery.compile(p);
                    if (!cache[p]) {
        ...
        selectNode: function (path, root) {
            return eoms.DomQuery.select(path, root)[0]
        },
        selectValue: function (path, root, defaultValue) {
            path = path.replace(trimRe, "");
            if (!valueCache[path]) {
                valueCache[path] = eoms.DomQuery.compile(path, "select")
            }
        ...
        selectNumber: function (path, root, defaultValue) {
            var v = eoms.DomQuery.selectValue(path, root, defaultValue || 0);
            return parseFloat(v)
        },
        is: function (el, ss) {
            if (typeof el == "string") {
            ...
            var result = eoms.DomQuery.filter(isArray ? el : [el], ss);
            return isArray ? (result.length == el.length) : (result.length > 0)
        },
        filter: function (els, ss, nonMatches) {
            ss = ss.replace(trimRe, "");
            if (!simpleCache[ss]) {
                simpleCache[ss] = eoms.DomQuery.compile(ss, "simple")
            }
        ...
        pseudos: {
            "first-child": function (c) {
                ...
                return eoms.DomQuery.filter(c, ss, true)
            }, odd: function (c) {
        ...
        readOnly: function () {
            ...
            for (var i = 0; i < a.length; ++i) {
                ...
                if (eoms.$(a[i]).type == "select-one") {
                    eoms.$(a[i]).onmouseover = function () {
                        this.setCapture();
                    };
                    eoms.$(a[i]).onmouseout = function () {
                        this.releaseCapture();
                    };
                } else {
