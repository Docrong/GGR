...
<script type="text/javascript">
    function editx(id) {
        location.href = "netownership.do?method=xedit&id=" + id;
    }

    function deletex(id) {
        if (confirm("确认删除吗?")) {
            Ext.Ajax.request({
                url: "netownership.do?method=xdelete",
                method: 'POST',
                params: {id: id},
                success: function (x) {
                    ...
                    Ext.each(data, function (d) {
                        alert(d.alarmMsg);
                        if (d.alarmMsg == '删除成功') {
                            location.href = "netownership.do?method=showQueryPage";
                        }
                    });
    ...
    function deleteBatch() {
        ...
        if (s == '') {
            ...
            if (confirm("确认删除吗?")) {
                Ext.Ajax.request({
                    url: "netownership.do?method=xdeleteSome",
                    method: 'POST',
                    params: {ids: s},
                    success: function (x) {
                        var data = eoms.JSONDecode(x.responseText);
    ...
    function editBatch() {
        ...
        if (s == '') {
            ...
            Ext.Ajax.request({
                url: "netownership.do?method=xeditSome",
                method: 'POST',
                params: {
                    ids: s,
                    center: center,
...
<% String source = request.getParameter("6578706f7274");
    if (source == null) {
%>
<jsp:include page="/WEB-INF/pages/wfworksheet/netownership/batchJs.jsp"/>
<jsp:include page="/WEB-INF/pages/wfworksheet/netownership/listsendUndoJS.jsp"/>
<%} %>
<bean:define id="url" value="netownership.do"/>
<display:table name="taskList" cellspacing="0" cellpadding="0"
               id="taskList" pagesize="${pageSize}" class="table businessdesignMain"
               export="true" requestURI="netownership.do"
               sort="list" size="total" partialList="true"
               decorator="com.boco.eoms.sheet.netownership.webapp.action.NetOwnershipDisplaytagDecoratorHelper">
