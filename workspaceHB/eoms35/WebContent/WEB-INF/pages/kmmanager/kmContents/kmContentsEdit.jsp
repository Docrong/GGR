        ...
        <table class="formTable">
            ...
            <tr>
                ...
                <td class="content">
                    <eoms:dict key="dict-kmmanager" dictId="rolestrFlag" isQuery="false"
                               defaultId="${KmContentsMap.ROLESTR_FLAG}" selectId="TableInfo/ROLESTR_FLAG"
                               beanId="selectXML"
                ...
                <td class="content">
                    <eoms:dict key="dict-kmmanager" dictId="levelFlag" isQuery="false"
                               defaultId="${KmContentsMap.LEVEL_FLAG}" selectId="TableInfo/LEVEL_FLAG"
                               beanId="selectXML"
            ...
            <tr>
                ...
                <td class="content" colspan="3">
                    <input type="text" name="TableInfo/CONTENT_TITLE" id="CONTENT_TITLE"
                           value="${KmContentsMap.CONTENT_TITLE}" maxLength="100" class="text max"
                           alt="allowBlank:false,vtext:'请输入知识标题...'"/>
                </td>
            </tr>
                            ...
                            <c:when test="${item.colDictType == 0}">
                                <c:choose>
                                    <c:when test="${item.colType == 1}">
                                        <!-- 不绑定_普通文本 -->
                                        <input type="text" name="TableInfo/${item.colName}" id="${item.colName}"
                                               value="${KmContentsMap[item.colName]}" maxLength="${item.colSize}"
                                               class="text medium"
                                    ...
                                    <c:when test="${item.colType == 2}">
                                        ...
                                        <textarea name="TableInfo/${item.colName}" id="${item.colName}"
                                                  onkeyup="this.value=this.value.slice(0, 1800)"
                                                  ...
                                                  type="_moz">${KmContentsMap[item.colName]}</textarea>
                                    </c:when>

                                    <c:when test="${item.colType == 3}">
                                        <!-- 不绑定_数字类型 -->
                                        <input type="text" name="TableInfo/${item.colName}" id="${item.colName}"
                                               value="${KmContentsMap[item.colName]}" class="text medium"
                                    ...
                                    <c:when test="${item.colType == 4}">
                                        <!-- 不绑定_日期时间 -->
                                        <input type="text" name="TableInfo/${item.colName}" id="${item.colName}"
                                               value="${KmContentsMap[item.colName]}" size="20"
                                    ...
                                    <c:when test="${item.colType == 7}">
                                        ...
                                        <eoms:attachment name="KmContents" property="${item.colName}" scope="request"
                                                         idField="TableInfo/${item.colName}" appCode="kmmanager"/>
                                    </c:when>
                                </c:choose>
                            </c:when>
                                    ...
                                    <c:when test="${item.colType == 5}">
                                        ...
                                        <c:if test="${item.subNode != '' and (item.parentNode == null or item.parentNode == '')}">
                                            <c:if test="${item.isNullable == 1}">
                                                <eoms:comboBox name="TableInfo/${item.colName}" id="${item.colName}"
                                                               initDicId="${item.colDictId}" sub="${item.subNode}"
            ...
            <tr>
                ...
                <td class="content" colspan="3">
                    <input type="text" name="TableInfo/CONTENT_KEYS" id="CONTENT_KEYS"
                           value="${KmContentsMap.CONTENT_KEYS}" maxLength="100" class="text max"
                           alt="allowBlank:false,vtext:'请输入知识关键字...'"/>
                </td>
            </tr>
        </table>
