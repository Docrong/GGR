...
<html:hidden property="id"/>
    ...
    <li>
        ...
        <input type="text" id="deptName" readonly="readonly" name="deptName" class="text" value="<eoms:id2nameDB id="${sheetLimitForm.deptId}" beanId="tawSystemDeptDao" />"/>
...
<logic:present name="specialtyList" scope="request">
  <logic:iterate id="roleFilter" name="specialtyList" type="com.boco.eoms.commons.system.role.util.RoleFilter" length="4" offset="0" indexId="number">
  	<div style="margin-bottom:10px"><label>${roleFilter.name}</label>
	  <div>
	  	  <c:choose>
	  	    <c:when test="${number=='0'}">
	  	      <eoms:comboBox name="specialty1" id="${roleFilter.businessName}" defaultValue="${sheetLimitForm.specialty1}" initDicId="${roleFilter.dictId}" 
	  	       sub="${roleFilter.sub}" size="1" styleClass="select-class"/>
	  	   </c:when>
	  	    <c:when test="${number=='1'}">
	  	      <eoms:comboBox name="specialty2" id="${roleFilter.businessName}" defaultValue="${sheetLimitForm.specialty2}" initDicId="${sheetLimitForm.specialty1}" 
	  	       sub="${roleFilter.sub}" size="1" styleClass="select-class"/>
	  	   </c:when>
	  	    <c:when test="${number=='2'}">
	  	      <eoms:comboBox name="specialty3" id="${roleFilter.businessName}" defaultValue="${sheetLimitForm.specialty3}" initDicId="${sheetLimitForm.specialty2}" 
	  	       sub="${roleFilter.sub}" size="1" styleClass="select-class"/>
	  	   </c:when>
	  	   <c:otherwise>
	  	      <eoms:comboBox name="specialty4" id="${roleFilter.businessName}" defaultValue="${sheetLimitForm.specialty4}" initDicId="${sheetLimitForm.specialty3}" 
	  	       sub="${roleFilter.sub}" size="1" styleClass="select-class"/>
	  	   </c:otherwise>
		  </c:choose>
    ...
    <li>
        ...
        <eoms:comboBox name="stepId" id="stepId" initDicId="10121"  sub="roleName" defaultValue="${sheetLimitForm.stepId}" styleClass="select-class"/>
    </li>
