...
var webFXTreeConfig = {
    ...
    forwardIcon: "images/forward.gif",
    backIcon: "images/back.gif",
...
_p.getRowHtml = function getRowHtml() {
    var t = this.getTree();
    var strTemp = "";
    strTemp += "<div class=\"" + this.getRowClassName() + "\" style=\"padding-left:" +
...
_p.getLabelHtml = function getLabelHtml() {
    ...
    return "<a href=\"" + webFXTreeHandler.textToHtml(this._getHref()) +
        "\" class=\"webfx-tree-item-label " + this.status + "\" tabindex=\"-1\"" +
...
_p.getIconHtml = function getIconHtml() {
    ...
    var t = this.getTree();
    if (t.showRootNode && this == t) {
        return "<img class=\"webfx-tree-icon\" src=\"" + this.getIconSrc() + "\">";
    } else {
        var strTemp = "<span>";
        //strTemp += "<img class=\"webfx-tree-icon\" src=\"" + this.getIconSrc() + "\">";
        if (this.back != "")
            strTemp += "<a href='" + this.backAction + "' class=\"webfx-tree-back-icon\"><img src=\"" + webFXTreeConfig.backIcon + "\"/>" + this.back + "</a>";
        if (this.forward != "")
            strTemp += "<a href='" + this.forwardAction + "' class=\"webfx-tree-forward-icon\">" + this.forward + "<img src=\"" + webFXTreeConfig.forwardIcon + "\"/></a>";
        strTemp += "</span>";
        return strTemp;
    }
...
_p._onclick = function _onclick(e) {
    ...
    //forward/back icon
    if (/webfx-tree-forward-icon/.test(el.className)) {
        if (typeof this.forwardAction == "function") {
            this.forwardAction();
        } else if (this.forwardAction != null) {
            window.open(this.forwardAction, this.target || "_self");
        }
        return false;
    }

    if (/webfx-tree-back-icon/.test(el.className)) {
        if (typeof this.backAction == "function") {
            this.backAction();
        } else if (this.backAction != null) {
            window.open(this.backAction, this.target || "_self");
        }
        return false;
    }
...
function WebFXTreeItem(sText, oAction, eParent, sIcon, sOpenIcon,
                       sForward, sForwardAction, sBack, sBackAction, sStatus) {
    WebFXTreeAbstractNode.call(this, sText, oAction);
    if (sIcon) this.icon = sIcon;
    if (sOpenIcon) this.openIcon = sOpenIcon;

    /* added by mios >> */
    if (sForward) this.forward = sForward;
    else this.forward = "";

    if (sForwardAction) this.forwardAction = sForwardAction;
    else this.forwardAction = "";

    if (sBack) this.back = sBack;
    else this.back = "";

    if (sBackAction) this.backAction = sBackAction;
    else this.backAction = "";

    if (sStatus) this.status = sStatus;
    else this.status = "status-finish";
    /* added by mios << */
