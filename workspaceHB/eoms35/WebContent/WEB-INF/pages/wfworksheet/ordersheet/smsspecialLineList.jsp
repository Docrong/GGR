...
>
    <display:column headerClass="sortable" title="详细信息">
        ...
        <a onclick="window.open('${app}/businessupport/product/smsspeciallines.do?method=showDetail&orderId=<%=orderId%>&sheetType=<%=sheetType%>&taskName=<%=taskNamePara%>&id=<%=((HashMap)pageContext.getAttribute("list")).get("id")%>&isShowSms=<%=isShowSms %>&specialtyType=<%=specialtyType %>');"
    ...
    <display:column media="html" title="行业网关接入勘察">
        <a onclick="popupIrmsPreSurvey('<%=((HashMap)pageContext.getAttribute("list")).get("id")%>','inputLanguage');"
           style="cursor:pointer">行业网关接入勘察</a>
    </display:column>
    <%}%>
