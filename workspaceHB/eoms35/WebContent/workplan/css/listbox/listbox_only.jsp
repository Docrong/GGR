        ...
        function fnEOMSListboxMoveRow(elName) {
            if (objSelectedItems.count() == 0 && objSelectedItems.getSelectedGroup() == null)
                ...
                switch (objSelectedItems.action) {
                    case "add" :
                        fnEOMSListboxAddRow(elName);
                        break;
                    case "del" :
                        fnEOMSListboxDelRow(elName);
                        break;
                }
            }
        }
...
//					"\nitem.tagInfo=" + item.tagInfo);
                    ...
                    //判断此项目能否被选择
                    if (item.property == "dept" && getRealID(item.ID) == mydept && selectself != "yes") {
                        alertItemInfo += "不能对本部门进行操作，请选择其他部门。";
                    }
                    if (item.property == "user" && getRealID(item.ID) == myuser && selectself != "yes") {
                        alertItemInfo += "不能对自己进行操作，请选择其他人员。";
                    }

                    //1229增加只能派发一个部门 开始
                    if (item.property == "dept" && elName == arrSortID[0]) {
                        for (var iii = 0; iii < itemCount(); iii++) {
                            if (getItemProperty(iii) == "dept") {
                                alertItemInfo += "\n只能派发到一个部门。";
                                break;
                            }
                        }
                    ...
                    if (checkall != "no") {
                        for (j = 0; j < arrSortID.length; j++) {
                            if (inBox(arrSortID[j], item.value, item.property)) {
                                alertItemInfo += "\n该项目\"" + item.text + "\"已经被选择了.不能重复选择。";
                                break;
                            }
                            if (item.property == "user" && inBox(arrSortID[j], item.parentValue, "dept")) {
                                alertItemInfo += "\n人员\"" + item.text + "\"所属的部门已经被选择了, 该人员不能被选择。";
                                break;
                            }
//						alert(item.value);
                            if (item.property == "dept" && myuserinBox(arrSortID[j], item.value)) {
                        ...
                        if (inBox(elName, item.value, item.property)) {
                            alertItemInfo += "\n该项目\"" + item.text + "\"已经被选择了.不能重复选择。";
                        }
                        if (item.property == "user" && inBox(elName, item.parentValue, "dept")) {
                            alertItemInfo += "\n人员\"" + item.text + "\"所属的部门已经被选择了, 该人员不能被选择。";
                        }
                        if (item.property == "dept" && myuserinBox(elName, item.value)) {
                            alertItemInfo += "\n部门\"" + item.text + "\"下的人员已经被选择了，该部门不能被选择。";
                        }
                    }
                    ...
                    if (alertItemInfo == "") {
                        if (item.property == "dept")
                            addItem("I", item.text, item.value, -1, deptIcon.src, item.ID);
...
//				alert(item.text+"ok");
        ...
        function inBox(boxName, targetValue, targetProperty) {
            with (window.document.all(boxName)) {
                for (var i = 0; i < itemCount(); i++) {
                    if (targetProperty == getItemProperty(i) && targetValue == getItemValue(i)) return true;
                }
                return false;
            }
        }

        function myuserinBox(boxName, targetValue) {
            with (window.document.all(boxName)) {
                for (var i = 0; i < itemCount(); i++) {
//				alert("getItemProperty="+getItemProperty(i)+"getItemParentValue="+getItemParentValue(i));
                    if (getItemProperty(i) == "user" && targetValue == getItemParentValue(i)) return true;
                }
                return false;
            }
        }
    ...
    <style>
        BOCO\:EOMSTab {
            POSITION: relative;
            leftImage: url(../tab/images/tabLeft.gif);
            centerImage: url(../tab/images/tabCenter.gif);
            rightImage: url(../tab/images/tabRight.gif);
            leftImageOver: url(../tab/images/tabLeftOver.gif);
            centerImageOver: url(../tab/images/tabCenterOver.gif);
            rightImageOver: url(../tab/images/tabRightOver.gif);
            leftImageDown: url(../tab/images/tabLeftDown.gif);
            centerImageDown: url(../tab/images/tabCenterDown.gif);
            rightImageDown: url(../tab/images/tabRightDown.gif)
        }
    </style>
                ...
                <TABLE cellSpacing=0 cellPadding=0 width=100% border=0 align="center">
                    ...
                    <TR>
                        <TD valign="top" width="50%">
                            ...
                            <!-- 组 开始------------------------>
                            <SPAN class="EOMSListbox" id=groupRoot
                                  onchange="jscript: fnEOMSListboxOnChange('groupRoot');" style="margin:5px;">
...
<c:choose>
	...
	<TD class="EOMSListboxImage"><IMG id="groupimg_parentid_<%=groupId%>" name="groupimg_parentid_<%=groupId%>"
                                      src="images/false.gif" onclick="openGroup(<%=groupId%>)" style="cursor:hand">
        <!--<IMG src="<%=webapp%>/sysadmin/flow/listbox/images/group.gif">--></TD>
    ...
    <TD class="EOMSListboxTag"></TD>
			...
			<td id="grouptd_treeid_<%=groupId%>" colspan="3">
			...
			<%
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    out.println("<td class=EOMSListboxTag></TD></tr>");
                }
            %>
				</tbody></table>
                            ...
                            <!-- 部门树 开始------------------------>
                            <SPAN class="EOMSListbox" id=listboxRoot
                                  onchange="jscript: fnEOMSListboxOnChange('listboxRoot');"
...
%>
        <TR class=EOMSListboxItem id="tr_parentid_<%=deptid%>">
		  ...
		  <IMG id="img_parentid_<%=deptid%>" name="img_parentid_<%=deptid%>" src="images/false.gif"
               onclick="load('',<%=deptid%>)" style="cursor:hand"></TD>
          ...
          <TD class=EOMSListboxTag></TD></TR>
			...
			<td width=5><IMG height=5 src="" width=5></td>
			<td id="td_treeid_<%=deptid%>" colspan="3">
...
<% for (int b = 0; b < arrSort.length; b++) { %>
	...
	<BOCO:genericButton css="classic" onclick="fnEOMSListboxMoveRow('listboxSort<%=b+1%>');"
                        image="<%=request.getContextPath()%>/css/listbox/images/btnarrow.gif">
	 <%=arrSort[b]%>
	</BOCO:genericButton>
	</td></tr>
<% } %>
...
<% for (int b = 0; b < arrSort.length; b++) { %>
	<tr><td>
	<SPAN class="EOMSListbox" id="listboxSort<%=b+1%>"
          style=" WIDTH: 100%; HEIGHT: expression( (document.body.offsetHeight - 90 - <%=arrSort.length%> * 3) / <%=arrSort.length%>)"
          onchange="fnEOMSListboxOnChange('listboxSort<%=b+1%>');"
          ondblclick="fnEOMSListboxDelRow('listboxSort<%=b+1%>')">
      <TABLE cellSpacing=0 cellPadding=0>
...
<% } %>
	...
	<SPAN>
        ...
        <tr>
            <td bgcolor="#E2F1FC">
                &nbsp;显示人员:<%
                if (strUseroff.equals("no")) out.print("否");
                else {
                    out.print("是");
                }
            %>&nbsp;|
                &nbsp;允许选择多个操作:<%
                if (strCheckall.equals("no")) out.print("是");
                else {
                    out.print("否");
                }
            %>&nbsp;|
                &nbsp;可以选择本部门:<%
                if (strSelectSelf.equals("yes")) out.print("是");
                else {
                    out.print("否");
                }
            %>&nbsp;|
            ...
            <td colspan="2" class="buttonBanner" height="30">
                ...
                <TABLE cellpadding="5" width="100%">
                    <TR>
                        ...
                        <TD width="50">
                            <BOCO:genericButton onclick="<%=strOnclick%>window.returnValue=true;window.close();">
                                确定 </BOCO:genericButton></TD>
                        <TD width="50">
