...
import com.boco.eoms.sheet.businessbackout.model.BusinessBackoutMain;
...
public class BusinessBackoutAction extends SheetAction {
    /**
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward performDeal(ActionMapping mapping, ActionForm form,
        ...
        if (sheetKey.equals(""))
            ...
            try {
                BusinessBackoutMain main = (BusinessBackoutMain) baseSheet.getMainService().getSingleMainPO(sheetKey);
                System.out.println("isCustomerFlag=" + main.getIsCustomerFlag() + "##");
                if (main.getIsCustomerFlag() != null && main.getIsCustomerFlag().trim().equals("1")) {    //接口派单
                    ...
                    if (sendImmediately.equalsIgnoreCase("true")) {
                        ...
                        System.out.println("serviceType=" + sType);
                        ...
                        if (taskName.equals("ExcuteHumTask")
                            ...
                            String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(request.getParameterMap(), StaticMethod.getFilePathForUrl("classpath:config/businessBackout-crm.xml"), nodeName);
                            CrmLoader.withdrawWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSBACKOUT, serviceType,
                                    main.getParentCorrelation(), opDetail, attachRef);
                        } else if (phaseType.equals("") && operateType.equals("9")) {// 阶段回复
                            String nodeName = "notifyWorkSheet";
                            String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(request.getParameterMap(), StaticMethod.getFilePathForUrl("classpath:config/businessBackout-crm.xml"), nodeName);
                            CrmLoader.notifyWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSBACKOUT, serviceType,
                                    main.getParentCorrelation(), opDetail, attachRef);
                        } else if (operateType.equals("205")) {// 回复
                            String taskId = StaticMethod.nullObject2String(request.getParameter("TKID"));
                            ITaskService iTaskService = (ITaskService) ApplicationContextHolder.getInstance().getBean("ibusinessbackoutTaskManager");
                            ...
                            String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(request.getParameterMap(), StaticMethod.getFilePathForUrl("classpath:config/businessBackout-crm.xml"), nodeName, chNameList, enNameList, contentList);
                            CrmLoader.replyWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSBACKOUT, serviceType,
                                    main.getParentCorrelation(), opDetail, attachRef);
                        }
                    }
                } else
                    System.out.println("isCustomerFlag<>1");
            } catch (Exception err) {
                System.out.println("crm failed");
                err.printStackTrace();
            }
        }
            ...
            if (main.getIsCustomerFlag() != null && main.getIsCustomerFlag().trim().equals("1")) {    //接口派单
                ...
                if (sendImmediately.equalsIgnoreCase("true")) {
                    ...
                    if (taskName.equals("ExcuteHumTask") && operateType.equals("61")) {// 受理
                        ...
                        String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(request.getParameterMap(), StaticMethod.getFilePathForUrl("classpath:config/businessBackout-crm.xml"), nodeName);
                        CrmLoader.confirmWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSBACKOUT, serviceType,
                                main.getParentCorrelation(), opDetail, attachRef);
                    }
                }
            }
