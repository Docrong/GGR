                        ...
                        <td class="msource">
                            <select name="groupRuleId">
                                <c:forEach items="${rules.rules.rule}" var="ruleIdItem">
                                    <option value="${ruleIdItem.id }"
                                            <c:if test="${ruleIdItem.id == groupRefItem.ruleId }">selected</c:if>>${ruleIdItem.id}</option>
                                </c:forEach>
                            </select>
                        </td>
                    ...
                    <form action="<html:rewrite page='/RuleWebToolAction.do?method=addGroup'/>" method="post">
                        ...
                        <table class="member-table" cellspacing="0" id="ruleTable-UID-">
                            <tr ondblclick="javascript:toggleRows(this)">
                                ...
                                <th class="msource"><input class="wide" value="Group description"
                                                           name="groupDescription" size="20"></th>
                                <th class="msource" style="text-align:right">
                                    <input type="submit" onclick=""
                                           value="<bean:message key='rule.table.button.submit'/>"/>
                                    <input type="button"
                                           onclick="toggleRows(this,'show');Dyna.all['ruleTable-UID-'].add()"
                                           value="<bean:message key='rule.table.button.newRule'/>">
                                    <input type="button" onclick="removeTable(this);"
                                           value="<bean:message key='rule.table.button.deleteGroup'/>">
                                </th>
                            </tr>
                            <tr>
                                <td class="micon">&nbsp;</td>
...
<img src="${app}/images/icons/plus.png" onclick="javascript:togglet('rules',this);" alt="<bean:message key='rule.table.title.openclose'/>">
 -->
            ...
            <div class="detail-wrap" id="rules">
                <div class="mdetail">
                    <input type="button" onClick="Dyna.all['group'].add();"
                           value="<bean:message key='rule.table.button.newGroup'/>">
                </div>
                <div id="group-table" class="mcontent">
                        ...
                        <form action="<html:rewrite page='/RuleWebToolAction.do?method=updateGroup'/>" method="post">
                            ...
                            <table class="member-table" cellspacing="0" id="ruleTable-${groupItem.id}">
                                ...
                                <tr ondblclick="javascript:toggleRows(this)">
                                    ...
                                    <th class="mdesc"><bean:message key="rule.table.title.id"/></th>
                                    <th class="msource">
                                        <input type="text" value="${groupItem.id}" name="mgroupId" class="wide"
                                               size="30"/></th>
                                    <th class="mdesc"><bean:message key="rule.table.title.description"/></th>
                                    <th class="msource"><input type="text" value="${groupItem.description }"
                                                               name="groupDescription" size="30" class="wide"/></th>
                                    <th class="msource" style="text-align:right">
                                        <input type="submit" onclick=""
                                               value="<bean:message key='rule.table.button.submit'/>">
                                        <input type="button"
                                               onclick="toggleRows(this,'show');Dyna.all['ruleTable-${groupItem.id}'].add()"
                                               value="<bean:message key='rule.table.button.newRule'/>">
                                        <!-- <input type="button" onclick="removeTable(this);" value="Delete Rule Group">-->
                                        <input type="button" onclick="delGroup('${groupItem.id }');"
                                               value="<bean:message key='rule.table.button.deleteGroup'/>"></th>
                                </tr>
                                <c:forEach items="${groupItem.groupRef}" var="groupRefItem" varStatus="status">
                                    <tr>
                                        ...
                                        <td class="mdesc"><bean:message key="rule.table.title.pri"/></td>
                                        <td class="msource"><input type="text" name="groupRulePri"
                                                                   value="${groupRefItem.pri}" class="wide"/></td>
                                        <td class="mrowbtns">
                                            <!-- <a href="#" onclick="removeRow(this);">删除</a> --><input name="button2"
                                                                                                         type=button
                    ...
                    <table width="600">
                        <tr>
                            ...
                            <td width="15%"><a
                                    href="<html:rewrite page='/RuleWebToolAction.do?method=listRule'/>"><bean:message
                                    key="rule.table.title.return"/></a></td>
                        </tr>
                        <form name="pageForm" id="pageForm" method="post"
                              action="<c:url value='RuleWebToolAction.do?method=detailRuleGroup'/>">
                            <input type="hidden" name="id" value="${rule.id }"/>
                            <input type="hidden" name="currPage" value="1" id="currPage2"/>
                            <input type="hidden" name="qgroupId" value="${qgroupId }"/>
                            <input type="hidden" name="qdescription" value="${qdescription }"/>
                        </form>
                        <form name="delGroupForm" id="delGroupForm" method="post"
                              action="<html:rewrite page='/RuleWebToolAction.do?method=delGroup'/>">
                            <input type="hidden" name="id" value="${rule.id }"/>
