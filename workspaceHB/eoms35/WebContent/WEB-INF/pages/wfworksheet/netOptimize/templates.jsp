...
function check() {
	...
	if (templateId == "") {
		...
		if (parent == null)
			return;
		window.opener.location.href = "${app}/sheet/netOptimize/netOptimize.do?method=referenceTemplate&templateId="+templateId ;
		window.close();
	}
...
<bean:define id="url" value="netOptimize.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true" headerClass="sortable" title="${eoms:a2u('模板名称')}">
			<a href="${app}/sheet/netOptimize/netOptimize.do?method=editTemplateInfo&templateId=${taskList.id}&type=${templateManage}">
