...
>
    <display:column headerClass="sortable" title="详细信息">
        <a onclick="window.open('${app}/businessupport/product/languagespeciallines.do?method=showDetail&orderId=<%=orderId%>&sheetType=<%=sheetType%>&taskName=<%=taskNamePara%>&id=<%=((HashMap)pageContext.getAttribute("list")).get("id")%>&isShowLanguage=<%=isShowLanguage %>');"
           style="cursor:pointer">查看详细信息</a>
    </display:column>
    ...
    <display:column media="html" title="查看资源使用情况">
        <a onclick="viewOccupyRes('<%=((HashMap)pageContext.getAttribute("list")).get("id")%>');"
           style="cursor:pointer">查看资源使用情况</a>
    </display:column>
    <%}%>
    <%if (taskNamePara.equals("UserTask") && !taskStatus.equals("2")) {%>
    <display:column media="html" title="客户端勘查">
        <a onclick="popupIrmsPreSurvey('<%=((HashMap)pageContext.getAttribute("list")).get("id")%>','inputUser');"
           style="cursor:pointer">客户端勘查</a>
    </display:column>
    <%}%>
    <%if ((taskNamePara.equals("AccessTask")) && !taskStatus.equals("2")) {%>
    <display:column media="html" title="接入点勘查">
        <a onclick="popupIrmsPreSurvey('<%=((HashMap)pageContext.getAttribute("list")).get("id")%>');"
           style="cursor:pointer">接入点勘查</a>
    </display:column>
    <%}%>
