...
<c:choose>
    ...
    <c:when test="${taskName=='ByRejectHumTask'}">
        <c:set var="methodValue" value="showDraftPage"/>
        <c:set var="operateType" value="52"/>
    </c:when>
    <c:otherwise>
...
<c:url var="urlDeal" value="/sheet/accountpopedomapply/accountpopedomapply.do">
...
<script type="text/javascript">
    ...
    function initPage() {
        ...
        if ("${taskName}" != "cc" && "${operateType}" != '4' && "${operateType}" != '11') {
            v.preCommitUrl = "${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performPreCommit";
        }
    }
    ...
    Ext.onReady(function () {
        ...
        if (dealTemplateId != null && dealTemplateId != "" && taskName != "DraftHumTask" && taskName != "ByRejectHumTask") {
            strUrl = '${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId=' + dealTemplateId;
        }
        var config = {
            url: strUrl,
            callback: initPage
    ...
    function changeType1() {
        $('${sheetPageName}phaseId').value = "AuditHumTask";
    ...
    function saveDealInfo() {
        ...
        if (v.check()) {
            Ext.Ajax.request({
                form: ajaxForm,
                method: "post",
                url: "${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performSaveInfo",
                success: function () {
                    alert("保存成功！");
                }
            });
        ...
        if (confirm("请注意，不保存派发对象")) {
            if (type == "new") {
                Ext.Ajax.request({
                    form: ajaxForm,
                    method: "post",
                    url: "accountpopedomapply.do?method=saveDealTemplate",
                    success: function () {
                        alert("确认保存模板吗？");
                    }
                });
                ...
                Ext.Ajax.request({
                    form: ajaxForm,
                    method: "post",
                    url: "accountpopedomapply.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}",
                    success: function () {
                        alert("保存模板成功！");
                    }
                });
    ...
    function saveTemplate(type) {
        ...
        if (confirm("确认保存模板吗？")) {
            if (templateManage == "templateManage") {
                form.action = "${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=saveTemplate&templateId=${templateId}&type=${type}";
                form.submit();
            } else {
                if (type == "new") {
                    Ext.Ajax.request({
                        form: ajaxForm,
                        method: "post",
                        url: "accountpopedomapply.do?method=saveTemplate&type=${type}",
                        success: function () {
                            alert("保存模板成功！");
                        }
                    });
                    ...
                    Ext.Ajax.request({
                        form: ajaxForm,
                        method: "post",
                        url: "accountpopedomapply.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
                        success: function () {
                            alert("保存模板成功！");
                        }
                    });
            ...
            <tr>
                <td width="100%" align="left">
                    <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&&taskName !='ByRejectHumTask' && taskName!='DraftHumTask'}">
                        ...
                        <input type="hidden" name="dictKey" value="dict-sheet-accountpopedomapply"/>
                    </c:if>
                </td>
            </tr>
        </table>
        ...
        <c:choose>
            <c:when test="${operateType=='61'}">
                <div class="form-btns" id="btns">
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performClaimTask'"
            ...
            <c:when test="${taskName=='DraftHumTask'}">
                <div class="form-btns hide" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performSaveInfoAndExit&draft=1'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=showDraftList'"
            ...
            <c:when test="${taskName=='ByRejectHumTask'}">
                <div class="form-btns" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performSaveInfoAndExit'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
            ...
            <c:otherwise>
                ...
                <div class="form-btns hide" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/accountpopedomapply/accountpopedomapply.do?method=performSaveInfo'"
