        ...
        if (xboxName.indexOf(objName) < 0) {
            ...
            if ('${chooseType}' == 'user') {
                new xbox({
                    btnId: objName,
                    dlgId: 'hello-dlg3',
                    treeDataUrl: userFromDeptAction,
                    treeRootId: '-1',
                    treeRootText: '人员',
                    treeChkMode: 'single',
                    treeChkType: 'user',
                    showChkFldId: objName,
                    saveChkFldId: objId
                });
            }
    ...
    function newBoxDict(objName, objId, dictId, dictName) {
        if (xdictName != objName) {
            ...
            new xbox({
                btnId: objName,
                dlgId: 'hello-dlg5',
                treeDataUrl: dictAction,
                treeRootId: dictId,
                treeRootText: dictName,
                treeChkMode: '',
                treeChkType: 'dict',
                showChkFldId: objName,
                saveChkFldId: objId
...
<table id="sheet" class="formTable">
    ...
    <tr style="display:none">
        <td class="label"><!-- 设备厂家 --> <bean:message
                bundle="mofficedata" key="mofficeDataMain.mainDeviceFa"/>*
        </td>
    ...
    <tr>
        ...
        <td colspan="3">
            ...
            <select name="mainStyle5" id="mainStyle5" onchange="changeModeType1(this);"
                    alt="allowBlank:false,vtext:'请选择专业科室！'">
    ...
    <tr>
        ...
        <td class="content">
            ...
            <select name="mainSendModeType" id="mainSendModeType" onchange="changeModeType(this.value);"
                    alt="allowBlank:false,vtext:'请选择制作模式！'">
                <option value="">请选择</option>
        ...
        <td class="label"><bean:message bundle="mofficedata"
                                        key="mofficeDataMain.mainDataNo"/>*
        </td>
        <td class="content"><input type="text" class="text"
    ...
    <tr>
        <td class="label"><bean:message bundle="mofficedata"
                                        key="mofficeDataMain.mainDescRemark"/>*
        </td>
            ...
            <table id="sheet2" class="formTable">
                ...
                <tr>
                    ...
                    <td class="label" align="center">操作<c:if test="${delFlag=='true' }">
                        <img src='${app}/images/icons/add.gif' border='0' onclick="addBasestation()"
                ...
                <c:forEach var="mat" items="${prolist}" varStatus="status">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" class="text max" readonly="readonly" name="producerName"
                                   id="producerName${status.index + 1}" value="${mat.producerName }"/>
                            <input type="hidden" name="producerId" id="producerId${status.index + 1}"
                                   alt="allowBlank:false,vtext:'请选择制作人！'" value="${mat.producerId}"/>
                        </td>
                        <td>
                            <input type="text" class="text max" readonly="readonly" value="${mat.netName }"
                                   name="netName" id="netName${status.index + 1}"/>
                            <input type="hidden" name="netId" id="netId${status.index + 1}" value="${mat.netId }"
                                   alt="allowBlank:false,vtext:'请选择网元类型！'"/>
                        </td>
                            ...
                            <select name="mainStyle9" alt="allowBlank:false">
                                ...
                                <logic:iterate id="cityList" name="cityList">
                                    <option value="${cityList.keyId}"
                                            <c:if test="${mat.mainStyle9==cityList.keyId}">selected="selected"</c:if>>${cityList.keyName}</option>
                                </logic:iterate>
                            </select>
                        </td>

                        <td>
                            <input type="text" class="text max" readonly="readonly" value="${mat.mainStyle10}"
                                   name="mainStyle10" id="mainStyle10${status.index + 1}"/>
                        </td>


                        <td>
                            <input type="text" class="text max" readonly="readonly" name="deviceName"
                                   id="deviceName${status.index + 1}" value="${mat.deviceName }"/>
                            <input type="hidden" name="deviceId" id="deviceId${status.index + 1}" value="101010360"
                                   alt="allowBlank:false,vtext:'请选择设备厂家！'"/>
                        </td>
                        <td><a id="newProject" href="" target="_blank"></a></td>
                        <td>
                            <iframe id="UIFrame1-accessories${status.index + 1}"
                                    name="UIFrame1-accessories${status.index + 1}" class="uploadframe" frameborder="0"
                                    scrolling="auto"
                            ...
                            <input type="hidden" name="accessories" id="accessories${status.index + 1}"
                                   alt="allowBlank:false,blankText:'请上传附件！'"/></td>
                        <td align="center">
                            <c:if test="${delFlag=='true' }">
                                <img src='${app}/images/icons/delete.gif' border="0"
                                     onclick="deleteMaterialByNets(this.parentElement.parentElement,'${mat.id}')"
                                     style="vertical-align: middle;">
                            </c:if>
                        </td>
                    </tr>
        ...
        <c:if test="${sheetMain.mainSendModeType==1}">
            <table id="sheet3" class="formTable">
                ...
                <tr>
                    ...
                    <td class="label" align="center">网元类型</td>
                    <td class="label" align="center">操作<c:if test="${delFlag=='true' }">
                        <img src='${app}/images/icons/add.gif' border='0' onclick="addBasestation3()"
                             style="vertical-align: middle;">
                    </c:if></td>
                </tr>
                <c:forEach var="mat" items="${prolist}" varStatus="status">
                    ...
                    <td>
                        <input type="text" class="text max" readonly="readonly" value="${mat.netName }" name="netName"
                               id="netName${status.index + 1}"/>
                        <input type="hidden" name="netId" id="netId${status.index + 1}" value="${mat.netId }"
                               alt="allowBlank:false,vtext:'请选择网元类型！'"/>
                    </td>
                    <td>
    ...
    <c:otherwise>
        <table id="sheet2" class="formTable">
            ...
            <tr>
                ...
                <td class="label" align="center">业务标准模板</td>
                <td class="label" align="center">上传附件</td>
                <td class="label" align="center">操作<img src='${app}/images/icons/add.gif' border='0'
                                                        onclick="addBasestation()" style="vertical-align: middle;"></td>
            </tr>
            <tr>
        ...
        <table id="sheet3" class="formTable">
            ...
            <tr>
                ...
                <td class="label" align="center">操作<img src='${app}/images/icons/add.gif' border='0'
                                                        onclick="addBasestation3()" style="vertical-align: middle;">
                </td>
            </tr>
            <tr>
                <input type="hidden" name="proid" value=""/>
