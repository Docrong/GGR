...
<html:form action="tawWorkbenchReports" method="post" styleId="tawWorkbenchReportForm">
        ...
        <c:if test="${flag!=null&&flag=='week'}">
            <input type="button" style="margin-right: 5px"
                   onclick="location.href='<c:url
                           value="/workbench/report/tawWorkbenchReports.do?method=add&flag=week"/>'"
                   value="<fmt:message key="button.add"/>"/>
        </c:if>
    ...
    <display:table name="tawWorkbenchReportList" cellspacing="0" cellpadding="0"
                   ...
                   export="false" requestURI="/workbench/report/tawWorkbenchReports.do?method=listUnAudited"
        ...
        <c:if test="${flag==null||flag!='week'}">
            <display:column property="reportPersonName" sortable="true" headerClass="sortable"
                            url="/workbench/report/tawWorkbenchReports.do?method=edit" paramId="id" paramProperty="id"
                            titleKey="tawWorkbenchReportForm.reportPersonName"/>
        </c:if>

        <c:if test="${flag!=null&&flag=='week'}">
            <display:column property="reportPersonName" sortable="true" headerClass="sortable"
                            url="/workbench/report/tawWorkbenchReports.do?method=edit&flag=week" paramId="id"
        ...
        <c:if test="${flag==null||flag!='week'}">
            <display:column
                    titleKey="shenhe" url="/workbench/report/tawWorkbenchReports.do?method=audit" paramId="id"
                    paramProperty="id">
        ...
        <c:if test="${flag!=null&&flag=='week'}">
            <display:column
                    titleKey="shenhe"
                    url="/workbench/report/tawWorkbenchReports.do?method=audit&flag=<c:out value='${flag }'/>"
