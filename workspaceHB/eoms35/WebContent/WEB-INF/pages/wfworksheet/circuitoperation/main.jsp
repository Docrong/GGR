...
<script type="text/javascript">
	...
	function initPage(){
		 ...
		 $('btns').show();   
	//	 v.preCommitUrl = "${app}/sheet/circuitoperation/circuitoperation.do?method=performPreCommit";		
	}
   ...
   Ext.onReady(function(){
	...
	tabs.addTab('sheetform', "<bean:message bundle="circuitoperation" key="circuitoperation.header"/>");
	...
	if ('${templateId}'!= null && '${templateId}' != "") {
		mgr.update("${app}/sheet/circuitoperation/circuitoperation.do?method=showTemplateInputSheetPage&templateId=${templateId}");
	} else {
		mgr.update("${app}/sheet/circuitoperation/circuitoperation.do?method=showNewInputSheetPage&processname=CircuitOperationProcess&parentCorrelation=${parentCorrelation}&parentSheetId=${parentSheetId}&parentSheetName=${parentSheetName}&parentPhaseName=${parentPhaseName}");
	}
   ...
   function changeType1(){
   	...
   	$('${sheetPageName}phaseId').value = 'MakeZTask';
	   	...
	   	if (templateManage == "templateManage") {
		   	...
		   	if (type == "new"){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "circuitoperation.do?method=saveTemplate&templateId=${templateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
  ...
  function removeTemplate() {
   if (confirm('<bean:message bundle="sheet" key="worksheet.delete.confirm" />')) {
		...
		thisform.action = "${app}/sheet/circuitoperation/circuitoperation.do?method=removeTemplate&templateId=${templateId}&type=${type}";
		thisform.submit();
	}
 }
...
<html:form action="/circuitoperation.do?method=newPerformAdd" styleId="theform">
	...
	<logic:notPresent name="templateManage">
		...
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./circuitoperation.do?method=getTemplatesByUserId',null,'left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes')">
