...
<div id="sheetform">
<html:form action="/complaint.do?method=performProcessEvent" styleId="theform">
 <%@ include file="/common/taglibs.jsp"%>
...
<%@ include file="/WEB-INF/pages/wfworksheet/complaint/baseinputlinkhtmlnew.jsp"%>
     ...
     <tr>
      ...
      <logic:iterate id="toTask" name="toOperaterList" type="com.boco.eoms.sheet.complaint.task.impl.ComplaintTask">  
        <tr>
          <td>
     ...
     <input type="hidden" name="${sheetPageName}operateType" value="-11"/> 
        ...
        <tr>
         <td class="label"><bean:message bundle="complaint" key="complaint.isDeferReploy" />*</td>
         <td>
		  	<eoms:comboBox name="${sheetPageName}isDeferReploy" id="${sheetPageName}isDeferReploy" 
			      initDicId="10301" defaultValue="${sheetLink.isDeferReploy}"  styleClass="select-class"/>
	     </td>	
        </tr>
    	<tr>
	       <td class="label">
    	...
    	<tr>
	       <td class="label">
			...
			<td colspan="3">
		           <textarea name="${sheetPageName}linkTransmitContent" class="textarea max" id="${sheetPageName}linkTransmitContent" 
		              alt="allowBlank:false,width:500,maxLength:1000,vtext:'最多输入500汉字'" alt="width:'500px'">${sheetlink.linkTransmitContent}</textarea>
		  </td>
		</tr>
		</table>
		<%} %>
 ...
 <%if(taskName.equals("SendDeferExamine")) {%>	
	<fieldset id="link4">
	 <legend>
			...
			<bean:message bundle="complaint" key="role.Examine"/>			 
	 </legend>	
		<eoms:chooser id="test" type="role" roleId="216" flowId="052" config="{returnJSON:true,showLeader:true}"
