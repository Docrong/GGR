...
<script language="JavaScript">
    ...
    function checkForm() {
        ...
        callerWindowObj.document.forms[0].modelnamelist.value = sValuestr;
        callerWindowObj.document.forms[0].modellist.value = sValue;
        window.close();
...
<FORM name='frmAddress' method='post' onsubmit='return checkForm();'>
    <TABLE cellSpacing=0 cellPadding=0 align=center border=0>
        <TR>
            <TD vAlign=top align=center width='200'><P>可选模板</P></TD>
            <TD align=center width='80'></TD>
            <TD vAlign=top align=center width='200'><P>&nbsp;已选模板</P></TD>
        </TR>
        <tr>
            <TD vAlign=top align=center width='200'>
                <SELECT size='15' width='100%' name='select5' multiple style='width:65pt'>
                    <%
                        List modellist = (List) request.getAttribute("tawwpmodellist"); //获取年度作业计划集合结构
                        for (int i = 0; i < modellist.size(); i++) {
                            TawwpModelPlanVO tawwpModelPlanVO = (TawwpModelPlanVO) modellist.get(i);
                    %>
                    <option value="<%=tawwpModelPlanVO.getId()%>"><%=tawwpModelPlanVO.getName()%>
                    </option>
                    <%
                        }
                    %>
