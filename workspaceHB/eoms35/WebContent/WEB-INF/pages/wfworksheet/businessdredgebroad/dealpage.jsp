...
<%
 ...
 String bType="";
 if(basemain!=null){
 bType=com.boco.eoms.base.util.StaticMethod.nullObject2String(basemain.getBusinesstype1());
 }
 System.out.println("dealpage bType=="+bType);
 //end by yangna
%>
...
<c:url var="urlDeal" value="/sheet/businessdredgebroad/businessdredgebroad.do">
...
function initPage(){
	...
	if("${operateType}"=='111'){
    	 v.preCommitUrl = "${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=performPreCommit";	
	}
 } 
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {	
   		strUrl = '${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
    ...
    var form = document.forms[0];
   	var ajaxForm = Ext.getDom(form);
		    ...
		    Ext.Ajax.request({
				   		form: ajaxForm,
						method:"post",
						url: "${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=performSaveInfo",
						success: function(){
				        	alert("${eoms:a2u('保存成功！')}");
				 		}
				    });
	   	...
	   	if (confirm("${eoms:a2u('确认保存模板吗？')}")) {
		    if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "businessdredgebroad.do?method=saveDealTemplate",
					success: function(){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "businessdredgebroad.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
	   	...
	   	if (templateManage == "templateManage") {
		   	...
		   	if (type == "new"){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "businessdredgebroad.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
...
function tempInterCom() {
  ...
  if(ope =='205'){
    ...
    if(btype1 == '101100102' || btype1 == '101100103'){
      var tempBusinessCode = $('${sheetPageName}tempBusinessCode').value;
      //alert("tempBusinessCode=="+tempBusinessCode);
      var tempMasId = $('${sheetPageName}tempMasId').value;
      //alert("tempMasId=="+tempMasId);
      var tempPassword = $('${sheetPageName}tempPassword').value;
      //alert("tempPassword=="+tempPassword);
      var dealDesc = $('${sheetPageName}dealDesc').value;
      //alert("dealDesc=="+dealDesc);
      dealDesc ='${eoms:a2u('描述：')}'+dealDesc+ '; '+'${eoms:a2u('业务代码:')}'+tempBusinessCode +'${eoms:a2u('; MAS ID:')}'+ tempMasId +'${eoms:a2u('; 帐户密码:')}'+ tempPassword ;
      //alert("dealDesc2=="+dealDesc);
      $('${sheetPageName}dealDesc').value = dealDesc;
...
<div id="sheetform">
  <html:form action="/businessdredgebroad.do?method=performDeal" styleId="theform">
   <input type="hidden" name="operateType" value="${operateType}"/>
	<table>
	  ...
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'ByRejectHumTask'&& taskName != 'DraftHumTask'&& taskName != 'TaskCreateAuditHumTask'}">
		...
		<input type="hidden" name="dictKey" value="dict-sheet-businessdredgebroad"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftHumTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType1();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
    ...
    <c:when test="${taskName=='ByRejectHumTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType2();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	    </div>  	 
   	</c:when>   	
    <c:otherwise>  
      <c:if test="${operateType=='111'}">
	    ...
	    <div class="form-btns hide" id="btns">
         <input type="submit" class="submit" name="method.save" id="method.save"  onclick="javascript:ifisCopy();tempInterCom();" value="<bean:message bundle='sheet' key='button.done'/>" >	  
