...
<c:choose>
    ...
    <c:when test="${operateType == 61}">
        <input type="submit" class="submit"
               onclick="this.form.action='${app}/sheet/${module}/${module}.do?method=performClaimTask'"
    ...
    <c:otherwise>
        <!-- 提交 -->
        <input type="submit" class="submit" name="method.save" id="method.save" onclick="javascript:isCopy()"
               value="<bean:message bundle='sheet' key='button.done'/>">
        <!-- 保存 -->
        <input type="submit" class="submit"
               onclick="v.passing=true;this.form.action='${app}/sheet/${module}/${module}.do?method=saveDealInfo'"
               value="<bean:message bundle='sheet' key='button.save'/>"/>
        <!-- 保存并返回 -->
        <input type="submit" class="submit"
               onclick="v.passing=true;this.form.action='${app}/sheet/${module}/${module}.do?method=performSaveInfoAndExit'"
               value="<bean:message bundle='sheet' key='button.saveback'/>"/>
        <!-- 返回 -->
        <input type="submit" class="submit"
               onclick="v.passing=true;this.form.action='${app}/sheet/${module}/${module}.do?method=showListsendundo'"
