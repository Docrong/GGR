...
<div id="tabs">
    ...
    <div id="form" class="tab-content">
        <table cellpadding="0" class="table logList" cellspacing="0">
            ...
            %>
            <tr class="sortable">
                <td width='10%' align='center'><%=i%>
                </td>
                <td width='30%' align='center'><%=obj.get(0).toString()%>
                </td>
                <td width="20%" align='center'><a
                        href="${app}/log/TawCommonLogCondition/logstatdeptdo.do?flag=<%=i%>&searchbystarttime=<%=starttime%>&searchbyendtime=<%=endtime%>"
                        target="_blank"><%=obj.get(1).toString()%>
                </a></td>
                <td width="20%" align='center'><a
                        href="${app}/log/TawCommonLogCondition/logstatuserdo.do?flag=<%=i%>&searchbystarttime=<%=starttime%>&searchbyendtime=<%=endtime%>"
                        target="_blank"><%=obj.get(2).toString()%>
                </a></td>
                <td width="20%" align='center'><a
                        href="${app}/log/TawCommonLogCondition/logstatalldo.do?flag=<%=i%>&searchbystarttime=<%=starttime%>&searchbyendtime=<%=endtime%>"
                        target="_blank"><%=obj.get(3).toString()%>
                </a></td>
            </tr>
            <% }%>
