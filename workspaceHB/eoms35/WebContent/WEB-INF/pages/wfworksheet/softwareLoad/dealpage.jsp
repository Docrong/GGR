...
<c:url var="urlDeal" value="/sheet/softwareLoad/softwareLoad.do">
...
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {	
   		strUrl = '${app}/sheet/softwareLoad/softwareLoad.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
    ...
    var form = document.forms[0];
   	var ajaxForm = Ext.getDom(form);
		    ...
		    Ext.Ajax.request({
				   		form: ajaxForm,
						method:"post",
						url: "${app}/sheet/softwareLoad/softwareLoad.do?method=performSaveInfo",
						success: function(){
				        	alert("保存成功！");
				 		}
				    });
	   	...
	   	if (confirm("确认保存模板吗？")) {
		    if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "softwareLoad.do?method=saveDealTemplate",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "softwareLoad.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
   	...
   	var templateManage = "${type}";
   	 ...
   	 v.passing = false;
	   	...
	   	if (templateManage == "templateManage") {
	   		form.action = "${app}/sheet/softwareLoad/softwareLoad.do?method=saveTemplate&templateId=${templateId}&type=${type}";
	   		form.submit();
	   	} else {
		   	if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "softwareLoad.do?method=saveTemplate&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "softwareLoad.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
...
<div id="sheetform">
  <html:form action="/softwareLoad.do?method=newPerformDeal" styleId="theform">
   <input type="hidden" name="operateType" value="${operateType}"/>
	<table>
	  ...
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'RejectTask'&& taskName != 'DraftTask'&& taskName != 'AuditTask'&& taskName != 'cc'}">
		...
		<input type="hidden" name="dictKey" value="dict-sheet-softwareLoad"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/softwareLoad/softwareLoad.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType1();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/softwareLoad/softwareLoad.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/softwareLoad/softwareLoad.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
    ...
    <c:when test="${taskName=='RejectTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType2();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/softwareLoad/softwareLoad.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/softwareLoad/softwareLoad.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	   </div>	 
   	</c:when>
    <c:otherwise>  
	    <!-- buttons -->
