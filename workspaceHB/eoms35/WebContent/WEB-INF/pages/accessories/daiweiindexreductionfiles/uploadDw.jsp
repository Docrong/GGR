...
function confirmForm(){ 
    ...
    if(filetype!=null || !filetype.eques("")){
      ...
      // 核减状态
      var ReserveE = parent.document.getElementById("ReserveE");
     if(parent.document.getElementById("ReserveE") !=null){ 
     		parent.document.getElementById("ReserveE").value=ReserveE; 
     }
      // 判断核减专业是否为空  
    ...
    // 上傳文件的路徑 daiweiindexreduction/subtracttablesupload 
    var noPassObj = parent.document.getElementById("noPass");
    var noPass='';
    if(noPassObj!=null){
    	noPass = noPassObj.value;
    }
    var mainReserveAObj = parent.document.getElementById("mainReserveA");  // main表里的关联字段
    //alert(mainReserveA);
    var mainReserveA='';
    if(mainReserveAObj!=null){
    	mainReserveA = mainReserveAObj.value;
    }
    var url = "${app}/sheet/daiweiindexreduction/daisubtracttablesupload.do?method=upload&profName="+profName+"&appId="+appId+"&sheetFlag="+sheetFlag+"&noPass="+noPass+"&refSheetId="+mainReserveA;
    // alert(url);
             ...
             if(data.result!=null && data.result!=""){
			 	...
			 	//add by wyf begin 
					...
					var refSheetId = data.refSheetId;
	         		...
	         		//alert("reserveA");
	         		var url="${app}/sheet/daiweiindexreduction/daiweiindexreduction.do?method=search&ifShowOther=no&refSheetId=" + refSheetId;
	         		var obj = window.parent.document.getElementById('iframepage');
	         		if(window.parent.document.getElementById('mainReserveA') != null){
  ...
  <table>
   <tr>
     <td colspan="2">
       ...
       <input type="file" id="files" name="files" size="30" class="upload" class="file">
       <input type="text" id ="ss" name="ss" value="代维公司指标导出" readonly="readonly" style="color:#FF0000;border:none;">
