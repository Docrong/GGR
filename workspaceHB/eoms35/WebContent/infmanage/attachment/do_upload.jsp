...
// 逐一提取上传文件信息，同时可保存文件。
        ...
        for (int i = 0; i < su.getFiles().getCount(); i++) {
            ...
            String tempfilename = gzUtil.getCurrentDateTime("yyyyMMddHHmmss") + "." + file.getFileExt();
            int attId = tawAttachmentDAO.insert(StaticMethod.dbNStrRev(file.getFileName()), StaticMethod.dbNStrRev(tempfilename), file.getSize(), "admin", "", Integer.parseInt(aAppInformation.getAppId()));
...
<FORM METHOD="POST"
      ACTION="do_upload.jsp?app=<%=aAppInformation.getAppCode()%>&filelist=<%=fileIdList%>&idfile=<%=request.getParameter("idfile")%>"
