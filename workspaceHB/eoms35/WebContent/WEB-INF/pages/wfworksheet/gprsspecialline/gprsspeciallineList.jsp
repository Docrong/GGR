...
<tr class="buttonBar bottom">
     <input type="button"  styleClass="button"
            onclick="location.href='<c:url value="/businessupport/product/gprsspeciallines.do?method=xedit&type=add&ifReference=${ifReference}"/>'" 
            value="添加"/>
  <logic:present name="back" scope="request">
     <input type="button"  styleClass="button"
            onclick="location.href='<c:url value="/businessupport/product/gprsspeciallines.do?method=xquery&ifReference=${ifReference}"/>'" 
            value="返回"/>
  </logic:present>
</tr>
</html:form>
