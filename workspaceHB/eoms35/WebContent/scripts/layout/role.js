...
Ext.onReady(function () {
    ...
    var modifybtn = form.addButton({text: '保存修改', hidden: true}, function () {
        ...
        Ext.lib.Ajax.request(
            ...
            {
                success: function () {
                    reloadTree(selectNode.parentNode);
                    unmask();
    ...
    var filterInput = Ext.DomHelper.insertBefore('subRoleList', {
        tag: 'input', type: 'text', cls: 'text', value: defaultText, style: 'margin:0 5px 5px 0'
    }, true);
    ...
    filterInput.on('focus', function (e, dom) {
        if (dom.value == defaultText) dom.value = "";
    });
    filterInput.on('blur', function (e, dom) {
        if (dom.value == "") dom.value = defaultText;
    });
    ...
    Ext.MessageBox.confirm("确认:", "您确定删除项目吗？", function (btn) {
        if (btn == "yes") {
            ...
            Ext.Ajax.request({
                ...
                success: function () {
                    gridds.reload();
                    Ext.MessageBox.alert('提示', "删除子角色成功。");
                },
                failure: function () {
                    alert("fail")
                }
    ...
    Ext.MessageBox.prompt("修改子角色名称:", "请输入子角色的新名称", function (btn, text) {
        if (btn == "ok") {
            ...
            Ext.Ajax.request({
                ...
                failure: function () {
                    Ext.MessageBox.alert('提示', "修改名称失败。");
                },
