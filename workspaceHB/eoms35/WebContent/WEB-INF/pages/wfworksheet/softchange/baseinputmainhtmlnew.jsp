 ...
 <tr> 
  ...
  <td colspan="3">
	<div id="areaview" class="hide"></div>
      ...
      <textarea class="textarea max" readonly="true" name="${sheetPageName}showDept" style="height:50px" id="${sheetPageName}showDept"  alt="allowBlank:false,maxLength:50,vtext:'请填入地市，最多输入50字符'"><c:forTokens items="${sheetMain.toDeptId}" delims="," var="toDeptId" varStatus="status"><eoms:id2nameDB id="${toDeptId}" beanId="tawSystemAreaDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
...
<c:if test="${status==1}">
  ...
  <tr>
    ...
    <td colspan="3">
      <eoms:comboBox name="${sheetPageName}holdStatisfied" defaultValue="${sheetMain.holdStatisfied}" id="${sheetPageName}holdStatisfied" initDicId="10303" styleClass="select"/>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td colspan="3">
      <textarea name="${sheetPageName}endResult" id="${sheetPageName}endResult" alt="width:'500px',allowBlank:false">${sheetMain.endResult} </textarea>
    </td>
  </tr>
