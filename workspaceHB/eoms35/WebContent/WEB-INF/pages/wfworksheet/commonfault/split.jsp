...
<%String subtaskName = request.getParameter("subtaskName");
 String teamFlag = com.boco.eoms.base.util.StaticMethod.nullObject2String(request.getParameter("teamFlag"));
 %>

<div id="sheetform">
    <html:form action="/commonfault.do?method=performCreateSubTask" styleId="theform">
       <%@ include file="/WEB-INF/pages/wfworksheet/commonfault/baseinputlinkhtmlnew.jsp"%>		
        ...
        <input type="hidden" name="${sheetPageName}taskNamespace" value="http://CommonFaultProcess"/>   
        <input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="${operateType}" />
         <input type="hidden" name="${sheetPageName}teamFlag" value="<%=teamFlag%>"/> 
      <br/>         
        ...
        <tr>
          ...
          <td colspan="3">            
			 <textarea name="${sheetPageName}linkFaultDealInfo" id="${sheetPageName}linkFaultDealInfo" 
			    class="textarea max" alt="allowBlank:false">${sheetLink.linkFaultDealInfo}</textarea>	  
		  </td>
		 </tr>
		 <tr>	
		  <td class="label">            
          ...
          <td colspan="3">
		    <textarea name="${sheetPageName}remark" id="${sheetPageName}remark" 
			    class="textarea max" alt="allowBlank:false,width:500,maxLength:1000,vtext:'最多输入50汉字'"  >${sheetLink.remark}</textarea>
		  </td>	
	   </tr>
	    <tr>	
		  <td class="label">            
