...
>
    <display:column headerClass="sortable" title="详细信息">
        <a onclick="window.open('${app}/businessupport/product/ipspeciallines.do?method=showDetail&orderId=<%=orderId%>&isView=1&taskStatus=<%=taskStatus%>&sheetType=<%=sheetType%>&taskName=<%=taskNamePara %>&id=<%=((HashMap)pageContext.getAttribute("list")).get("id")%>');"
