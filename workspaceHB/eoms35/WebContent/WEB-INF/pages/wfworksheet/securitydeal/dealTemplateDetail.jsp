...
<%@ include file="/WEB-INF/pages/wfworksheet/securitydeal/baseinputlinkhtmlnew.jsp"%>
 <html:form action="/securitydeal.do" styleId="theform">       
	<br/>
               ...
               <input type="hidden" name="${sheetPageName}mainClassName" value="com.boco.eoms.sheet.securitydeal.model.securitydealMain"/>	<!--main\u8868Model\u5bf9\u8c61\u7c7b\u540d-->	
               <input type="hidden" name="${sheetPageName}linkClassName" value="com.boco.eoms.sheet.securitydeal.model.securitydealLink"/> <!--link\u8868Model\u5bf9\u8c61\u7c7b\u540d-->
     <table class="listTable">
			      <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkSecurityInproveAdvice"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkSecurityInproveAdvice" id="${sheetPageName}linkSecurityInproveAdvice" >${sheetLink.linkSecurityInproveAdvice}</textarea>
                    </td>
		          </tr>
	                <tr>
  						<td class="label"><bean:message bundle="securitydeal" key="securitydeal.linkSecurityInproveProgram"/></td>
		   				 <td  colspan='3'>
		   				 <eoms:attachment idList="" idField="${sheetPageName}linkSecurityInproveProgram" appCode="securitydealsheet"/>
		                </td>
		            </tr>
			      <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkAuditViews"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkAuditViews" id="${sheetPageName}linkAuditViews" >${sheetLink.linkAuditViews}</textarea>
                    </td>
		          </tr>
			      <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkAuditResult"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkAuditResult" id="${sheetPageName}linkAuditResult" >${sheetLink.linkAuditResult}</textarea>
                    </td>
		          </tr>
			      <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkPerformResult"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkPerformResult" id="${sheetPageName}linkPerformResult" >${sheetLink.linkPerformResult}</textarea>
                    </td>
		          </tr>
			      <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkIfStartChangeProcess"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkIfStartChangeProcess" id="${sheetPageName}linkIfStartChangeProcess" >${sheetLink.linkIfStartChangeProcess}</textarea>
                    </td>
		          </tr>
	 </table>
</table>
