...
<form name="modelexecutelist">
    ...
    <table class="listTable">
        <caption><bean:message
                key="itemlist.tawwpyear.planFmTitle"/>&lt;&nbsp;<%=tawwpYearPlanVO.getName()%>&nbsp;&gt;
        </caption>
        <tr>
    ...
    <table class="listTable" id="executeTable">
        <caption>&lt;&nbsp;<%=tawwpYearPlanVO.getName()%>&nbsp;&gt;<bean:message
                key="itemlist.tawwpyear.executeFmTitle"/></caption>
        <thead>
        <tr>
        ...
        <tr>
            ...
            <td width="150">
                ...
                %>
                <a target="_blank"
                   href="../tawwpaddons/addonsread.do?action=read&window=new&myid=&model=50&addonsid=<%=tawwpYearExecuteVO.getFormId()%>&reaction=/tawwpaddons/addonslist.do"><%=tawwpYearExecuteVO.getFormName()%>
                </a>
                <%
    ...
    <table>
        <tr>
            <td>
                ...
                %>
                <input type="button" value="<bean:message key="itemlist.tawwpyear.btnYearReport" />"
                       onclick="javascript:onYearReport();" class="button">
                <%
                    }
                    if (tawwpYearPlanVO.getState().equals("0") || tawwpYearPlanVO.getState().equals("2")) {
                %>

                <input type="button" value="<bean:message key="itemlist.tawwpyear.btnAdd" />"
                       onclick="javascript:onYearPlanAdd();" class="button">

                <%
                    }
                    if (tawwpYearPlanVO.getStep() == null && tawwpYearPlanVO.getState().equals("0")) {
                %>
                <input type="button" value="<bean:message key="itemlist.tawwpyear.btnDeploy" />"
                       onclick="javascript:onDeploy();" class="button">
                <%
                    }
                %>
                <input type="button" value="<bean:message key="itemlist.tawwpyear.btnBack" />"
                       onclick="javascript:onBack();" class="button">
            </td>
        </tr>
    </table>
</form>
...
<form name="yearrefer" method="post" action="yeardispatch.do?yearplanid=<%=request.getParameter("yearplanid")%>">
    ...
    <input type="button" value="<bean:message key="itemlist.tawwpyear.btnCheck" />" onclick="javascript:onRefer();"
           class="button">
