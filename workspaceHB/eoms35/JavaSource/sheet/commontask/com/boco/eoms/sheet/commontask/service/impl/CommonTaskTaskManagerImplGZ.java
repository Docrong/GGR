...
public class CommonTaskTaskManagerImplGZ extends CommonTaskTaskManagerImpl
{
		...
		throws Exception
	{
		...
		if ("1".equals(type))
		{
			sql = "select distinct task" + ruternColumnName + " from " + " CommonTaskTaskUndo as task," + mainObject.getClass().getName() + " as main " + "where main.id =task.sheetKey and main.deleted <> 1 and main.id = task.sheetKey and task.taskStatus in ('2', '8') and task.taskDisplayName <> '²Ý¸å' and task.taskDisplayName <> '½×¶Î»Ø¸´' and task.operateRoleId in ('" + userId +"', '" + deptId + "')" + " ";
		...
		} else
		{
			...
			" main.deleted <> 1 and task.taskowner in ('" + userId + "', '" + deptId + "') and taskstatus in ('2','8') and task.sheetkey = link.mainid(+) and task.taskDisplayName <> '²Ý¸å' and task.taskDisplayName <> '½×¶Î»Ø¸´' and mainid is null and task.sheetkey =main.id" + agentsql;
