...
<div id="sheet-list">
    <div class="list-title"><%=reportDefine.getCaption() %>
    </div>
    <div class="list">
        <table cellspacing="0" cellpadding="0" border="0" id="list-table">
            ...
            <tr id="tr<%=iRowHead %>" class="header">
                ...
                %>
                <td nowrap align="center" rowspan="<%=reportDefine.getViewRowSpanMax()%>"
                    width="<%=reportSummary.getViewWidth()%>"><%=reportSummary.getViewName()%>
                </td>
                ...
                %>
                <!-- data field header -->
                <td nowrap align="center" colspan="<%=reportField.getViewColumnSpan() %>"
                    rowspan="<%=reportField.getViewRowSpan() %>"
                    width="<%=reportField.getViewWidth() %>"><%=reportField.getViewName()%>
                </td>
            ...
            <tr>
                <% if (iRowHead == 0) { %>
                <td rowspan="<%=reportDefine.getViewRowSpanMax() %>"><%=iRow %>
                </td><!-- 序号 -->
                ...
                <td rowspan=<%= (rowspanSum * reportDefine.getViewRowSpanMax())%>>
                    <eoms:id2nameDB id="<%= iSummaryData%>" beanId="<%=reportSummary.getIdToName() %>"/>
                </td>
                <%
                    }
                    //合并行 end
                ...
                <td colspan="<%=reportField.getViewColumnSpan() %>" rowspan="<%=reportField.getViewRowSpan() %>">
                    ...
                    %>
                    <a href="<%=listUrl%>&kpiDefineName=<%=reportDefine.getKpiDefineName()%>&queryDefineName=<%=""%>&fieldAsName=<%=reportField.getExpression()%>&summaryData=<%=summaryData %>"
                       target="sheetlist"><%=fieldData%>
                    </a>
                    <%} else {%>0<%}%>
                </td>
                <% }
