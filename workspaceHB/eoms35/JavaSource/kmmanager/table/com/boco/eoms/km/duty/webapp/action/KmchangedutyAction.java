...
import com.boco.eoms.km.duty.bo.KmchangedutyBO;
import com.boco.eoms.km.duty.dao.KmchangedutyDAO;
import com.boco.eoms.km.duty.webapp.form.KmchangedutyForm;
...
public class KmchangedutyAction extends Action {
	...
	private ActionForward performView(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) {
		KmchangedutyForm form = (KmchangedutyForm) actionForm;
		try {
	...
	private ActionForward performSave(ActionMapping mapping,
		...
		try {
			...
			if (strutsAction == KmchangedutyForm.ADD) {
			...
			} else if (strutsAction == KmchangedutyForm.EDIT) {
	...
	private ActionForward performEdit(ActionMapping mapping,
			...
			HttpServletResponse response) {
		KmchangedutyForm form = (KmchangedutyForm) actionForm;
		form.setStrutsAction(KmchangedutyForm.EDIT);
		try {
	...
	private ActionForward performAdd(ActionMapping mapping,
			...
			HttpServletResponse response) {
		KmchangedutyForm form = (KmchangedutyForm) actionForm;
		form.setStrutsAction(KmchangedutyForm.ADD);
		return mapping.findForward("success");
	}
