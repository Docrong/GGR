...
public class StaticMethodbak {
    ...
    public final static URL URI = StaticMethodbak.class.getResource("/");


    public StaticMethodbak() {
    }
     ...
     * @param filePath 文件路径
     * @return
     * @see StaticMethodbak#getClasspath()
     ...
     *
     * @return classpath路径
     * @see StaticMethodbak#getFilePath(String)
     * @see StaticMethodbak#getFileUrl(String)
     */
            ...
            if (filePath.length() > CLASSPATH_FLAG.length()) {
                if (CLASSPATH_FLAG.equals(filePath.substring(0, CLASSPATH_FLAG
                    ...
                    try {
                        ...
                        url = StaticMethodbak.class.getClassLoader().getResource(
                                getPathButClasspath(filePath));
                        URL url1 = StaticMethodbak.class
    ...
    public static String getWebPath() throws FileNotFoundException {
        ...
        String result = StaticMethodbak.class.getResource("StaticMethod.class")
                .toString();
    ...
    public static boolean fHasId(int id, ArrayList array) throws Exception {
        ...
        try {
            while (i < array.size() && ret == false) {
                if (id == StaticMethodbak.nullObject2int(array.get(i))) {
                    ret = true;
                }
                i++;
            }
    ...
    public static ArrayList getArrayList(ArrayList array1, ArrayList array2) {
        ...
        try {
            for (int i = 0; i < array1.size(); i++) {
                int temp = StaticMethodbak.nullObject2int(array1.get(i));
                if (StaticMethodbak.fHasId(temp, array2) == true) {
    ...
    public static ArrayList getArrayList2(ArrayList array1, ArrayList array2) {
        ...
        if ((size2 == 0) || (size1 == 0)) {
            ...
            try {
                for (int i = 0; i < array1.size(); i++) {
                    int temp = StaticMethodbak.nullObject2int(array1.get(i));
                    if (StaticMethodbak.fHasId(temp, array2) == false) {
                        array.add(new Integer(temp));
                    }
                }
            } catch (Exception e) {
    ...
    public static Integer nullObject2Integer(Object s) {
        return new Integer(StaticMethodbak.nullObject2int(s));
    }
    ...
    public static ArrayList getSubList(ArrayList list1, ArrayList list2) {
        ...
        if ((j > 0) && (k > 0)) {
            ...
            for (int i = 0; i < j; i++) {
                String temp = StaticMethodbak.nullObject2String(list1.get(i));
                for (int l = 0; l < k; l++) {
                    String temp2 = StaticMethodbak.nullObject2String(list2.get(l));
    ...
    public static GregorianCalendar String2Cal(String strDate) {
        ...
        strDate = StaticMethodbak.null2String(strDate).replaceAll("\\.0", "");
        Vector vecDate = StaticMethodbak.getVector(strDate, " ");
        if (vecDate.size() > 0 && vecDate.size() < 3) {
            if (vecDate.size() == 1) {
                Vector vecData = StaticMethodbak.getVector(String.valueOf(vecDate
                        .elementAt(0)), "-");
                if (vecData.size() == 3) {
            ...
            if (vecDate.size() == 2) {
                Vector vecData = StaticMethodbak.getVector(String.valueOf(vecDate
                        .elementAt(0)), "-");
                Vector vecTime = StaticMethodbak.getVector(String.valueOf(vecDate
                        .elementAt(1)), ":");
                if (vecData.size() == 3) {
                    int intYear = Integer.parseInt(String.valueOf(vecData
                            .elementAt(0)));
    ...
    public static String getLocalString(String str) {
        ...
        Vector temp = StaticMethodbak.getVector(str, " ");

        if (!temp.isEmpty()) {
            Vector first = StaticMethodbak.getVector(temp.get(0).toString(), "-");
            Vector last = StaticMethodbak.getVector(temp.get(1).toString(), ":");
    ...
    public static int getTimeIntervals(String historyTime, String nowTime) {
        ...
        int year1 = StaticMethodbak.getIntValue(timeHistory[0]);
        int year2 = StaticMethodbak.getIntValue(timeNow[0]);
        int month1 = StaticMethodbak.getIntValue(timeHistory[1]);
        int month2 = StaticMethodbak.getIntValue(timeNow[1]);
        int day1 = StaticMethodbak.getIntValue(timeHistory[2]);
        int day2 = StaticMethodbak.getIntValue(timeNow[2]);
        int hour1 = StaticMethodbak.getIntValue(timeHistory[3]);
        int hour2 = StaticMethodbak.getIntValue(timeNow[3]);
        int min1 = StaticMethodbak.getIntValue(timeHistory[4]);
        int min2 = StaticMethodbak.getIntValue(timeNow[4]);
        int sec1 = StaticMethodbak.getIntValue(timeHistory[5]);
        int sec2 = StaticMethodbak.getIntValue(timeNow[5]);
        int flag, minutes;
    ...
    public static Object invokeStringMethod(Object obj, String attributeName,
        ...
        try {
            String setMethod = "set"
                    + StaticMethodbak.firstToUpperCase(attributeName);
