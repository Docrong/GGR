 ...
 * @version 3.6
 */
public class TawpartMscid extends BaseObject {

    /**
     *
     */
    private static final long serialVersionUID = 4359360487092514095L;
    ...
    public boolean equals(Object o) {
        if (o instanceof TawpartMscid) {
            TawpartMscid tawpartMscid = (TawpartMscid) o;
            if (this.id != null || this.id.equals(tawpartMscid.getId())) {
                return true;
            } else {
                return false;
            }
    ...
    public TawpartMscid cloneTawpartMscid() {
        TawpartMscid tpm = new TawpartMscid();
        tpm.setAreaId(areaId);
        tpm.setAreaName(areaName);
        tpm.setAssigner(assigner);
        tpm.setAssignTime(assignTime);
        tpm.setCreateTime(createTime);
        tpm.setDistributed(distributed);
        tpm.setCreator(creator);
        tpm.setEquipmentName(equipmentName);
        tpm.setEquipmentType(equipmentType);
        tpm.setHeadNumber(headNumber);
        tpm.setNumberFree(numberFree);
        tpm.setNumberM0(numberM0);
        tpm.setNumberM1(numberM1);
        tpm.setNumberM2(numberM2);
        tpm.setCityname(cityname);
        tpm.setSignalname(signalname);
        tpm.setEquipmentfactory(equipmentfactory);
        return tpm;
    }
}