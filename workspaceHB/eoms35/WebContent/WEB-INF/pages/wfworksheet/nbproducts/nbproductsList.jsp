...
<tr>
<td nowrap><content tag="heading">${eoms:a2u('新业务产品目录库')}</content></td>
...
<td width="130" align="left">
  <input type="text" id="txtwords" name="txtwords" class="headinpout2" maxlength="20" onkeydown="onkeydownEvt(event);"/>
  <input type="hidden" name="${sheetPageName}txtwords" id="${sheetPageName}txtwords" value="${sheetPageName}txtwords" />
  <input type="hidden" name="${sheetPageName}ifReference" id="${sheetPageName}ifReference" value="<%=ifReference%>" />
  </td>
<td>
<input id="btnSearch" type="button" align="right" name= "button" onclick="query();" value="<fmt:message key="button.query"/>">
</input>
</td>
</tr>
</table>
...
<tr class="buttonBar bottom">
  <logic:present name="ifReference" scope="request">
     ...
     <input type="button"  styleClass="button"
            onclick="location.href='<c:url value="/sheet/nBProducts/nbproductss.do?method=xedit&type=add&ifReference=${ifReference}"/>'" 
            value="${eoms:a2u('添加')}"/>
