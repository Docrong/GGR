        ...
        <logic:iterate id="tawInformation" name="TAWINFORMATIONS" type="com.boco.eoms.kbs.model.TawInformation">
            ...
            <tr>
                <td width="5%" nowrap bgcolor="#e5edf8" class="clsthd2">
                    <input type="checkbox" name="chk_sel_id" id="<%=listNum%>"
                           value="<bean:write name="tawInformation" property="id" scope="page"/>">
                    <input type="hidden" name="chk_sel_pid" id="<%=listNum%>"
                           value="<bean:write name="tawInformation" property="parentId" scope="page"/>">
                    <!--<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>-->
                ...
                <td width="45%" nowrap bgcolor="#e5edf8">
                    ...
                    %>
                    <a href="viewinfo.do?id=<%=tawInformation.getId()%>&boardId=<%=tawInformation.getBoardId()%>&infoType=<%=tawInformation.getInfoType()%>"
                       title="<%=tawInformation.getTopic()%>">
    ...
    <table id="tp1" bgcolor="#709fd5" width="100%" cellspacing="1" border="0" cellpadding="1" class="clsbkgd">
        <tr>
            <td align="right" bgcolor="#d7dae1">
                ...
                <bean:message key="taw.total"/> <%=listCount%> <bean:message key="taw.piece"/> <%=pageNum%>
                <bean:message key="taw.pageDescription"/>
