...
<logic:present name="HISTORY" scope="request">
      <logic:iterate id="baselink" name="HISTORY" type="com.boco.eoms.sheet.agentmaintenance.model.AgentMaintenanceLink">  
       <%         
        jNum += 1;       
     ...
     <div class="history-item"><!-- add space to hack ie-->&nbsp;
       <div class="history-item-title">
		 ...
		 <bean:write name="baselink" property="operateTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="page"/>&nbsp; 
  	     ...
  	     <eoms:dict key="dict-sheet-agentmaintenance" dictId="activeTemplateId" itemId="${baselink.activeTemplateId}" beanId="id2descriptionXML" />
          <bean:message bundle="agentmaintenance" key="agentmaintenance.operateType"/>: 
  	     <eoms:dict key="dict-sheet-agentmaintenance" dictId="mainOperateType" itemId="${baselink.operateType}" beanId="id2descriptionXML" />
  ...
  <!-- 			<%if(baselink.getOperateRoleId()!=null && ! baselink.getOperateRoleId().equals("")) {%>
            ...
            <%if(baselink.getActiveTemplateId()!=null && (baselink.getActiveTemplateId().equals("TaskCreateAuditHumTask")
             ...
             <%if(baselink.getOperateType()!=null && ( baselink.getOperateType().intValue() == 201 || baselink.getOperateType().intValue() == 202
		  		 	...
		  		 	<tr>  				
		  				  <td class="column-title">
		  				     <bean:message bundle="agentmaintenance" key="agentmaintenance.linkAuditResult"/>
		  				  </td>
		  				  <td class="column-content" colspan="3">
                               <eoms:id2nameDB id="${baselink.linkTaskAuditResult}" beanId="ItawSystemDictTypeDao"/>
		                  </td>                  
		  			</tr>
		  		 	<tr>  				
		  				  <td class="column-title">
		  				     <bean:message bundle="agentmaintenance" key="agentmaintenance.linkAuditIdea"/>
		  				  </td>
		  				  <td class="column-content" colspan="3">
                              <pre> <bean:write name="baselink" property="linkTaskAuditIdea" scope="page"/></pre>
		                  </td>                  
		  			</tr>			  					  			
  			<%}}%>	
         <%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("ExcuteHumTask")){%>  		
            ...
            <%if(baselink.getOperateType()!=null && baselink.getOperateType().intValue() == 10){%> 
				  <tr>  				
		  		    <td class="column-title">
		  				 <bean:message bundle="agentmaintenance" key="agentmaintenance.fenpairesion"/>
		  		    </td>
		  		    <td class="column-content" colspan="3">
                          <pre><bean:write name="baselink" property="linkComUntreadReason" scope="page"/></pre>
		            </td>                  
		  		 </tr>       		  		 
		  	<%}%>
		  <%if(baselink.getOperateType()!=null && baselink.getOperateType().intValue() == 8){%> 
				  <tr>  				
		  		    <td class="column-title">
		  				 <bean:message bundle="agentmaintenance" key="agentmaintenance.yijiaoresion"/>
		  		    </td>
		  		    <td class="column-content" colspan="3">
                          <pre><bean:write name="baselink" property="linkTaskComplete" scope="page"/></pre>
		            </td>                  
          ...
          <%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("AffirmHumTask")){%> 
             <%if(baselink.getOperateType()!=null && ( baselink.getOperateType().intValue() == 211 || baselink.getOperateType().intValue() == 212)){%> 				
  				 <tr>
	  				  <td class="column-title">
	  				     <bean:message bundle="agentmaintenance" key="agentmaintenance.advice"/>
	  				  </td>
	  				  <td class="column-content" colspan=3>
	  				   	 <pre><bean:write name="baselink" property="linkTaskComplete" scope="page"/></pre>
	                  </td>
...
<script type="text/javascript">
 ...
 function copy(id){
    ...
    Ext.Ajax.request({
		method:"get",
		url: "agentmaintenance.do?method=getJsonLink&id="+id+"&beanName=AgentMaintenance",
		success: function(x){
