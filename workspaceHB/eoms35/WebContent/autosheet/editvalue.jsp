...
<p><b><font size="5">
    <center>编辑表单输入框</center>
</font></b></p>
<table cellspacing=0 cellpadding=0 border=0 width="30%" align="center">
    <tr>

        <td nowrap height="34"><a href="./sheetattr.jsp?sheetID=<%out.print(sheet_id);%>&reFactor=reFactor">增加输入框名称</a>
        </td>
        <td nowrap height="34"><a href="./sheetvalue.jsp?sheetID=<%out.print(sheet_id);%>&reFactor=reFactor">增加输入框</a>
        </td>

    </tr>
</table>
<table id=table1 cellspacing=0 cellpadding=0 border=1 width="100%" bordercolordark="#FFFFFF" bordercolorlight="#66CCFF"
    ...
    <tr>
        ...
        <td>&nbsp;
            <%
                if (rs.getString(7).equals("1"))
                    ...
                    out.println("否");
            %></td>
        <td nowrap>&nbsp;<%=shAttr.getAttrName(rs.getString(8))%>
        </td>
        <td>&nbsp;<%=rs.getString(9)%>
        </td>
        <td>&nbsp;<% String valign = "";
        ...
        <td nowrap>&nbsp;<%=rs.getString(13)%>
        </td>
        <td nowrap>&nbsp;<%=rs.getString("height")%>
        </td>
        ...
        <td>&nbsp;<%=comm.getInputType(StaticMethod.getIntValue(rs.getString(17), 0))%>
        </td>
        <td>&nbsp;<%=rs.getString(18)%>
        </td>
        <td>&nbsp;<%=rs.getString(19)%>
        </td>
        <td>&nbsp;<%=codetype.getTypeName(rs.getString(20))%>
        </td>
        <td nowrap>&nbsp;<%=rs.getString(21)%>
        </td>
        <td>&nbsp;<%=rs.getString("DEFAULTVAL")%>
        </td>
        <td>&nbsp;<%=rs.getString("V_CTRL")%>
        </td>
        <td nowrap><a href="./editValueDetail.jsp?value_id=<%=rs.getString(1)%>&sheet_id=<%=sheet_id%>">修改</a></td>
        <td nowrap><a
                href="<%=request.getContextPath()%>/sheetvalueservlet?flag=delete&value_id=<%=rs.getString(1)%>&sheet_id=<%=sheet_id%>"
