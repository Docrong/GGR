...
<html:form method="post" action="/workserialselect.do?method=saveWorkSerial" styleId="tawSystemSessionForm">
    ...
    <div class="room">
    <div id="room-text"><img src="${app}/images/icons/computer.gif" align="absmiddle">&nbsp;<fmt:message
            key="SaveSessionBean.selectRoom"/></div>
    <div id="room-select">
        <select size="<%=vecId.size()%>" style="width:350px" name="Wrf_RoomID" onchange="onRoomSel(this.value);">
            <%for (int i = 0; i < vecId.size(); i++) {%>
            <option value='<%=vecId.get(i).toString()%>'><%=vecName.get(i).toString()%>
            </option>
            <%}%>
        </select>
    </div>
    <div id="workserial" class="hide">
        <div id="workserial-text"><img src="${app}/images/icons/date.gif" align="absmiddle">&nbsp;<fmt:message
                key="SaveSessionBean.selectWorkserial"/></div>
        <select id="str_workserial" name="str_workserial" style="width:350px"></select>
    </div>

    <div id="btn">
        <input type="submit" name="submit" id="submit" class="submit" value='<fmt:message key="label.ok" />'
               onclick="return fun_confirm();">
        <div>
        </div>
