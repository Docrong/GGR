...
<script type="text/javascript">
   ...
   Ext.onReady(function(){
	...
	tabs.addTab('sheetform', "<bean:message bundle="equipmentinstallation" key="equipmentinstallation.header"/>");
	...
	if ('${templateId}'!= null && '${templateId}' != "") {
		mgr.update("${app}/sheet/equipmentinstallation/equipmentinstallation.do?method=showTemplateInputSheetPage&templateId=${templateId}");
	} else {
		mgr.update("${app}/sheet/equipmentinstallation/equipmentinstallation.do?method=showNewInputSheetPage&processname=EquipmentInstallation");
	}
...
$('phaseId').value = "Audit";
	   	...
	   	if (templateManage == "templateManage") {
		   	...
		   	if (type == "new"){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "equipmentinstallation.do?method=saveTemplate&templateId=${templateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
  ...
  function removeTemplate() {
   if (confirm('<bean:message bundle="sheet" key="worksheet.delete.confirm" />')) {
		...
		thisform.action = "${app}/sheet/equipmentinstallation/equipmentinstallation.do?method=removeTemplate&templateId=${templateId}&type=${type}";
		thisform.submit();
	}
 }
</script>
...
<html:form action="/equipmentinstallation.do?method=newPerformAdd" styleId="theform">
	...
	<logic:notPresent name="templateManage">
		...
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./equipmentinstallation.do?method=getTemplatesByUserId',null,'left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes')">
