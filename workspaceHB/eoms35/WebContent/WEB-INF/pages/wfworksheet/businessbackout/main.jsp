...
<script type="text/javascript">
   ...
   Ext.onReady(function(){
	...
	tabs.addTab('sheetform', "<bean:message bundle="businessbackout" key="businessbackout.header"/>");
	tabs.activate('sheetform');     
    var el = Ext.get("idSpecial");
	var mgr = el.getUpdateManager();
	mgr.loadScripts = true;
	if ('${templateId}'!= null && '${templateId}' != "") {
		mgr.update("${app}/sheet/businessbackout/businessbackout.do?method=showTemplateInputSheetPage&templateId=${templateId}");
	} else {
		mgr.update("${app}/sheet/businessbackout/businessbackout.do?method=showNewInputSheetPage&processname=BusinessBackoutMainFlowProcess");
	}
   ...
   function changeType1(){
   	...
   	$('${sheetPageName}operName').value = '<bean:message bundle="businessbackout" key="operateType.send"/>';
   ...
   function changeType2(){
   	...
   	$('${sheetPageName}operName').value = '<bean:message bundle="businessbackout" key="operateType.draft"/>';
	   	...
	   	if (templateManage == "templateManage") {
		   	...
		   	if (type == "new"){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "businessbackout.do?method=saveTemplate&templateId=${templateId}&type=${type}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
  ...
  function removeTemplate() {
   if (confirm('<bean:message bundle="sheet" key="worksheet.delete.confirm" />')) {
		...
		thisform.action = "${app}/sheet/businessbackout/businessbackout.do?method=removeTemplate&templateId=${templateId}&type=${type}";
		thisform.submit();
	}
 }
</script>
...
<html:form action="/businessbackout.do?method=performAdd" styleId="theform">
	...
	<logic:notPresent name="templateManage">
		...
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./businessbackout.do?method=getTemplatesByUserId',null,'left=300,top=150,height=400,width=500,scrollbars=yes')">
