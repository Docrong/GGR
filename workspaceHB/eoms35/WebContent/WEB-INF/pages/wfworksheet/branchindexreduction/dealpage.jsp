...
<c:url var="urlDeal" value="/sheet/branchindexreduction/branchindexreduction.do">
...
<script type="text/javascript">
    ...
    Ext.onReady(function () {
        ...
        if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {
            strUrl = '${app}/sheet/branchindexreduction/branchindexreduction.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId=' + dealTemplateId;
        }
    ...
    function saveDealInfo() {
        ...
        if (v.check()) {
            ...
            Ext.Ajax.request({
                form: ajaxForm,
                method: "post",
                url: "${app}/sheet/branchindexreduction/branchindexreduction.do?method=performSaveInfo",
                success: function () {
                    alert("保存成功！");
                }
            });
            ...
            if (confirm("确认保存模板吗？")) {
                if (type == "new") {
                    Ext.Ajax.request({
                        form: ajaxForm,
                        method: "post",
                        url: "branchindexreduction.do?method=saveDealTemplate&operateType=${operateType}",
                        success: function () {
                            alert("保存模板成功！");
                        }
                    });
                    ...
                    Ext.Ajax.request({
                        form: ajaxForm,
                        method: "post",
                        url: "branchindexreduction.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}&operateType=${operateType}",
                        success: function () {
                            alert("保存模板成功！");
                        }
                    });
                ...
                if (templateManage == "templateManage") {
                    ...
                    if (type == "new") {
                        ...
                        Ext.Ajax.request({
                            form: ajaxForm,
                            method: "post",
                            url: "branchindexreduction.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
                            success: function () {
                                alert("保存模板成功！");
                            }
                        });
    ...
    function ifisCopy() {  // 提交时校验
        ...
        if (ope2 == '101') {
            ...
            if (document.getElementById("infoId") != null) {
                document.getElementById("infoId").value = infoIds;
            }
            if (document.getElementById("infoReserveC") != null) {
                document.getElementById("infoReserveC").value = reasons;
            }
            if (document.getElementById("infoReserveD") != null) {
                document.getElementById("infoReserveD").value = descs;
            }
...
<div id="sheetform">
    <html:form action="/branchindexreduction.do?method=performDeal" styleId="theform">
            ...
            <tr>
                <td width="100%" align="left">
                    <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'RejectTask'&& taskName != 'DraftTask'}">
                        ...
                        <input type="hidden" name="dictKey" value="dict-sheet-branchindexreduction"/>
                    </c:if>
                </td>
            </tr>
        </table>
        ...
        <c:choose>
            <c:when test="${operateType=='61'}">
                <div class="form-btns" id="btns">
                    <input type="submit" class="submit"
                           onclick="this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=performClaimTask'"
            ...
            <c:when test="${taskName=='DraftTask'}">
                <div class="form-btns" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=performSaveInfoAndExit&draft=1'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=showDraftList'"
            ...
            <c:when test="${taskName=='RejectTask'}">
                <div class="form-btns" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=performSaveInfoAndExit'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=showListsendundo'"
                           value="<bean:message bundle='sheet' key='button.back'/>"/>
                    <input type="submit" class="submit" title="执行该操作，工单将进入强制归档状态，其他人不能再处理工单"
                           onclick="v.passing=true;this.form.action='${app}/sheet/branchindexreduction/branchindexreduction.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=forceHold'"
                           value="放弃重派并归档"/>
            ...
            <c:otherwise>
                ...
                <div class="form-btns hide" id="btns">
                    <input type="submit" class="submit" name="method.save" id="method.save" onclick="return ifisCopy();"
