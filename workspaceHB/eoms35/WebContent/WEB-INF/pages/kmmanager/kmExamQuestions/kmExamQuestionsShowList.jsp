...
<table id="table${questionType}<%=count%>">
    <c:forEach items="${kmExamQuestionsList}" var="item">
        <tr id="rowId<%=count%>" width="100%">
            ...
            <input type="hidden" name="questionID${questionType}<%=count%>" id="questionID${questionType}<%=count%>"
                   value="${item.questionID}">
            <input type="hidden" name="contentID${questionType}<%=count%>" id="contentID${questionType}<%=count%>"
                   value="<%=contentIDArr[i]%>">
            <td class="content"><input type="text" name="score${questionType}<%=count%>" value="<%=scoreArr[i]%>"
                                       size="2" alt="allowBlank:false,vtext:'请设置分数'">分
            </td>
            <c:if test="${isPublic=='0'}">
                <td class="content" width="10%"><input type="button" name="del${questionType}<%=count%>" class="btn"
                                                       value="删除"
                                                       onclick="DeleteChoiceDB${questionType}<%=flag%>('rowId<%=count%>','<%=contentIDArr[i]%>')">
                </td>
            </c:if>
            <% count++;
                i++; %>
...
<table id="table${questionType}<%=count%>">
    <c:forEach items="${kmExamQuestionsList}" var="item">
        <tr id="rowId<%=count%>" width="100%">
            ...
            <input type="hidden" name="contentID${questionType}<%=count%>" id="contentID${questionType}<%=count%>"
                   value="">
            <td class="content"><input type="text" name="score${questionType}<%=count%>" size="2"
                                       alt="allowBlank:false,vtext:'请设置分数'">分
            </td>
            <td class="content"><input type="button" name="del${questionType}<%=count%>" class="btn" value="删除"
                                       onclick="DeleteChoice${questionType}<%=flag%>('rowId<%=count%>')"></td>
            <% count++; %>
        </tr>
    </c:forEach>
</table>
