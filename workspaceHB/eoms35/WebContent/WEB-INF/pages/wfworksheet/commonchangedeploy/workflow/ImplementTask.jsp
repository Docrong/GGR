...
<input type="hidden" name="phaseId" id="phaseId" value="${toPhaseIdMap[operateType]}" />
	<input type="hidden" name="hasNextTaskFlag" id="hasNextTaskFlag" value="true" />
		...
		<tr><td class="label">
			...
			<td colspan="3">
				<textarea class="textarea max" name="linkCutComment" 
								id="linkCutComment" alt="allowBlank:false,maxLength:255,vtext:'请填入 实施情况说明 信息，最多输入 255 字符'">${sheetLink.linkCutComment}</textarea>
		...
		<tr><td class="label">
			...
			<td colspan="3">
				<textarea class="textarea max" name="linkEffectBusinessInstance" 
								id="linkEffectBusinessInstance" alt="allowBlank:false,maxLength:255,vtext:'请填入 影响业务情况说明 信息，最多输入 255 字符'">${sheetLink.linkEffectBusinessInstance}</textarea>
			...
			<td colspan="3">
				<textarea class="textarea max" name="linkAlertRecord" 
								id="linkAlertRecord" alt="allowBlank:false,maxLength:255,vtext:'请填入 告警情况记录 信息，最多输入 255 字符'">${sheetLink.linkAlertRecord}</textarea>
			...
			<td colspan="3">
				<textarea class="textarea max" name="linkUnnormalComment" 
								id="linkUnnormalComment" alt="allowBlank:false,maxLength:255,vtext:'请填入 统计报告异常说明 信息，最多输入 255 字符'">${sheetLink.linkUnnormalComment}</textarea>
		...
		<tr>
			...
			<td colspan="3">
				<eoms:attachment name="sheetLink" property="linkTestReport" scope="request" idField="linkTestReport" appCode="commonchangedeploy" /> 
...
<c:if test="${operateType == '93'}">
	...
	<fieldset id="link4">
		...
		<div ID="permit3">  
			<eoms:chooser id="sendObject" type="role" roleId="405" flowId="${flowId}" config="{returnJSON:true,showLeader:true,mainPanelTitle:'可选择的子角色'}"
...
<c:if test="${operateType == '93' || operateType == '11'}">		
		...
		<tr><td class="label">
			...
			<td class="content" colspan="3">
				<eoms:attachment name="sheetLink" property="linkTestReport"  scope="request" idField="linkTestReport" appCode="commonchangedeploy"  viewFlag="Y"/> 
