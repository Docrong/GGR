...
<div id="sheet-detail-page">
  ...
  <div id="sheetinfo">
	...
	<tr>
	  ...
	  <td class="content">   
       	 		<c:forTokens items="${sheetMain.mainEquipmentFactory}" delims="," var="mainEquipmentFactory" varStatus="status"><eoms:id2nameDB id="${mainEquipmentFactory}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
      </td>
	</tr>
	  ...
	  <tr>
		...
		<td class="content">
			<bean:write name="sheetMain" property="mainExpectationCompleteTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="request"/>
		</td>
...
<!--    <tr>
	  ...
	  <tr>
		...
		<td class="content"  colspan='3'><pre><bean:write name="sheetMain" property="mainChangeExplain" scope="request"/></pre></td>	  
	  </tr>
	  <tr>
		...
		<td class="content"  colspan='3'><eoms:attachment name="sheetMain" property="sheetAccessories" 
		            scope="request" idField="sheetAccessories" appCode="netchange" 
		             viewFlag="Y"/></td>	  
	  </tr>
...
<div class="sheet-deal">
		 ...
		 <!-- 是父任务 -->
			...
			<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
					<!-- 流程步骤，移交，阶段回复，分派 -->
		  			<option value="${urlShowProjectCreateDealPage}"><bean:message bundle="netchange" key="operateType.ProjectCreateTask"/> </option>
			...
			<%} else { %>
				...
				<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
					<option value="${urlShowDispatchPage}"><bean:message bundle="netchange" key="operateType.subTaskReply"/></option>
				<%}%>
			<% } %>
		...
		<select id="dealSelector">
		 ...
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
			...
			<%if(ifsub.equals("") || ifsub.equals("false")){%>
				<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
			  		<option value="${urlShowAuditDealPage}"><bean:message bundle="netchange" key="operateType.AuditTask"/> </option>
			  		<option value="${urlShowTransferkPage}"><bean:message bundle="sheet" key="common.transfer"/></option>
			  		<option value="${urlShowPhaseReplyPage}"><bean:message bundle="sheet" key="event.reply"/></option>
			 	<%} %>
			<%} else { %>
		...
		<select id="dealSelector">
		 ...
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
			...
			<%if(ifsub.equals("") || ifsub.equals("false")){%>
				<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
			  		<option value="${urlShowPermitDealPage}"><bean:message bundle="netchange" key="operateType.PermitTask"/> </option>
			  		<option value="${urlShowTransferkPage}"><bean:message bundle="sheet" key="common.transfer"/></option>
			  		<option value="${urlShowPhaseReplyPage}"><bean:message bundle="sheet" key="event.reply"/></option>
			 	<%} %>
			<%} else { %>
		...
		<select id="dealSelector">
		 ...
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
			...
			<%if(ifsub.equals("") || ifsub.equals("false")){%>
				<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
			  		<option value="${urlShowExecuteDealPage}"><bean:message bundle="netchange" key="operateType.ExecuteTask"/> </option>
			  		<option value="${urlShowTransferkPage}"><bean:message bundle="sheet" key="common.transfer"/></option>
			  		<option value="${urlShowPhaseReplyPage}"><bean:message bundle="sheet" key="event.reply"/></option>
			 	<%} %>
			<%} else { %>
		...
		<select id="dealSelector">
		 ...
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
			...
			<%if(ifsub.equals("") || ifsub.equals("false")){%>
				<!-- 不需要等待回复 -->
				<% if (ifwaitfor.equals("false")) {%>
			  		<option value="${urlShowTestDealPage}"><bean:message bundle="netchange" key="operateType.TestTask"/> </option>
			  		<option value="${urlShowTransferkPage}"><bean:message bundle="sheet" key="common.transfer"/></option>
			  		<option value="${urlShowPhaseReplyPage}"><bean:message bundle="sheet" key="event.reply"/></option>
			 	<%} %>
			<%} else { %>
	...
	<%}}else if(taskName.equals("DraftTask")){%>	
		<select id="dealSelector">
		  <option value="${urlShowDraftDealPage}"><bean:message bundle="netchange" key="operateType.draft"/></option>
		</select>
	...
	<%}else if(taskName.equals("HoldTask")){ %>
		<select id="dealSelector">
		  <option value="${urlShowHoldDealPage}"><bean:message bundle="netchange" key="operateType.hold"/></option>
		</select>	
	<%}else if(taskName.equals("netChangeSubTask1")){ %>
        ...
        <%if(taskStatus.equals(Constants.TASK_STATUS_READY)){%>
		   <option value="${urlShowAcceptDealPage}"><bean:message bundle="sheet" key="common.accept"/> </option>
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
		  <option value="${urlShowCompleteDealPage}"><bean:message bundle="netchange" key="operateType.subTaskReply"/>
		  </option>
		  <%} %>
		</select>	
	<%} %>
