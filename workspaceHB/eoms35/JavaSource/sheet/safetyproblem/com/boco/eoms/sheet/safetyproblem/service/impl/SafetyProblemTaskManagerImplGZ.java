...
// Source File Name:   SafetyProblemTaskManagerImplGZ.java

package com.boco.eoms.sheet.safetyproblem.service.impl;

/*import com.boco.eoms.base.util.StaticMethod;
import com.boco.eoms.sheet.base.model.BaseMain;
import com.boco.eoms.sheet.safetyproblem.dao.ISafetyProblemTaskDAO;
...
public class SafetyProblemTaskManagerImplGZ extends SafetyProblemTaskManagerImpl {
		...
		throws Exception
	{
		...
		if ("1".equals(type))
		{
			sql = "select distinct task" + ruternColumnName + " from " + " SafetyProblemTaskUndo as task," + mainObject.getClass().getName() + " as main " + "where main.id =task.sheetKey and main.deleted <> 1 and main.id = task.sheetKey and task.taskStatus in ('2', '8') and task.taskDisplayName <> '草稿' and task.taskDisplayName <> '阶段回复' and task.operateRoleId in ('" + userId +"', '" + deptId + "')" + " ";
		...
		} else
		{
			...
			" from SafetyProblem_task task,(select * From SafetyProblem_link  where operatedeptid = '" + deptId + "' and operateuserid <>'" + userId + "') link  ,SafetyProblem_main main where " +
			" main.deleted <> 1 and task.taskowner in ('" + userId + "', '" + deptId + "') and taskstatus in ('2','8') and task.sheetkey = link.mainid(+) and task.taskDisplayName <> '草稿' and task.taskDisplayName <> '阶段回复' and mainid is null and task.sheetkey =main.id" + agentsql;
			sql = "select distinct decode(task.taskname,'AffirmHumTask',task.sheetkey,task.id) id,task.TASKNAME,task.TASKDISPLAYNAME,NULL CREATETIME,task.TASKSTATUS,'' PROCESSID,task.SHEETKEY,task.SHEETID,task.TITLE title1,task.ACCEPTTIMELIMIT,task.COMPLETETIMELIMIT,task.OPERATEROLEID,task.TASKOWNER,'' PRELINKID,'' FLOWNAME,'' CURRENTLINKID,'' OPERATETYPE,'' PARENTTASKID,'' SUBTASKDEALFALG,task.IFWAITFORSUBTASK,'' SUBTASKFLAG,0 CREATEYEAR,0 CREATEMONTH,0 CREATEDAY,task.SENDTIME,'' PREDEALDEPT,'' PREDEALUSERID,'' PARENTPROCESSNAME,'' preDealRoleId,main.title,main.revert " + 
			" from SafetyProblem_task task,(select * From SafetyProblem_link  where operatedeptid = '" + deptId + "' and operateuserid <>'" + userId + "') link  ,SafetyProblem_main main where " +
			" main.deleted <> 1 and task.taskowner in ('" + userId + "', '" + deptId + "') and taskstatus in ('2','8') and task.sheetkey = link.mainid(+) and task.taskDisplayName <> '草稿' and task.taskDisplayName <> '阶段回复'  and task.sheetkey =main.id" + agentsql;
			...
			Integer total = ((ISafetyProblemTaskDAO)getTaskDAO()).getCountTaskBySQL(counthql.toString());
			taskMap = ((ISafetyProblemTaskDAO)getTaskDAO()).getTaskListBySQL(hql.toString(), startIndex, length);
