...
LightPortal.prototype.turnOnRememberLoginUser = function () {
    ...
    if (userId != null && userId != "" && password != null && password != ""
        ...
        var params = "userId=" + userId
            + "&password=" + password;
        new Ajax.Request(Light.portal.contextPath + Light.loginRequest, {
            parameters: params,
...
LightPortalTab.prototype.responseGetPortletsByTab = function (t) {
    ...
    for (var i = 0; i < portletsCount; i++) {
        ...
        if (vPortlet.allowJS) {
            ...
            ajaxEngine.sendRequestAndUpdate(vPortlet.request, Light._PC_PREFIX + this.index + "_" + vPortlet.position + "_" + vPortlet.index, {
                evalScripts: true,
                parameters: pars
            });
        } else {
...
function changeLocale(locale) {
    ...
    new Ajax.Request(Light.portal.contextPath + Light.changeLocale, {
        parameters: params,
...
function changeTheme(id) {
    ...
    if (params.length > 0)
        new Ajax.Request(Light.portal.contextPath + Light.changeTheme, {
            parameters: params,
...
function addContent(id, name) {
    ...
    ;
    new Ajax.Request(Light.portal.contextPath + Light.addContentRequest, {
        parameters: params,
...
function responseAddContent(t) {
    if (Light.getCookie(Light._ON) == "on") {
        ...
        if (portlet.allowJS) {
            ...
            ajaxEngine.sendRequestAndUpdate(portlet.request, Light._PC_PREFIX + index + "_" + portlet.position + "_" + portlet.index, {
                evalScripts: true,
                parameters: pars
            });
        } else {
...
function signUpPortal(id) {
  ...
  for(var i = 0; i < len; i++) {
    ...
    new Ajax.Request(Light.portal.contextPath + Light.signUpRequest, {
        parameters: params,
...
function saveProfile(id) {
  ...
  var len =  document.forms['form_'+id]['plChannels'].length;  
   for(var i = 0; i < len; i++) {
    ...
    new Ajax.Request(Light.portal.contextPath + Light.profileRequest, {
        parameters: params,
