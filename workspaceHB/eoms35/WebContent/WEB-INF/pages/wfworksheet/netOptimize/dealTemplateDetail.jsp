       ...
       %>
			...
			<input type="hidden" name="${sheetPageName}phaseId" id="${sheetPageName}phaseId" value="callProcess" />
               ...
               <tr>
  				  ...
  				  <td class="content max" colspan=3>
  				   	<textarea name="linkTestResult" id="linkTestResult" class="textarea max" alt="allowBlank:false,maxLength:200,vtext:'${eoms:a2u('请输入分析/测试结果，最大长度为200个汉字')}'">${sheetLink.linkTestResult}</textarea>	  
                  </td>
	  		  </tr>	
	  		 <tr>  				
  			 <%}%>		
		...
		<%if(operateType.equals("56")|| operateType.equals("55")){ %>
			...
			<input type="hidden" name="${sheetPageName}phaseId" id="${sheetPageName}phaseId" value="ExcuteTask" />
               <tr>                 
  				  ...
  				  <td class="content max" colspan=3>
  				   <eoms:comboBox name="${sheetPageName}linkAuditOpinion" id="${sheetPageName}linkAuditOpinion" alt="allowBlank:false" initDicId="1011804" onchange="AuditIfPass(this.value);" />                
                  </td>	                  
	  		  </tr>				
			 ...
			 <tr>
				  <td class="label">
		 		  ...
		 		  <td class="content max"  colspan=3>
				     <textarea name="linkAuditResult" id="linkAuditResult" class="textarea max" alt="allowBlank:false,maxLength:100,vtext:'${eoms:a2u('请输入审批结果，最大长度为100个汉字！')}'">${sheetLink.linkAuditResult}</textarea>	  
                </td>
  		       </tr>		
	      <%} }%>	
			   ...
			   <input type="hidden" name="${sheetPageName}phaseId" id="${sheetPageName}phaseId" value="ResultEvaluateTask" />
		  		...
		  		<tr>
	  				  ...
	  				  <td class="content max"  colspan=3>
 	                       ...
 	                       id="linkSummarize" class="textarea max" alt="allowBlank:true,maxLength:200,vtext:'${eoms:a2u('您输入网优实施概述超出最大长度限制，最多支持200个汉字！')}'" >${sheetLink.linkSummarize}</textarea>
		              </td>
		  		</tr>
		  ...
		  <%if(operateType.equals("64") || operateType.equals("11")){%>
			...
			<input type="hidden" name="${sheetPageName}phaseId" id="${sheetPageName}phaseId" value="HoldTask" />
		          ...
		          <tr>
 	                   ...
 	                   id="linkResult" class="textarea max" alt="allowBlank:false,maxLength:1000,vtext:'${eoms:a2u('请输入评定结果，最大长度为200个汉字！')}'">${sheetLink.linkAssessResult}</textarea>
	                  </td>
		  		 </tr>
