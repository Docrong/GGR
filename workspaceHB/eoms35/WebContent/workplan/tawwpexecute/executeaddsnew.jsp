...
<table>
    ...
    <tr>
        <td>
            ...
            %>
            <form name="executeadds_<%=formindex %>_<%=stubFlag %>" method="POST"
                  action="../tawwpexecute/executesavesnew.do?contentidstr=<%=contentIdStr.substring(0,contentIdStr.length()-1)%>&flag=<%=flag%>&executetype=<%=executeType%>&formindex=<%=formindex %>&stubFlag=<%=stubFlag %>">
                <table border="0" cellspacing="1" cellpadding="1" class="listTable" align=center width="90%">
                    <caption><font color="red">
                        计划：<%=tawwpmonthplanvo.getName() %>&nbsp;&nbsp;&nbsp;&nbsp;网元:<%=tawwpmonthplanvo.getNetName() %>
                    </font></caption>
                    <thead>
                    <tr>
                        <td width="15%">
                    ...
                    <tr class="tr_show">
                        <td width="15%" class="clsthd2">
                            ...
                            <input type="hidden"
                                   name="executecontentuserid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getExecuteContentUserId()%>">
                            <input type="hidden" name="executecontentid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                            ...
                            <input type="hidden" name="userid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getUserByStub()%>">
                            <input type="hidden" name="stubuser<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
                            <%
                            } else {
                            %>
	        ...
	        <td width="15%">
	         ...
	         -->
                        ...
                        <td width="10%">
                            <input type="hidden" name="writeDate<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   size="20" class="text"
                                   value="<%=StaticMethod.getCurrentDateTime()%>"><%=StaticMethod.getCurrentDateTime() %>
                        </td>
                        <td width="20%">
                            <textarea name="content<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getContent()%></textarea>
                        </td>
                        <td width="6%" align=center>
                            <textarea name="remark<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getRemark()%></textarea>
                        </td>
                        <td width="10%" class="clsthd2" align="left">
                            ...
                            <input type="hidden" name="filename<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   dataType="null" value="<%=tawwpExecuteContentVO.getFileName()%>">
                            <input type="button" class="button"
                                   name="count<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   onclick="javascript:onFile(filename<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,count<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,executecontentid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,executecontentuserid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,userid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,stubuser<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>,'showLink_<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>','file<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>','executeadds_<%=formindex %>_<%=stubFlag %>');"
                        ...
                        <td width="10%" class="clsthd2" rowspan="<%=strArray.length%>">
                            ...
                            %>
                            <a href="javascript:onAddons('../tawwpaddons/addonsread.do?action=new&window=new&formid=<%=tawwpExecuteContentVO.getFormId()%>&monthid=<%=tawwpmonthplanvo.getId() %>&date=<%=date%>&executeId=<%=tawwpExecuteContentVO.getId() %>&reaction=/tawwpexecute/redirection.jsp');"><%=tawwpExecuteContentVO.getFormName()%>
                            </a>
                            <input type="hidden" name="formdataid<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=formDataId_%>">
                            <%} else { %>
                        ...
                        <td width="6%" align=center>
                            <iframe name="workflowFrame<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>"
                            ...
                            <div align=center>
                                ...
                                <input type="button" value="添加" class="button"
                                       onclick="javascript:addworkflow('<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>','<%=tawwpExecuteContentVO.getId()%>')">
                            </div>
                    ...
                    <tr class="tr_show">
                        <td width="15%" class="clsthd2">
                            <%=tawwpExecuteContentVO.getName()%>
                            <input type="hidden"
                                   name="executecontentuserid<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getExecuteContentUserId()%>">
                            <input type="hidden" name="executecontentid<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getId()%>">
                            <%
                                if ("1".equals(tawwpExecuteContentVO.getStubFlag())) {
                            %>
                            [<font color="red"><bean:message key="stubuseradd.title.formAgent"/></font>]
                            <input type="hidden" name="userid<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getUserByStub()%>">
                            <input type="hidden" name="stubuser<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
                            <%} else {%>
                            <input type="hidden" name="userid<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
	        ...
	        <td width="15%">
	         ...
	         -->
                        ...
                        <td width="10%">
                            <input type="hidden" name="writeDate<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   size="20" class="text"
                                   value="<%=StaticMethod.getCurrentDateTime()%>"><%=StaticMethod.getCurrentDateTime() %>
                        </td>
                        <td width="20%">
                            <textarea name="content<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getContent()%></textarea>
                        </td>
                        <td width="6%" align=center>
                            <textarea name="remark<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getRemark()%></textarea>
                        </td>
                        <td width="10%" class="clsthd2" align="left">
                        ...
                        <td width="6%" align=center>
                            ...
                            %>
                            <input type="hidden" name="formdataid<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                   value="<%=formDataId%>">
                        </td>
                        <td width="6%" align=center>
                            <iframe name="workflowFrame<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>"
                                    src="../tawwpexecute/workflowList.do?executeId=<%=tawwpExecuteContentVO.getId()%>"
                                    frameborder=0></iframe>
                            <div align=center>
                                ...
                                &nbsp;
                                <input type="button" value="添加" class="button"
                                       onclick="javascript:addworkflow('<%=strArray[k]%>_<%=formindex %>_<%=stubFlag %>','<%=tawwpExecuteContentVO.getId()%>')">
                            </div>
                ...
                <input type="submit" value="<bean:message key="executeadds.title.btnSubmit" />" name="B1"
                       class="submit">

                <input type="button" value='复制并保存' name="B3" class="button"
                       onclick="javascript:onCopy('<%=tawwpmonthplanvo.getId() %>','<%=tawwpmonthplanvo.getUserByStub() %>','<%=date%>','<%=flag %>');">
         ...
         -->
            ...
            <%
                ...
                while (stubi.hasNext()) {
                    ...
                    if (contentHash.size() != 0) {
            %>
            <form name="executeadds_<%=stubformindex %>_<%=stubFlag %>" method="POST"
                  action="../tawwpexecute/executesavesnew.do?contentidstr=<%=contentIdStr.substring(0,contentIdStr.length()-1)%>&flag=<%=flag%>&executetype=<%=executeType%>&formindex=<%=stubformindex %>&stubFlag=<%=stubFlag %>">
                ...
                <table border="0" cellspacing="1" cellpadding="1" class="listTable" align=center width="90%">
                    ...
                    <tr class="tr_show">
                        <td width="15%" class="clsthd2">
                            <%=tawwpExecuteContentVO.getName()%>
                            <input type="hidden"
                                   name="executecontentuserid<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getExecuteContentUserId()%>">
                            <input type="hidden"
                                   name="executecontentid<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getId()%>">
                            <%
                                if ("1".equals(tawwpExecuteContentVO.getStubFlag())) {
                            %>
                            [<font color="red"><bean:message key="stubuseradd.title.formAgent"/></font>]
                            <input type="hidden" name="userid<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getUserByStub()%>">
                            <input type="hidden" name="stubuser<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
                            <%
                            } else {
                            %>
	        ...
	        <td width="15%">
	         ...
	         -->
                        ...
                        <td width="10%">
                            <input type="hidden" name="writeDate<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   size="20" class="text"
                                   value="<%=StaticMethod.getCurrentDateTime()%>"><%=StaticMethod.getCurrentDateTime() %>
                        </td>
                        <td width="20%">
                            <textarea name="content<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getContent()%></textarea>
                        </td>
                        <td width="6%" align=center>
                            <textarea name="remark<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getRemark()%></textarea>
                        </td>
                        <td width="10%" class="clsthd2" align="left">
                        ...
                        <td width="10%" class="clsthd2" rowspan="<%=strArray.length%>">
                            ...
                            <input type="hidden" name="formdataid<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=formDataId_%>">
                            <%} else { %>
                            <a href="javascript:onAddons('../tawwpaddons/addonsread.do?action=edit&window=edit&executeId=<%=tawwpExecuteContentVO.getId() %>&reaction=/tawwpexecute/redirection.jsp');"><%=tawwpExecuteContentVO.getFormName()%>
                            </a>
                        ...
                        <td width="6%" align=center>
                            <iframe name="workflowFrame<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>"
                                    src="../tawwpexecute/workflowList.do?executeId=<%=tawwpExecuteContentVO.getId()%>"
                                    frameborder=0></iframe>
                            <div align=center>
                                ...
                                &nbsp;
                                <input type="button" value="添加" class="button"
                                       onclick="javascript:addworkflow('<%=strArray[0]%>_<%=stubformindex %>_<%=stubFlag %>','<%=tawwpExecuteContentVO.getId()%>')">
                            </div>
                    ...
                    <tr class="tr_show">
                        <td width="15%" class="clsthd2">
                            <%=tawwpExecuteContentVO.getName()%>
                            <input type="hidden"
                                   name="executecontentuserid<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getExecuteContentUserId()%>">
                            <input type="hidden"
                                   name="executecontentid<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getId()%>">
                            <%
                                if ("1".equals(tawwpExecuteContentVO.getStubFlag())) {
                            %>
                            [<font color="red"><bean:message key="stubuseradd.title.formAgent"/></font>]
                            <input type="hidden" name="userid<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=tawwpExecuteContentVO.getUserByStub()%>">
                            <input type="hidden" name="stubuser<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
                            <%} else {%>
                            <input type="hidden" name="userid<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=currUser%>">
	        ...
	        <td width="15%">
	         ...
	         -->
                        ...
                        <td width="10%">
                            <input type="hidden" name="writeDate<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   size="20" class="text"
                                   value="<%=StaticMethod.getCurrentDateTime()%>"><%=StaticMethod.getCurrentDateTime() %>
                        </td>
                        <td width="20%">
                            <textarea name="content<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getContent()%></textarea>
                        </td>
                        <td width="6%" align=center>
                            <textarea name="remark<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>" rows=3 cols="45"
                                      value=""><%=tawwpExecuteContentVO.getRemark() %></textarea>
                        </td>
                        <td width="10%" class="clsthd2" align="left">
                        ...
                        <td width="6%" align=center>
                            ...
                            %>
                            <input type="hidden" name="formdataid<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                   value="<%=formDataId%>">
                        </td>
                        <td width="6%" align=center>
                            <iframe name="workflowFrame<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>"
                                    src="../tawwpexecute/workflowList.do?executeId=<%=tawwpExecuteContentVO.getId()%>"
                                    frameborder=0></iframe>
                            <div align=center>
                                ...
                                &nbsp;
                                <input type="button" value="添加" class="button"
                                       onclick="javascript:addworkflow('<%=strArray[k]%>_<%=stubformindex %>_<%=stubFlag %>','<%=tawwpExecuteContentVO.getId()%>')">
                            </div>
