...
<form name="form1" action="<%=app %>/webtool/tag" onSubmit="return checkInput()">
    ...
    <table class="form_table">
        <tr>
            <th>此操作返回变量名称：</th>
            <td>
                <input id="name" name="name" type="text" value="<%=tag.getName() %>" size="20"><font
                    color="#FF0000">*</font>
            </td>
        </tr>
        <tr>
            <th>SQL语句：</th>
            <td>
                <textarea rows="4" id="statement" name="statement" type="text"><%=tag.getStatement() %></textarea><font
                    color="#FF0000">*</font>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="button" value="选择参数变量"
                       onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=7','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
            </td>
        </tr>
        <tr>
            <th>SQL类型</th>
            <td>
                <select id="sqlType" name="sqlType"
                        onchange="javascript:changeVerOrDis(form1.sqlType.value,form1.verOrDis.value)">
                    <option value="select" <%if (tag.getSqlType().equals("select")) out.print("selected"); %>>select
                    </option>
                    <option value="update" <%if (tag.getSqlType().equals("update")) out.print("selected"); %>>update
                    </option>
                    <option value="insert" <%if (tag.getSqlType().equals("insert")) out.print("selected"); %>>insert
                    </option>
                    <option value="delete" <%if (tag.getSqlType().equals("delete")) out.print("selected"); %>>delete
                    </option>
                </select>
            </td>
        </tr>
        <tr>
            <th>结果显示或判断</th>
            <td>
                <select id="verOrDis" name="verOrDis"
                        onchange="javascript:changeVerOrDis(form1.sqlType.value,form1.verOrDis.value)">
                    <option value="display" <%if (tag.getVerOrDis().equals("display")) out.print("selected"); %>>显示
                    </option>
                    <option value="verify" <%if (tag.getVerOrDis().equals("verify")) out.print("selected"); %>>判断
                    </option>
                </select>
            </td>
        </tr>
...
%>
		...
		<tr>
		<th>是否分页</th>
		<td><select id="isPageination" name="isPageination"
                    onchange="javascript:isPageinationPageSize(form1.isPageination.value)">
		...
		<tr>
	       ...
	       <td><textarea rows="4" id="text8" name="text" type="text"><%=tag.getText()%></textarea>
		      <input type="button" value="选择变量参数"
                     onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=8','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
		  </td>
	    </tr>
		</table>
    ...
    <table class="form_table">
            ...
            <td>
                <select id="verifyMethod" name="verifyMethod" onchange="javascript:verValue(form1.verifyMethod.value)">'+
                    <option value="listSize" <%
                        if (!(tag.getVerifyMethod().indexOf("listSize") < 0)) out.print("selected"); %>>判断记录数
                    </option>
                    <option value="isEqual" <%
                        if (!(tag.getVerifyMethod().indexOf("isEqual") < 0)) out.print("selected"); %>>两值比对
                    </option>
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><input id="text1" type="text" name="text" value=<%=content[0] %>>
                <input type="button" value="选择变量参数"
                       onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=1','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
            </td>
        </tr>
        <tr>
            <td>校验失败显示内容</td>
            <td><input id="text2" type="text" name="text" value=<%=content[1] %>>
                <input type="button" value="选择变量参数"
                       onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=2','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
            </td>
        </tr>
    </table>
    <span id="span_verValue">
		    ...
		    <table>
			  <tr>
			    <td>
			       ...
			       <input type="button" value="选择变量参数"
                          onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=5','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
			    </td>
			    <td>
			        比对值：<input id="text6" name="method_value" type="text" value="<%=temp[1]%>"/>
			        <input type="button" value="选择比对参数"
                           onclick="window.open('<%=app %>/webtool/common/getNodeVariableList.jsp?wapNodeId=<%=wapNodeId %>&k=6','','height=400,width=340,status=no,toolbar=no,menubar=no,location=no')">
			    </td>
			  </tr>
			</table>
		<%} %>
