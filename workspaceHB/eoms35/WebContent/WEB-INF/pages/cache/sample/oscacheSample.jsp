...
<table border="1">
    ...
    <tr>
        ...
        <td>
            任意包含pattern属性指定的值的缓存项都将被刷新。只指定一个pattern值而没有指定scope不起作用。
            (注意：OSCache项目组已经不赞成使用pattern这个属性赖刷新缓存，二是鼓励使用具有更好的灵活性和性能的group属性来代替)
        </td>
    </tr>
    <tr>
        <td>
...
<li>
    ...
    use
    告诉所在的
    <cache>标签是否使用已经缓存的内容（缺省为true，使用缓存的内容）。可以使用这个标签来控制缓存。比如使用
        <frush>标签刷新某个key的缓存，但可以在必要的地方即使这样的强制刷新也仍然使用缓存内容而不刷新。
...
<li>
    ...
    addgroup标签
    addgroup:必须嵌套在cache标签中。It allows groups to be dynamically added to a cached block. It is useful when the group(s) a
    cached block should belong to are unknown until the block is actually rendered. As each group is 'discovered', this
    tag can be used to add the group to the block's group list.
