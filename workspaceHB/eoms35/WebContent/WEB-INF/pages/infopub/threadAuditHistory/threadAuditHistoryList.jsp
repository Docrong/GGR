...
<html:form action="/threadAuditHistory.do?method=audit" method="post" styleId="threadAuditHistoryForm">
    ...
    <display:table name="threadAuditHistoryList" cellspacing="0" cellpadding="0" id="threadAuditHistoryList"
                   pagesize="${pageSize }" class="table threadAuditHistoryList" export="true"
        ...
        <display:column sortable="true" headerClass="sortable" titleKey="infopub.threadAuditHistoryForm.threadId"
                        paramId="id" paramProperty="threadId" href="${app}/workbench/infopub/thread.do?method=detail">
            <eoms:id2nameDB id="${threadAuditHistoryList.threadId}" beanId="threadDao"/>
        </display:column>


        <display:column property="status" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.status"
                        href="${app}/workbench/infopub/threadAuditHistory.do?method=device" paramId="threadId"
                        paramProperty="threadId"/>

        <display:column property="submitTime" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.submitTime" format="{0,date,yyyy-MM-dd HH:mm:ss}"/>

        <display:column property="auditTime" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.auditTime" format="{0,date,yyyy-MM-dd HH:mm:ss}"/>

        <display:column property="orgId" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.orgId"/>

        <display:column property="opinion" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.opinion"/>

        <display:column property="note" sortable="true" headerClass="sortable"
                        titleKey="infopub.threadAuditHistoryForm.note"/>
    ...
    <logic:notEmpty name="threadAuditHistoryList">
        ...
        <input type="button" onclick="javascript:submitForm('<%=InfopubConstants.AUDIT_PASS%>');"
               value="<bean:message key='threadAuditHistoryForm.button.passaudit' />"/>

        <input type="button" onclick="javascript:submitForm('<%=InfopubConstants.AUDIT_NO_PASS%>');"
               value="<bean:message key='threadAuditHistoryForm.button.nopassaudit' />"/>
    </logic:notEmpty>
