...
public  class NetDataMethod extends BaseSheet {
   /* (non-Javadoc)
    ...
    public HashMap getInterfaceObjMap(ActionMapping mapping, ActionForm form,
		...
		if(taskName.equals("ProjectCreateTask")){
			String mainIsNeedDesign = StaticMethod.nullObject2String(request.getParameter("mainIsNeedDesign"));
    	...
    	try{
    		if(operatName.equals("forceHold") ){
	    		...
	    		hashMap.put("selfSheet", sheetMap);
					...
					taskName.equals("CheckDataTask")||
					taskName.equals("TestOperationTask")||
    ...
    public void dealFlowEngineMap(ActionMapping mapping, ActionForm form,
		...
		//TODO 变更来源不是其它模块，则将status设为1（归档），phaseId设为over，
		...
		if(taskName.equals("ProjectCreateTask")){
			...
			main.put("mainExecuteEndDate", link.get("linkCompleteLimitTime"));
			sheetMap.put("main", main);
			
			String[] extendPerformers = (StaticMethod.nullObject2String(operate.get("extendPerformer"))).split(",");
		...
		if(taskName.equals("PermitTask") || taskName.equals("PlanTask")){
//			Map link = (HashMap) sheetMap.get("link");
			Map operate = (HashMap)sheetMap.get("operate");
...
//				link.put("correlationKey", corrKey);
				if (operateType.equals("112") || operateType.equals("113")) {
					String linkId = "";
					String tmpLink = "";
					for(int i=0;i<dealperformers.length;i++){
						tmpLink = UUIDHexGenerator.getInstance().getID()+",";
		...
		Map operate = (HashMap)sheetMap.get("operate");
		String extendPerformer = StaticMethod.nullObject2String(operate.get("extendPerformer"));
		if(!extendPerformer.equals("")){
			...
			operate.put("extendKey1", tmpExtendKey1.substring(0,tmpExtendKey1.length()-1));
			operate.put("extendKey2", tmpExtendKey2.substring(0,tmpExtendKey2.length()-1));
			sheetMap.put("operate", operate);
		}
	...
	public void showInputNewSheetPage(ActionMapping mapping, ActionForm form,
			...
			try{
				INetDataMainDAO mainDAO = (INetDataMainDAO)this.getMainService().getMainDAO();
				...
				request.setAttribute("mainAssortSpeciality",UUIDHexGenerator.getInstance().getID());
			}catch(Exception e){
				e.printStackTrace();
			}
			//增加Step1处理的附件模板
	...
	public void showInputDealPage(ActionMapping mapping, ActionForm form,
		...
		if (taskName.equals("RejectTask") ||taskName.equals("ProjectCreateTask")|| taskName.equals("AuditTask") || taskName.equals("ExecuteTask")|| taskName.equals("TestOperationTask")|| taskName.equals("CheckDataTask")|| taskName.equals("HoldTask")) {
		...
		//是否立即进行业务测试
		if(taskName.equals("CheckDataTask")){
			INetDataLinkManager linkservice = (INetDataLinkManager)this.getLinkService();
			String sheetKey = StaticMethod.nullObject2String(request.getParameter("sheetKey"), "");
			List list = linkservice.getLinkbyOperateType(sheetKey, "111");
			if(list!=null&&list.size()>0){
				NetDataLink linkBean = (NetDataLink)list.get(0);
				if(linkBean.getLinkIfStartOperationTest().equals("1030101")){
					request.setAttribute("ifTest", "yes");
				}else{
					request.setAttribute("ifTest", "no");
				}
			}
		}
		...
		if(taskName.equals("HoldTask")){
			...
			System.out.println("netdata 执行了回调 ===========");
		    }
		}
	}
	/**
	...
	public HashMap makeStep1(HashMap sheetMap) throws Exception {
		...
		if(operateMap.get("phaseId").equals("AuditTask")){
			...
			//生成新增工单的link记录
			NetDataLink link1 = (NetDataLink) this.getLinkService()
			...
			//生成T1处理的确认受理的link记录
			NetDataLink link2 = (NetDataLink) this.getLinkService()
					.getLinkObject().getClass().newInstance();
			...
			//生成T1处理的Task记录
			NetDataTask task = (NetDataTask) this.getTaskService()
					.getTaskModelObject();
			...
			task.setFlowName("NetDataMainProcess");
