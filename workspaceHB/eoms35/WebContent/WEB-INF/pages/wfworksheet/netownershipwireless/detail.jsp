...
<input type="hidden" name="id" id="id" value="${main.id }"/>
	<table id="sheet" class="formTable">	
		...
		<tr>
	 		<td class="label">铁塔属性</td>	
			<td class="content">
			   <eoms:id2nameDB id="${main.tttype}" beanId="ItawSystemDictTypeDao"/>
			</td>
			<td class="label">是否高山站</td>	
			<td class="content">${main.ifgs }</td> 
		</tr>
		
		<tr>
	 		<td class="label">是否免责站</td>	
			<td class="content">${main.ifmz }</td>
			<td class="label">是否高等级站</td>	
			<td class="content">${main.ifgj }</td> 
		</tr>
		
		<tr>
	 		<td class="label">代维工作站属性</td>	
			<td class="content">${main.workstatprop }</td>
			<td class="label">类别属性</td>	
			<td class="content">${main.classes}</td> 
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="content" colspan="3">
				<eoms:chooser id="test22" type="role" roleId="8005106" flowId="051" config="{returnJSON:true,showLeader:true}"
             		category="[{id:'teamRoleId',text:'选择',nodeType:'subrole',vtext:'请选择修改对象'}]" data="[{id:'${main.teamRoleId}',nodeType:'subrole',categoryId:'teamRoleId'}]" />	
            </td>          
		</tr>	
	</c:if>
	<c:if test="${ifRight!=null && main.teamRoleId==null}">
	...
	<c:if test="${ifRight!=null && main.ttRoleId!=null}">
		<tr>
			<td class="label">铁塔班组</td>
			<td class="content" colspan="3">
				<eoms:chooser id="test33" type="role" roleId="8005106" flowId="051" config="{returnJSON:true,showLeader:true}"
             		category="[{id:'ttRoleId',text:'选择',nodeType:'subrole',vtext:'请选择修改对象'}]" data="[{id:'${main.ttRoleId}',nodeType:'subrole',categoryId:'ttRoleId'}]" />	
            </td>          
		</tr>	
	</c:if>
	<c:if test="${ifRight!=null && main.ttRoleId==null}">
		<tr>
			...
			<td class="content" colspan="3">
				<eoms:chooser id="test33" type="role" roleId="8005106" flowId="051" config="{returnJSON:true,showLeader:true}"
             		category="[{id:'ttRoleId',text:'选择',nodeType:'subrole',vtext:'请选择修改对象'}]" />	
            </td>          
		</tr>	
	</c:if>
	<c:if test="${ifRight==null }">
		<tr>
			<td class="label">铁塔班组</td>
			<td class="content" colspan="3">
				<eoms:id2nameDB id="${main.ttRoleId }" beanId="tawSystemSubRoleDao"/>	
            </td>
			...
			<td class="content" colspan="3">
			 <c:if test="${main.ccObject!=null}">
				<eoms:chooser id="test2" type="role" roleId="8005106" flowId="051" config="{returnJSON:true,showLeader:true}"
             		category="[{id:'ccObject',text:'选择',nodeType:'subrole',vtext:'请选择修改对象'}]" data="[{id:'${main.ccObject}',nodeType:'subrole',categoryId:'ccObject'}]" />	
             </c:if>
             <c:if test="${main.ccObject==null}">
				<eoms:chooser id="test3" type="role" roleId="8005106" flowId="051" config="{returnJSON:true,showLeader:true}"
	...
	<c:if test="${ifRight!=null }">
		<input type="submit" name="method.xeditandsave" value="保存" id="xeditandsave" class="btn" onclick="return check();"/>	
	</c:if>
</html:form>	  
<script type="text/javascript">	
