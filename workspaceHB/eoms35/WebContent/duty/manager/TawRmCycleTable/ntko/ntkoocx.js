...
var TANGER_OCX_OBJ=document.all.item("TANGER_OCX"); //The Control
...
function TANGER_OCX_OpenDoc2(docid)
{
	TANGER_OCX_OBJ = document.all.item("TANGER_OCX");
	if(docid != "")
	{	
...
function TANGER_OCX_SaveDoc()
{
	...
	try
	{
		...
		var iterator = document.forms[0].iterator.value;
		 
		window.location.href="cycletable.do?id="+id+"&path="+path+"&cycle="+iterator+"&url="+url;
