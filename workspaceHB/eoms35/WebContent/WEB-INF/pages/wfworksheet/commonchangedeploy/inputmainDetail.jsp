...
<table id="sheet" class="formTable" >
 ...
 <td class="label">
				<!-- 设备厂家 -->
				<bean:message bundle="commonchangedeploy" key="commonChangeDeployMain.mainFactory"/>*</td>
  <td colspan="3">
	<div id="factoryview" class="hide"></div>
      ...
      <textarea class="textarea max" readonly="true" name="${sheetPageName}showFactory" style="height:50px" id="${sheetPageName}showFactory"  alt="allowBlank:true,maxLength:250,vtext:'请填入设备厂家，最多输入125个汉字'"><c:forTokens items="${sheetMain.mainFactory}" delims="," var="mainFactory" varStatus="status"><eoms:id2nameDB id="${mainFactory}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
 ...
 <td class="label">
			...
			<td>
						<input type="text"  class="text" name="mainApplyReason" id="mainApplyReason"  alt="allowBlank:false,maxLength:32,vtext:'请填入 申请依据 信息，最多输入 32 字符'" value="${sheetMain.mainApplyReason}"/>
			</td>
 <td class="label">
				<!-- 变更网元 -->
			...
			<td>
						<input type="text"  class="text" name="mainCellInfo" id="mainCellInfo"  alt="allowBlank:false,maxLength:32,vtext:'请填入 变更网元 信息，最多输入 32 字符'" value="${sheetMain.mainCellInfo}"/>
			</td>
		</tr>
 ...
 <td class="label">
			...
			<td>
						<input type="text"  class="text" name="mainChangeSource" id="mainChangeSource"  alt="allowBlank:false,maxLength:32,vtext:'请填入 变更来源 信息，最多输入 32 字符'" value="${sheetMain.mainChangeSource}"/>
			</td>
			</tr>
			<tr>
 <td class="label">
			...
			<td>
						<input type="text"  class="text" name="mainCheckModel" id="mainCheckModel"  alt="allowBlank:false,maxLength:32,vtext:'请填入 审核对象 信息，最多输入 32 字符'" value="${sheetMain.mainCheckModel}"/>
			</td>
 <td class="label">
				<!-- 完成时限 -->
 ...
 <tr>
		            ...
		            涉及省份及地市*</td>
    				  ...
    				  <br/><br/>	
    				  涉及省份:<textarea class="textarea max" readonly="true" name="mainInvolvedProvince" style="height:50px" id="${sheetPageName}mainInvolvedProvince"  alt="allowBlank:false,maxLength:4000,vtext:'请填入备注信息，最多输入4000字符'">${sheetMain.mainInvolvedProvince }</textarea><br/>
    				  涉及地市:<textarea class="textarea max" readonly="true" name="mainInvolvedCity" style="height:50px" id="${sheetPageName}mainInvolvedCity" alt="allowBlank:true,maxLength:4000,vtext:'请填入备注信息，最多输入4000字符'">${sheetMain.mainInvolvedCity}</textarea>
...
function callback(jsonData,str){
 ...
 <td class="label">
			...
			<td>
						<input type="text"  class="text" name="mainDesignKey" id="mainDesignKey"  alt="allowBlank:false,maxLength:32,vtext:'请填入 技术方案关键字 信息，最多输入 32 字符'" value="${sheetMain.mainDesignKey}"/>
			</td>
		</tr>
		<tr>
 <td class="label">
			...
			<td colspan="3">
				<textarea class="textarea max" name="mainDesignComment" 
						id="mainDesignComment" alt="allowBlank:true,maxLength:255,vtext:'请填入 技术方案说明 信息，最多输入 255 字符'">${sheetMain.mainDesignComment}</textarea>
		...
		<tr><td class="label">
			...
			<td  colspan="3">
				<textarea class="textarea max" name="mainRiskEstimate" 
						id="mainRiskEstimate" alt="allowBlank:false,maxLength:32,vtext:'请填入 风险评估 信息，最多输入 32 字符'">${sheetMain.mainRiskEstimate}</textarea>
 ...
 <td class="label">
			...
			<td colspan="3">
				<textarea class="textarea max" name="mainEffectAnalyse" 
						id="mainEffectAnalyse" alt="allowBlank:false,maxLength:255,vtext:'请填入 影响业务分析 信息，最多输入 255 字符'">${sheetMain.mainEffectAnalyse}</textarea>
 ...
 <td class="label">
			...
			<td colspan="3">
				<eoms:attachment name="sheetMain" property="mainProjectAccessories" scope="request" idField="mainProjectAccessories" appCode="commonchangedeploy" /> 
...
<fieldset>
    ...
    <div class="x-form-item" >
			<eoms:chooser id="sendObject"  type="role" roleId="406" flowId="${flowId}" config="{returnJSON:true,showLeader:true}"
...
<table  class="formTable" > 
		...
		<tr><td class="label">
			...
			<td  class="content" colspan="3">
				<eoms:attachment name="sheetMain" property="mainProjectAccessories"  scope="request" idField="mainProjectAccessories" appCode="commonchangedeploy"  viewFlag="Y"/> 
