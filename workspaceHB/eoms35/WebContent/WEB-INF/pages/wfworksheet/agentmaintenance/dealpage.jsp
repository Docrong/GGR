...
<c:url var="urlDeal" value="/sheet/agentmaintenance/agentmaintenance.do">
...
function initPage(){
	...
	if("${taskName}"!="cc"){
	// v.preCommitUrl = "${app}/sheet/agentmaintenance/agentmaintenance.do?method=performPreCommit";	
	}
 } 
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {	
   		strUrl = '${app}/sheet/agentmaintenance/agentmaintenance.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
    ...
    var form = document.forms[0];
   	var ajaxForm = Ext.getDom(form);
		    ...
		    Ext.Ajax.request({
				   		form: ajaxForm,
						method:"post",
						url: "${app}/sheet/agentmaintenance/agentmaintenance.do?method=performSaveInfo",
						success: function(){
				        	alert("${eoms:a2u('保存成功！')}");
				 		}
				    });
	   	...
	   	if (confirm("${eoms:a2u('确认保存模板吗？')}")) {
		    if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "agentmaintenance.do?method=saveDealTemplate",
					success: function(){
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "agentmaintenance.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
   	...
   	var templateManage = "${type}";
   	 ...
   	 v.passing = false;
	   	...
	   	if (templateManage == "templateManage") {
	   		form.action = "${app}/sheet/agentmaintenance/agentmaintenance.do?method=saveTemplate&templateId=${templateId}&type=${type}";
	   		form.submit();
	   	} else {
		   	if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "agentmaintenance.do?method=saveTemplate&type=${type}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "agentmaintenance.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("${eoms:a2u('保存模板成功！')}");
			 		}
			    });
...
<div id="sheetform">
  <html:form action="/agentmaintenance.do?method=performDeal" styleId="theform">
   <input type="hidden" name="operateType" value="${operateType}"/>
	<table>
	  ...
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'ByRejectHumTask'&& taskName != 'DraftHumTask'&& taskName != 'TaskCreateAuditHumTask'&& taskName != 'cc'}">
		...
		<input type="hidden" name="dictKey" value="dict-sheet-agentmaintenance"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/agentmaintenance/agentmaintenance.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftHumTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType1();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/agentmaintenance/agentmaintenance.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/agentmaintenance/agentmaintenance.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
    ...
    <c:when test="${taskName=='ByRejectHumTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType2();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/agentmaintenance/agentmaintenance.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/agentmaintenance/agentmaintenance.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	   </div>	 
   	</c:when>
    <c:otherwise>  
	    <!-- buttons -->
