...
<display:table name="taskList" cellspacing="0" cellpadding="0"
    ...
    <c:if test="${!empty taskList && taskList.mainSleepStatus == '1' }">
        <display:column sortable="true" headerClass="sortable" title="工单流水号">
            <a href='widencomplaint.do?method=showDetailPage&sheetKey=${taskList.id }&u=${taskList.mainT1Dealer }&a=${taskList.mainSleepUser }&taskStatus=4'
    ...
    <c:if test="${!empty taskList && taskList.mainSleepStatus == '2' }">
        <display:column sortable="true" headerClass="sortable" title="工单流水号">
            <a href='widencomplaint.do?method=showDetailPage&sheetKey=${taskList.id }&u=${taskList.mainT1Dealer }&a=${taskList.mainSleepUser }&taskStatus=6'
               target=_blank>${taskList.sheetId }</a>
        </display:column>
