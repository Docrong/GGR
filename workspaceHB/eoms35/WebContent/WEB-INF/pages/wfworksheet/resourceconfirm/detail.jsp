...
<br>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=${orderSheetId}&specialtyType=${sheetMain.mainSpecifyType}&sheetType=resourceConfirm&taskName=${taskName}&taskStatus=${taskStatus}&addr=<%=addr%>" width="100%" height="300px"></iframe>
...
<div class="sheet-deal">
	<div class='sheet-deal-header'>
		...
		<%}else if(taskName.equals("DraftTask")){%>	
			<select id="dealSelector">
			  <option value="${urlShowDraftDeal}"><bean:message bundle="resourceconfirm" key="operateType.draft"/></option>
			</select>
		...
		<%}else if(taskName.equals("HoldTask")){ %>
			<select id="dealSelector">
			  <option value="${urlShowHoldDealPage}"><bean:message bundle="resourceconfirm" key="operateType.hold"/></option>
			</select>		
		<%} %>
		</td>
		</tr>
