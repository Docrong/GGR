    ...
    function check() {
        ...
        if (templateId == "") {
            ...
            if (taskName != null && taskName == "draft") {
                window.opener.location.href = "${app}/sheet/safeaudit/safeaudit.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId=" + templateId;
            } else {
                window.opener.location.href = "${app}/sheet/safeaudit/safeaudit.do?method=referenceTemplate&templateId=" + templateId;
            }
...
<bean:define id="url" value="safeaudit.do"/>
<display:table name="taskList" cellspacing="0" cellpadding="0"
               id="taskList" pagesize="${pageSize}" class="table resourceAssessTable"
               export="false" requestURI="safeaudit.do"
...
>
    ...
    <display:column sortable="true" headerClass="sortable" title="模板名称">
        <a href="${app}/sheet/safeaudit/safeaudit.do?method=editTemplateInfo&templateId=${taskList.id}&type=${templateManage}">
