package com.boco.eoms.im.adaptor.facade;
...
public interface IMAdaptorFacade {
    /**
     * 指定用户发送消息(无消息内容修饰消息构建方法)
     *
     * @param user     用户id
     * @param passwd   密码
     * @param toOrgIds 发送的组织ID,如userId#1,deptId#2,roleId#3 （其中1代表用户 2代表部门 3代表角色）
     * @param body     消息内容
     ...
     * @param toOrgIds      发送的组织ID,如userId#1,deptId#2,roleId#3 （其中1代表用户 2代表部门 3代表角色）
     * @param body          消息内容
     * @param imMsgListener 消息监听，提供在消息发送中插入代码
     * @return 消息唯一标识，供取消发送消息
     */
    public String sendMsg(String user, String passwd, String toOrgIds,
     ...
     * @param id 消息id唯一标识
     * @return 删除成功否
     * @throws CancelMsgErrorException 取消发送消息失败
     */
     ...
     * 使用系统用户发送文件
     *
     * @param filePath 文件路径，提供所在服务器的文件路径
     * @param toOrgIds 发送的组织ID,如userId#1,deptId#2,roleId#3 （其中1代表用户 2代表部门 3代表角色）
     * @throws IMAdaptorSendFileErrorException 发送错误则抛出异常
     */
     ...
     * @param filePath       文件路径
     * @param toOrgIds       发送的组织ID,如userId#1,deptId#2,roleId#3 （其中1代表用户 2代表部门 3代表角色）
     * @param imFileListener 文件监听，插入代码
     * @throws IMAdaptorSendFileErrorException 发送错误则抛出异常
     */
    public void sendFile(String filePath, String toOrgIds,
