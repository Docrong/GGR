...
%>
<form id==
"form" name = "form" method="post" action="save.do">
<input type="hidden" name="roomName" value="<%=roomName%>">
<table border="0" cellspacing="1" cellpadding="1" width="684" height="19" class="table_show" align="center"
       bgColor="#f3f3f3">
    <tr class="tr_show">
    ...
    %>
    <tr class="tr_show">
        <td class="clsfth" width="59" height="30" colspan="4"><%=roomName%>
        </td>
    </tr>
    <tr class="tr_show">
        ...
        <td class="clsfth" width="66" height="30">
            ...
            <input type="button" Class="clsbtn2" name="load" value="导入系统设置"
                   onclick="javascript:fun_load('<%=roomId%>')">&nbsp;&nbsp;&nbsp;&nbsp;
            <%}%>
        </td>
    </tr>
    <%
    ...
    <tr class="tr_show">
        <%if (i == 0) {%>
        <td class="clsfth" width="59" height="30" rowspan="<%=vecDefinetable.size()%>"><%=roomName%>
        </td>
        <%}%>
        <td class="clsfth" width="214" height="30">
            <p align="center"><a
                    href="../TawRmDefinemain/show.do?roomId=<%=roomId%>&tableId=<%=tawRmDefineTable.getTableId()%>&tableDesc=<%=tawRmDefineTable.getTableDesc()%>&roomName=<%=roomName%>"><%=tawRmDefineTable.getTableDesc()%>
            </a></td>
        <td class="clsfth" width="48" height="30">
            <input type="button" Class="clsbtn2" name="load" value="修改"
                   onclick="javascript:fun_update('<%=roomId%>','<%=String.valueOf(tawRmDefineTable.getTableId()).trim()%>','<%=tawRmDefineTable.getTableDesc().trim()%>');">&nbsp;
        </td>
        <td class="clsfth" width="52" height="30">
            <input type="button" Class="clsbtn2" name="load" value="删除"
                   onclick="javascript:fun_delete('<%=roomId%>','<%=String.valueOf(tawRmDefineTable.getTableId()).trim()%>');">&nbsp;
        </td>
    </tr>
    <%
        }
    ...
    <tr class="tr_show">
        ...
        <td class="clsfth" width="66" height="30" colspan="2">
            <input type="button" Class="clsbtn2" name="load" value="预览"
                   onclick="javascript:fun_preview('<%=roomId%>');">&nbsp;
        </td>
    </tr>
    <%
        }
