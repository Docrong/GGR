        ...
        <td>
            <table border="0" width="100%" cellspacing="1" cellpadding="1" class="listTable" align=center>
                ...
                <tr class="tr_show">
                    ...
                    <td><%=String.valueOf(getHander.elementAt(i))%>
                    </td>
                    <td><%=String.valueOf(getWorkserial1.elementAt(i))%>
                    </td>
                    <td><%=String.valueOf(getReceiver.elementAt(i))%>
                    </td>
                    <td><%=String.valueOf(getWorkserial2.elementAt(i))%>
                    </td>
                    <td><%=String.valueOf(getReason.elementAt(i))%>
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <input type="button" name="button" class="button" value=
                            <bean:message key="TawRmChangeduty.agree"/> Onclick="window.location.href ='${app}
                        /kmmanager/Kmchangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>
                        &roomId=<%=roomId %>&transact_flag=4'">
                        &nbsp;
                        <input type="button" name="button" class="button" value=
                            <bean:message key="TawRmChangeduty.disagree"/> Onclick="window.location.href ='${app}
                        /kmmanager/Kmchangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>
                        &roomId=<%=roomId %>&transact_flag=5'">
                        <%} else {%>
                        <input type="button" name="button" class="button" value="${eoms:a2u('删除记录')}"
                               Onclick="window.location.href ='${app}/kmmanager/Kmchangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>&roomId=<%=roomId %>&transact_flag=3'">
                        <%}%>
                    </td>
                </tr>
                <%
