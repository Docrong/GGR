     ...
     <table id="sheet" class="formTable">
     				<tr>
						       ...
						       <td class="content">
						                <input class="text" onclick="popUpCalendar(this, this)" type="text" 
						                   ...
						                   readonly="readonly" value="${eoms:date2String(sheetMain.sheetAcceptLimit)}" alt="vtype:'lessThen',link:'${sheetPageName}sheetCompleteLimit',vtext:'受理时限不能晚于完成时限',allowBlank:false"/>
				   ...
				   <tr>    	
			              ...
			              <td colspan="3"> 	
	                            ...
	                            <textarea class="textarea max" readonly="true" name="mainAssortSpeciality2" style="height:50px" id="mainAssortSpeciality2"  alt="allowBlank:true"><eoms:invert appCode="netdata" sheetKey="${sheetMain.mainAssortSpeciality}" beanId="CacheBean" scope="page"/></textarea>
				    ...
				    <tr>    	<!-- 是否涉及安全 -->	                
			                ...
			                <td>  
					        	<eoms:comboBox name="${sheetPageName}mainIsSecurity" id="${sheetPageName}mainIsSecurity" 
	            	      			initDicId="10301"  alt="allowBlank:false" styleClass="select-class" defaultValue="${sheetMain.mainIsSecurity}"/>
				        	</td>	  
			  	...
			  	<c:if test="${!empty parentSheetId}">
					<input type="text"  class="text" name="${sheetPageName}mainParentSheetId" id="${sheetPageName}mainParentSheetId"  alt="allowBlank:true,maxLength:32,vtext:'请填入相关工单号信息，最多输入32字符'" value="${parentSheetId}" readonly/>
			  	</c:if>
			  	<c:if test="${empty parentSheetId}">
					<input type="text"  class="text" name="${sheetPageName}mainParentSheetId" id="${sheetPageName}mainParentSheetId"  alt="allowBlank:true,maxLength:32,vtext:'请填入相关工单号信息，最多输入32字符'" value="${sheetMain.mainParentSheetId}"/>
			  	</c:if>
		                 ...
		                 <td>  
				        	<eoms:comboBox name="${sheetPageName}mainIsNeedDesign" id="${sheetPageName}mainIsNeedDesign" 
            	      		initDicId="10301"  alt="allowBlank:false" styleClass="select-class" defaultValue="${sheetMain.mainIsNeedDesign}" onchange="ifDoProjectDeal(this.value)"/>
			       ...
			       <tr>	
			            ...
			            <td  class="label"><bean:message bundle="netdata" key="netdata.mainCellInfo"/>*</td>
			              <td colspan="3"> 	
	    				  	<input type="text" class="text" name="${sheetPageName}mainCellInfo" id="${sheetPageName}mainCellInfo" alt="allowBlank:false,maxLength:32,vtext:'请填入变更网元，最多输入32字符'" value="${sheetMain.mainCellInfo}"/>
	                    </td>
		           </tr>
		           <tr>
		           		<!-- 网络数据变更需求说明 -->
			            <td  class="label"><bean:message bundle="netdata" key="netdata.mainNetDataCommont"/>*</td>
			              <td colspan="3"> 	
	    				  	<textarea class="textarea max" name="${sheetPageName}mainNetDataCommont" id="${sheetPageName}mainNetDataCommont" alt="allowBlank:false,maxLength:4000,vtext:'请填入网络数据变更需求说明，最多输入4000字符'">${sheetMain.mainNetDataCommont}</textarea>
	                    </td>
		           </tr>
	</table>
	<br/>
	      ...
	      <tr>
		   ...
		   <tr>
		    ...
		    <eoms:attachment name="sheetMain" property="sheetAccessories" 
		            scope="request" idField="${sheetPageName}sheetAccessories" appCode="netdata" alt="allowBlank:true"/> 				
		    </td>
		  </tr>			  
	</table>
	<br>
		...
		<!-- 方案制定  -->
			      <tr>
		            ...
		            <td> 
						...
						id="${sheetPageName}linkCompleteLimitTime" value="${eoms:date2String(sheetMain.linkCompleteLimitTime)}" 
		            ...
		            <!-- 技术方案关键字 -->
		             ...
		             <td> <input type="text"  class="text" name="${sheetPageName}linkDesignKey" id="${sheetPageName}linkDesignKey" value="${sheetMain.linkDesignKey}"  alt="allowBlank:false,maxLength:32,vtext:'请填入技术方案关键字，最多输入32字符'"/></td>
		          </tr>
		          <tr>
			      	<!-- 方案资源号 -->
			      ...
			      <tr>
		            ...
		            <td  class="label"><bean:message bundle="netdata" key="netdata.linkDesignComment"/></td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkDesignComment" id="${sheetPageName}linkDesignComment" alt="allowBlank:true,maxLength:4000,vtext:'请填入技术方案说明，最多输入4000字符'">${sheetMain.linkDesignComment}</textarea>
                    </td>
		          </tr>
			      <tr>
			      	<!-- 变更涉及省份 -->
		            ...
		            涉及省份及地市*</td>
    				  ...
    				  <br/><br/>	
    				  涉及省份:<textarea class="textarea max" readonly="true" name="linkInvolvedProvince" style="height:50px" id="${sheetPageName}linkInvolvedProvince"  alt="allowBlank:false,maxLength:4000,vtext:'请填入涉及省份，最多输入4000字符'">${sheetMain.linkInvolvedProvince }</textarea><br/>
    				  涉及地市:<textarea class="textarea max" readonly="true" name="linkInvolvedCity" style="height:50px" id="${sheetPageName}linkInvolvedCity" alt="allowBlank:true,maxLength:4000,vtext:'请填入涉及地市，最多输入4000字符'">${sheetMain.linkInvolvedCity}</textarea>
...
function callback(jsonData,str){
		...
		return true;
			      ...
			      <tr>
		            ...
		            <td  class="label"><bean:message bundle="netdata" key="netdata.linkRiskEstimate"/>*</td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkRiskEstimate" id="${sheetPageName}linkRiskEstimate" alt="allowBlank:false,maxLength:4000,vtext:'请填入风险评估，最多输入4000字符'">${sheetMain.linkRiskEstimate}</textarea>
                    </td>
		          </tr>
			      <tr>
			      	<!-- 影响业务分析 -->
		            <td  class="label"><bean:message bundle="netdata" key="netdata.linkEffectAnalyse"/>*</td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkEffectAnalyse" id="${sheetPageName}linkEffectAnalyse" alt="allowBlank:false,maxLength:4000,vtext:'请填入影响业务分析，最多输入4000字符'">${sheetMain.linkEffectAnalyse}</textarea>
                    </td>
		          </tr>
		          <tr>
					    <td class="label">
	...
	<div id="test1">
	  ...
	  <div class="x-form-item">
		   <eoms:chooser id="test11" type="role" roleId="245" flowId="78" config="{returnJSON:true,showLeader:true,mainPanelTitle:'可选择的子角色'}"
	...
	<fieldset id="link3">
	  ...
	  <div class="x-form-item">  
			<eoms:chooser id="test33" type="role" roleId="247" flowId="78" config="{returnJSON:true,showLeader:true,mainPanelTitle:'可选择的子角色'}"
