                    ...
                    <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61' && taskName !='rejectTask'}">
                        <c:if test="${taskName != 'draftTask'}">
                            <html:button styleClass="btn" property="method.save" styleId="method.save"
                                         onclick="window.open('./netOptimize.do?method=getDealTemplatesByUserId&sheetKey=${sheetMain.id}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskStatus=${taskStatus}&operateType=${operateType}',null,'left=300,top=150,height=400,width=500,scrollbars=yes')">
            ...
            <c:when test="${operateType=='61'}">
                <div class="form-btns" id="btns">
                    <input type="submit" class="submit"
                           onclick="this.form.action='${app}/sheet/netOptimize/netOptimize.do?method=performClaimTask'"
            ...
            <c:otherwise>
                ...
                <div class="form-btns hide" id="btns">
                    ...
                    <c:if test="${taskName!='cc'}">
                        <input type="submit" class="submit"
                               onclick="saveAlert();v.passing=true;this.form.action='${app}/sheet/netOptimize/netOptimize.do?method=performSaveInfo'"
                               value="<bean:message bundle='sheet' key='button.save'/>"/>
                        <input type="submit" class="submit"
                               onclick="saveAlert();v.passing=true;this.form.action='${app}/sheet/netOptimize/netOptimize.do?method=performSaveInfoAndExit'"
                               value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    </c:if>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/netOptimize/netOptimize.do?method=showListsendundo'"
