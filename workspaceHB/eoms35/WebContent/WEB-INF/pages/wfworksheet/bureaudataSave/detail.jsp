...
<%@page import="com.boco.eoms.sheet.bureaudataSave.model.BureaudataSaveTask" %>
...
BureaudataSaveTask task = new BureaudataSaveTask();
String operaterType = "";
String ifsub = "";
String ifwaitfor="";
if(request.getAttribute("task")!=null){
 task = (BureaudataSaveTask)request.getAttribute("task");
...
Ext.onReady(function() {
	var tabConfig = {
		items : [{
			...
			url : 'bureaudataSave.do?method=showSheetDealList&orderByDetp=true&sheetKey=${sheetMain.id}&taskName=${taskName}&ifWaitForSubTask=${task.ifWaitForSubTask}'
		}, {
			text : '<bean:message bundle="sheet" key="sheet.flowchar"/>',
			url : 'bureaudataSave.do?method=showWorkFlow&linkServiceName=iBureaudataSaveLinkManager&dictSheetName=dict-sheet-bureaudataSave&description=mainOperateType&sheetKey=${sheetMain.id}',
			isIframe : true
		},{
			text : '<bean:message bundle="sheet" key="sheet.filesView"/>',
			url : 'bureaudataSave.do?method=showSheetAccessoriesList&id=${sheetMain.id}',
			isIframe : true
		}, {
	...
	var url2="bureaudataSave.do?method=newShowCCPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=-10&taskStatus=${taskStatus}";
	eoms.util.appendPage("sheet-deal-content",url2);	
	<%}%>
	<%if(taskName.equals("reply")){ %>
	var url2="bureaudataSave.do?method=showRemarkPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}";
	eoms.util.appendPage("sheet-deal-content",url2);	
	<%}%>
...
function forceOperation(obj){
	...
	if(obj == 1){
		...
		var url2="bureaudataSave.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=forceHold";
		eoms.util.appendPage("sheet-deal-content",url2);
		
	}else if(obj == 2){
	     
		var url2="bureaudataSave.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=nullity";
		eoms.util.appendPage("sheet-deal-content",url2);
	}else{
	    
	    var url2="bureaudataSave.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=forceNullity";
		eoms.util.appendPage("sheet-deal-content",url2);
    }
   }
    function eventOperation(obj){
	if(obj == 1){
	     var url2="bureaudataSave.do?method=showPhaseAdvicePage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&taskName=advice&operateFlag=driveForward&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}";
...
<div id="sheet-detail-page">
  ...
  <div id="sheetinfo">
	...
	<%@ include file="/WEB-INF/pages/wfworksheet/bureaudataSave/basedetailnew.jsp"%>
	<br/>
...
<!-- 号段审核 -->
<c:url var="urlShowAssessorDealPage" value="bureaudataSave.do">
...
<!-- 草稿派发 -->
<c:url var="urlShowDraftDeal" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 重新派发 -->
<c:url var="urlShowBackDeal" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 归档 -->
<c:url var="urlShowHoldDealPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 退回 -->
<c:url var="urlShowRejectBackDealPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 转派 -->
<c:url var="urlShowTransferkPage" value="bureaudataSave.do">
...
<!-- 转审 -->
<c:url var="urlShowTransferAuditPage" value="bureaudataSave.do">
  <c:param name="method" value="showTransferWorkItemPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 阶段回复 -->
<c:url var="urlShowPhaseReplyPage" value="bureaudataSave.do">
...
<!-- 驳回 -->
<c:url var="urlShowRejectBackPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 确认受理 -->
<c:url var="urlShowAcceptDealPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 分派 -->
<c:url var="urlShowInputSplit" value="bureaudataSave.do">
  <c:param name="method" value="showInputSplit"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
  <c:param name="taskName" value="${taskName}"/>
  <c:param name="subtaskName" value="BureaudataSaveSubTask"/>
  <c:param name="operateRoleId" value="${operateRoleId}"/>
  <c:param name="TKID" value="${TKID}"/>
  <c:param name="preLinkId" value="${preLinkId}"/> 
...
<!-- 会审 -->
<c:url var="urlShowInputSplitAudit" value="bureaudataSave.do">
  <c:param name="method" value="showInputSplit"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 分派回复 -->
<c:url var="urlShowDispatchPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 会审回复 -->
<c:url var="urlShowDispatchAuditPage" value="bureaudataSave.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 处理回复通过 -->
<c:url var="urlShowDealReplyAcceptPage" value="bureaudataSave.do">
...
<!-- 处理回复不通过 -->
<c:url var="urlShowDealReplyRejectPage" value="bureaudataSave.do">
...
<!--				<option value="${urlShowInputSplit}"><bean:message bundle="sheet" key="common.split"/></option>		  -->
		...
		<%}else if(taskName.equals("DraftTask")){%>	
			<select id="dealSelector">
			  <option value="${urlShowDraftDeal}"><bean:message bundle="bureaudataSave" key="operateType.draft"/></option>
			</select>
		...
		<%}else if(taskName.equals("HoldTask")){ %>
			<select id="dealSelector">
			  <option value="${urlShowHoldDealPage}"><bean:message bundle="bureaudataSave" key="operateType.hold"/></option>
			</select>		
		<%} %>
		</td>
		</tr>
...
<script type="text/javascript">
	...
	try{
        ...
        if (dealTemplateId != null && dealTemplateId != "") {
   			url = "bureaudataSave.do?method=referenceTemplate&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+dealTemplateId
        }
