...
<%@ page import="com.boco.eoms.wfworksheet.stopstation.vo.StopstationLinkVO,
                 ...
                 java.util.List,
                 java.lang.Object,
                 com.boco.eoms.wfengine.task.dao.TaskDAO,
...
<table border="0" width="95%" cellspacing="1" cellpadding="1" class="table_show" align=center>
    ...
    <tr>
        <td align="right" colspan="9" bgcolor="#aec2e8">
            ...
            <!--  -->
            <font color="#FFFFFF">每页</font><font color="#FFFFFF"><%=intPageSize%>行 共</font><font
                color="#FFFFFF"><%=intRowCount%>行
    ...
    <tr bgcolor="#EEECED">
        <td align="right" bgcolor="#aec2e8" colspan="9">
            ...
            %>
            <A HREF="checkdetail.do?page=<%=intPageCount%>&starttime=<%=starttime%>&endtime=<%=endtime%>&roomId=<%=roomId%>">尾页</A>
            转到第
            <SELECT name="jumpPage" onchange="location.href=this.options[this.selectedIndex].value" size="1">
                <% for (int l = 1; l <= intPageCount; l++) {
                    if (l != intPage) {
                %>
                <OPTION value="checkdetail.do?page=<%=l%>&starttime=<%=starttime%>&endtime=<%=endtime%>&roomId=<%=roomId%>"><%=l%>
                </OPTION>

                <%} else {%>
                <OPTION selected <%=intPage%>><%=intPage%>
                </OPTION>
                <%
                        }
                    }
    ...
    <tr class="tr_show">
        <td width="14%" class="clsfth">
            <div align="center">班次</div>
        </td>
        <td width="20%" class="clsfth">
            <div align="center">工单归档处理开始时限</div>
        </td>
        <td width="21%" class="clsfth">
            <div align="center">工单归档处理结束时限</div>
        </td>
        <td width="15%" class="clsfth">
            <div align="center">班长</div>
        </td>
        <td width="30%" class="clsfth">
            <div align="center">未归档工单考核</div>
        </td>
    </tr>
    <%
    ...
    <tr>
        <td colspan="5" bgcolor="#FFFFCC">
            <div align="center"><font color="#cc00000">考核日期：<%=dateList.get(n).toString()%>
            </font></div>
    </tr>
    ...
    <tr class="tr_show">
        ...
        <td>
            <div align="center"><%=new String(obj.get(1).toString().getBytes("819"), "gb2312")%>
            </div>
        </td>


        <td>
            <div align="center"><%=new String(obj.get(3).toString().getBytes("819"), "gb2312").substring(0, 19)%>
            </div>
        </td>
        <td>
            <div align="center"><%=new String(obj.get(4).toString().getBytes("819"), "gb2312").substring(0, 19)%>
            </div>
        </td>


        <td>
            <div align="center"><%=new String(obj.get(2).toString().getBytes("819"), "gb2312")%>
            </div>
        </td>
        ...
        <td>
            <div align="center"><a
                    href="checklist.do?dutyId=<%=new String(obj.get(1).toString().getBytes("819"),"gb2312")%>&starttime=<%=starttime%>&endtime=<%=endtime%>&roomId=<%=roomId%>"><%=new String(obj.get(5).toString().getBytes("819"), "gb2312")%>
            </a></div>
        </td>
    </tr>
    <%
    ...
    <tr>
        <td colspan="5" bgcolor="#FFFFCC">
            <div align="center">你选择的查询条件没有数据</div>
    </tr>
    <%}%>
    <tr bgcolor="#EEECED">
