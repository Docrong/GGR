                ...
                <tr class="tr_show">
                    ...
                    <td>
                        ...
                        <input type="button" name="button" class="button" value=
                            <bean:message key="TawRmChangeduty.agree"/> Onclick="window.location.href ='
                        ../TawRmChangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>
                        &roomId=<%=roomId %>&transact_flag=4'">
                        &nbsp;
                        <input type="button" name="button" class="button" value=
                            <bean:message key="TawRmChangeduty.disagree"/> Onclick="window.location.href ='
                        ../TawRmChangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>
                        &roomId=<%=roomId %>&transact_flag=5'">
                        <%} else {%>
                        <input type="button" name="button" class="button" value="${eoms:a2u('删除记录')}"
                               Onclick="window.location.href ='../TawRmChangeduty/transact_audit.do?transact_id=<%=Integer.parseInt(String.valueOf(getId.elementAt(i)).trim())%>&roomId=<%=roomId %>&transact_flag=3'">
                        <%}%>
                    </td>
                </tr>
                <%
