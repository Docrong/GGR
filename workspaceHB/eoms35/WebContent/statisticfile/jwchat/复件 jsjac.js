...
JSJaCPacket.prototype.errorReply = function (stanza_error) {
    ...
    rPacket.appendNode('error', {code: stanza_error.code, type: stanza_error.type}, [[stanza_error.cond]]);
    return rPacket;
};
...
JSJaCIQ.prototype.reply = function (payload) {
    ...
    if (payload) {
        if (typeof payload == 'string')
            rIQ.getChild().appendChild(rIQ.getDoc().loadXML(payload)); else if (payload.constructor == Array) {
...
JSJaCConnection.prototype._doInBandReg = function () {
    ...
    iq.setType('set');
    iq.setID('reg1');
    iq.appendNode("query", {xmlns: "jabber:iq:register"}, [["username", this.username], ["password", this.pass]]);
...
JSJaCConnection.prototype._doLegacyAuth = function () {
    ...
    iq.appendNode('query', {xmlns: 'jabber:iq:auth'}, [['username', this.username]]);
...
JSJaCConnection.prototype._doLegacyAuth2 = function (iq) {
    ...
    query = iq.appendNode('query', {xmlns: 'jabber:iq:auth'}, [['username', this.username], ['resource', this.resource]]);
    if (use_digest) {
        query.appendChild(iq.buildNode('digest', {xmlns: 'jabber:iq:auth'}, hex_sha1(this.streamid + this.pass)));
    } else if (this.allow_plain) {
        query.appendChild(iq.buildNode('password', {xmlns: 'jabber:iq:auth'}, this.pass));
    } else {
...
JSJaCConnection.prototype._doSASLAuthDigestMd5S1 = function (el) {
    if (el.nodeName != "challenge") {
        ...
        this._sendRaw("<response xmlns='urn:ietf:params:xml:ns:xmpp-sasl'>" +
            binb2b64(str2binb(rPlain)) + "</response>", this._doSASLAuthDigestMd5S2);
    }
};
...
JSJaCConnection.prototype._doStreamBind = function () {
    ...
    iq.appendNode("bind", {xmlns: "urn:ietf:params:xml:ns:xmpp-bind"}, [["resource", this.resource]]);
...
JSJaCConnection.prototype._parseStreamFeatures = function (doc) {
    ...
    if (doc.getElementsByTagNameNS)
        errorTag = doc.getElementsByTagNameNS("http://etherx.jabber.org/streams", "error").item(0); else {
    ...
    this.has_sasl = false;
    if (this.has_sasl)
        this.oDbg.log("SASL detected", 2); else {
        this.authtype = 'nonsasl';
...
JSJaCHttpBindingConnection.prototype._getRequestString = function (raw, last) {
    ...
    if (this._rid <= this._last_rid && typeof (this._last_requests[this._rid]) != 'undefined')
        reqstr = this._last_requests[this._rid].xml; else {
        var xml = '';
        ...
        if (last)
            reqstr += "type='terminate'"; else if (this._reinit) {
...
var JSJaC = {
    Version: '$Rev: 456 $', require: function (libraryName) {
