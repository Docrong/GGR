select userid, deptid, password
from taw_system_user select *
from commonfault_main

select *
from commonfault_link select *
...
case when task.taskstatus = 2 then 0::integer when task.taskstatus=8 then 1:: integer end est_data_status,
    srole.roleid send_bigrole_id,
    case when task.operatetype=
'subrole' then orole.roleid else null :: integer end operate_bigrole_id,
    sd.linkid send_dept_level,
    od.linkid operate_dept_level,
--common main
    main.id MAINID, main.SHEETID, main.TITLE, main.SHEETACCEPTLIMIT, main.SHEETCOMPLETELIMIT, main.SENDTIME, main.SENDUSERID, main.SENDDEPTID,
...
from commonfault_main main
    join commonfault_task task
on main.templateflag=0 and main.id=task.sheetkey and task.taskstatus!=5
...
select case when to_date('2008-09-16 06:41:53', 'YYYY-MM-DD HH24:MI:SS') - sysdate > 0 then 1 else 0 end
from dual select *
from common

create view v_test
select TOORGROLEID, ACCEPTFLAG, ACCEPTTIME
from commonfault_link TOORGROLEID varchar(255) Y
    ACCEPTFLAG integer Y
    ACCEPTTIME TIMESTAMP (6)

select 1
from dual select null:: varchar (255)
from dual

select null
from dual select null::datetime year to second
from dual

create table test1 as
select null a
from dual drop table test1


------------------------------VIEW----------------------------------------
select *
from v_commomfault select distinct *
from v_commomfault
...
--common task
                          ...
                          taskoperatetype
--bussines main
    , MAINAPPLYSHEETID, MAINIFURGENTFAULT, MAINURGENTFAULTLOG, MAINIFSAFE, MAINIFMUTUALCOMMUNICATION,
                          MAINFAULTRESPONSELEVEL, MAINALARMNUM, MAINALARMSTATE, MAINALARMSOLVEDATE, MAINNETSORTONE,
                          MAINEQUIPMENTFACTORY, MAINEQUIPMENTNAME, MAINEQUIPMENTMODEL, MAINFAULTGENERANTTIME,
                          MAINFAULTGENERANTPRIV, MAINIFAFFECTOPERATION, MAINAFFECTSTARTTIME, MAINFAULTDISCOVERABLEMODE,
                          MAINSENDMODE, MAINALARMID, MAINALARMSOURCE, MAINALARMLOGICSORT, MAINALARMLOGICSORTSUB,
                          MAINFAULTSPECIALTY, MAINEQUIPMENTTYPE, MAINNETNAME, MAINCOMPLETELIMITT1, MAINCOMPLETELIMITT2,
                          MAINCOMPLETELIMITT3, SENDORGTYPE, MAINFAULTGENERANTCITY, MAINNETSORTTWO, MAINNETSORTTHREE,
                          MAINREJECTCOUNT, MAINDELAYFLAG, SENDYEAR, SENDMONTH, SENDDAY, MAINIFRECORD
--bussines link
    , LINKFAULTRESPONSELEVEL, LINKIFMUTUALCOMMUNICATION, LINKIFSAFE, LINKIFEXCUTENETCHANGE, LINKFAULTAVOIDTIME,
                          LINKOPERRENEWTIME, LINKAFFECTTIMELENGTH, LINKIFGREATFAULT, LINKIFURGENTFAULT,
                          LINKFAULTREASONSORT, LINKFAULTREASONSUBSECTION, LINKIFFINALLYSOLVEPROJECT,
                          LINKIFADDCASEDATABASE, LINKIFDEFERRESOLVE, LINKFAULTDEALINFO, OPERATEYEAR, OPERATEMONTH,
                          OPERATEDAY
    ) as
    (
--已处理
...
values (51, '2007-07-07 00:00:00', '故障工单基础表采集的上一次操作的时间');

drop table est_last_oper
delete
from est_last_oper
select *
from est_last_oper SELECT last_operate_time
FROM est_last_oper
WHERE
    id =51

select link.operatetime
from commonfault_link link

SELECT count(distinct mainid)
FROM commonfault_link
HAVING max(operatetime) >= to_date('2007-07-07 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
   and max(operatetime) < sysdate

select mainid, operatetime
from commonfault_link delete from est_commonfault
select *
from est_commonfault drop table est_commonfault
