...
public class InterSwitchAlarm {
     ...
     * 告警接收 E-OMS系统提供该接口服务用于接收需要自动派发故障工单的网管告警信息
     *
     * @param alarmId         string(80) 网管告警流水号 M 网管系统简称_告警ID
     * @param alarmStaId      string(80) 网管告警ID M /
     * @param oriAlarmId      string(40) 原始告警号 M,N /
     * @param alarmTitle      string(400) 告警标题 M /
     * @param alarmCreateTime string(20) 告警产生时间 M /
     * @param neType          string(20) 网元类型 M /
     * @param neName          string(32) 网元名称 M /
     * @param alarmLevel      string(20) 告警级别 M 梳理后网管定义的告警级别（四级）
     * @return result
     * 输出为一个字符串，格式为“sheetNo=工单号;errList=错误描述”。如果派单成功：“工单号”不为空串，“错误描述”为空串；如果派单失败，“工单号”为空串，“错误列表”为错误描述。约定“sheetNo=”和“errList=”不管派单成功与否都必须有，只是值可以为空串。
     * @throws 执行报错RuleToolXMLException
     * @paramserSupplier string(16) 服务提供方 M 参见附录A.3
...
//					"FieldContent");
            ...
            if (alarmList.size() == 1) {
                ...
                if (mainAlarmNum != null && mainAlarmNum.length >= 1)
                    alarmFisrtMap.put("mainAlarmNum", mainAlarmNum[0]);
     ...
     * @param alarmId     string(80) 网管告警ID M 网管系统简称_告警ID
     * @param alarmStatus Integer(2) 告警状态 M 1：自动清除；2：手工清除；
     * @param staffNo     string(32) 手工清除告警人员的工号 M,N /
