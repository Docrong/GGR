...
function check() {
	...
	if (templateId == "") {
		...
		if (taskName != null && taskName == "draft") {
			window.opener.location.href = "${app}/sheet/businessplan/businessplan.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+templateId ;
		} else {
			window.opener.location.href = "${app}/sheet/businessplan/businessplan.do?method=referenceTemplate&templateId="+templateId ;
		}
...
<bean:define id="url" value="businessplan.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		id="taskList" pagesize="${pageSize}" class="table resourceAssessTable"
		export="false" requestURI="businessplan.do" 
		...
		<display:column sortable="true" headerClass="sortable" title="${eoms:a2u('模板名称')}">
			<a href="${app}/sheet/businessplan/businessplan.do?method=editTemplateInfo&templateId=${taskList.id}&type=${templateManage}">
				${taskList.sheetTemplateName} 
			</a>
		</display:column>
		<display:column sortable="true" headerClass="sortable" title="${eoms:a2u('新业务分类')}">
			 <eoms:id2nameDB id="${taskList.mainProductType}" beanId="ItawSystemDictTypeDao"/>
		</display:column>
	</display:table>
