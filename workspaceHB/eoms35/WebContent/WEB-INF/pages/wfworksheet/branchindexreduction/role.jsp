...
Ext.onReady(function(){
	...
	deptTree = new xbox({
		...
		treeDataUrl:areaTreeAction,treeRootId:'-1',treeRootText:'${eoms:a2u("地域")}',treeChkMode:'',treeChkType:'area',
	...
	roleUserTree = new xbox({
		...
		treeDataUrl:userTreeAction,treeRootId:'-1',treeRootText:'${eoms:a2u("用户")}',treeChkMode:'',treeChkType:'user',viewer:roleUserViewer,
	...
	subRoleUserTree = new xbox({
		...
		treeDataUrl:userTreeAction,treeRootId:'-1',treeRootText:'${eoms:a2u("用户")}',treeChkMode:'',treeChkType:'user',viewer:userViewer,
		callback:function(json,ids){
			Ext.Ajax.request({    		
	    		method:"post",
	    		url:gridAction_saveUsers,
...
<div id="rolePanel" class="x-layout-inactive-content panel">
	...
	<div id="rolePanel-users" class="viewer-box">
		<div id="rolePanel-users-title" class="viewlistitem-role" style="display:inline;padding-top:0"></div> ${eoms:a2u('的人员列表:')}
...
<div id="gridPanel" class="x-layout-inactive-content panel">
	...
	<div id="subRoleUsers" class="viewer-box">
	  ...
	  <div id="user-subrolename" class="viewlistitem-role" style="display:inline;padding-top:0"></div> ${eoms:a2u('的人员列表:')}
	  <div id="user-list" class="viewer-list" style="margin:15px"></div>
	  <input type="button" value="${eoms:a2u('更改人员列表')}" id="userTreeBtn" class="btn">  
	  <input type="button" value="${eoms:a2u('设置组长')}" id="setLeaderBtn" class="btn" onclick="javascript:setTeamLeader();"> 
	</div>
</div>
	...
	<table width="100%" cellspacing="10" border="0">
		<tr>
			...
			<div id="distinction0" style="margin:10px 0">
				<input type="button" name="depttree" id="depttree" value="${eoms:a2u('选择地域')}" class="btn"/><br/>
				<textarea class="textarea" readonly="readonly" name="deptname" id="deptname"></textarea>					
				<input type="hidden" name="deptid" id="deptid"/>			
			</div>
			
			<div id="workflow-filters"></div>
				<div id="distinctionType" style="margin:10px 0">
				<label>${eoms:a2u('核减专业')}:</label><br/>	
				<div>
					<eoms:comboBox name ="class1" id ="subRole" sub="reasonType"  initDicId ="1013201" styleClass="select"/>
				</div>
				<div id="distinctionType" style="margin:10px 0">
				<label>${eoms:a2u('核减理由类别')}:</label><br/>	
				<div>
					<eoms:comboBox name ="class2" id ="reasonType"  initDicId ="" styleClass="select"/>
				</div>
			</div>
		...
		<td class="preHelp">
			...
			<div id="subpregrid-btns" style="display:none">
			  ...
			  <input type="button" value="${eoms:a2u('重置预览')}" onclick="javascript:resetPreGrid();" class="btn"/>
			</div>
			<!--
