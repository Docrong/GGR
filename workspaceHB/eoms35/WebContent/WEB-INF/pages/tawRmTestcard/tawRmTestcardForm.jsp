...
<%
	String recordId = StaticMethod.null2String(request.getParameter("id"));
	String cardType=(String)request.getAttribute("cardType");
	String fromProvince=StaticMethod.nullObject2String(request.getAttribute("fromProvince"));
	String fromCity=StaticMethod.nullObject2String(request.getAttribute("fromCity"));
	String toProvince=StaticMethod.nullObject2String(request.getAttribute("toProvince"));
	String toCity=StaticMethod.nullObject2String(request.getAttribute("toCity"));
	String supplyer=(String)request.getAttribute("supplyer");
	String fromCountry=(String)request.getAttribute("fromCountry");
%>

<!--<script type="text/javascript" src="${app}/scripts/form/country.js"></script>-->
...
if ( confirm('${eoms:a2u("是否要删除此测试卡记录")}') ){
	return true;
	}else{
	return false;
	}
...
function changeType(v){
  document.getElementById("intelFrom").style.display = (v==1) ? "none" : "block";
  document.getElementById("localFrom").style.display = (v==0) ? "none" : "block";
  document.getElementById("intelTo").style.display = (v==1) ? "none" : "block";
  document.getElementById("localTo").style.display = (v==0) ? "none" : "block";
  if(v==0){
  	//document.getElementById("fromProvince").value='';
  	//document.getElementById("fromCity").value='';
  	//document.getElementById("toProvince").value='';
  	//document.getElementById("toCity").value='';
  	document.getElementById("supplyer").value='${eoms:a2u("GMCC市场部")}';
  	document.getElementById("supplyer").readOnly=true;
  }else{
  	...
  	document.getElementById("supplyer").readOnly=false;
  	document.getElementById("supplyer").value='<%=supplyer%>';
  }
}  

function changeType2(v){
  document.getElementById("intelFrom").style.display = (v==1) ? "none" : "block";
  document.getElementById("localFrom").style.display = (v==0) ? "none" : "block";
  document.getElementById("intelTo").style.display = (v==1) ? "none" : "block";
  document.getElementById("localTo").style.display = (v==0) ? "none" : "block";
...
function confirmDate(){
	var openAccountDate = document.getElementById("openAccountDate");
	var logoutDate = document.getElementById("logoutDate");
	var takeOverDate = document.getElementById("takeOverDate");
	if(openAccountDate.value!=''&&logoutDate.value!=''){
		if(openAccountDate.value>=logoutDate.value)
	    { 
	      alert("${eoms:a2u('注销日期必须大于开通日期！')}");
	      return false;
		}
	}
	var i;
    ...
    { 
    	...
    	var fromOperator = document.getElementById("fromOperator");
    	var toCountry = document.getElementById("toCountry");
    	var toOperator = document.getElementById("toOperator");
    	if(fromCountry.value==""){
	      alert("${eoms:a2u('请选择归属地的国家！')}");
	      return false;
	    }
	    if(fromOperator.value==""){
	      alert("${eoms:a2u('请选择归属地的运营商！')}");
	      return false;
	    }
	    if(toCountry.value==""){
	      alert("${eoms:a2u('请选择拜访地的国家！')}");
	      return false;
	    }
	    if(toOperator.value==""){
	      alert("${eoms:a2u('请选择拜访地的运营商！')}");
	      return false;
	    }
	}
	if(i==1)
    { 
    	var fromProvince = document.getElementById("fromProvince");
    	var fromCity = document.getElementById("fromCity");
    	var toProvince = document.getElementById("toProvince");
    	var toCity = document.getElementById("toCity");
    	if(fromProvince.value==""){
	      alert("${eoms:a2u('请选择归属地的省份！')}");
	      return false;
	    }
	    if(fromCity.value==""){
	      alert("${eoms:a2u('请选择归属地的城市！')}");
	      return false;
	    }
	    if(toProvince.value==""){
	      alert("${eoms:a2u('请选择拜访地的省份！')}");
	      return false;
	    }
	    if(toCity.value==""){
	      alert("${eoms:a2u('请选择拜访地的城市！')}");
	      return false;
	    }
	}
}
...
function test(v){
  document.getElementById("fromCountry").options[0]=new Option('${eoms:a2u("请选择")}','');
  document.getElementById("toCountry").options[0]=new Option('${eoms:a2u("请选择")}','');
  if(v==""){
  	document.getElementById("fromCountry").options[0].selected=true;
  	document.getElementById("toCountry").options[0].selected=true;
  }
}  

window.onload = function setup(){
 	InitCitySelect2(document.getElementById("fromProvince"),document.getElementById("fromCity"),'<%=fromCity %>');
 	InitCitySelect2(document.getElementById("toProvince"),document.getElementById("toCity"),'<%=toCity %>');
 	confirmCheck('<%=cardType%>');
 	changeType(<%=cardType%>);
 }
Ext.onReady(function() {
	v = new eoms.form.Validation({form:'tawRmTestcardForm'});
});
</script>
<!--对表单的自动生成的处�?-->
...
<ul>
    ...
    <!--表示对所有的域有�? -->
		   ...
		   <html:hidden property="tmpCardType" />
		   <html:hidden property="tmpSupplyer" />
		   <html:hidden property="tmpIccid" />
		   <html:hidden property="tmpMsisdn" />
		   <html:hidden property="tmpImsi" />
		   <html:hidden property="tmpPin" />
		   <html:hidden property="tmpOpenAccountDate" />
		   <html:hidden property="tmpLogoutDate" />
		   <html:hidden property="tmpTakeOverDate" />
		   <html:hidden property="tmpState" />
		   <html:hidden property="tmpFromProvince" />
		   <html:hidden property="tmpFromCity" />
		   <html:hidden property="tmpFromCountry" />
		   <html:hidden property="tmpFromOperator" />
		   <html:hidden property="tmpToProvince" />
		   <html:hidden property="tmpToCity" />
		   <html:hidden property="tmpToCountry" />
		   <html:hidden property="tmpToOperator" />
	         <br>
		...
		<table class="formTable">
			<tr>
				...
				<td width="500" colspan="2">
					<eoms:label styleClass="desc" key="tawRmTestcardForm.intelCard" /><html:radio property="cardType" value="0" onclick="changeType2(this.value);"/>
					<eoms:label styleClass="desc" key="tawRmTestcardForm.localCard" /><html:radio property="cardType" value="1" onclick="changeType2(this.value);"/>
				</td>
			</tr>
			<tr id="intelFrom">
				<td width="100" class="label">
				...
				<td width="500" colspan="2">
					...
					<eoms:country name="fromCountry" prompt="${eoms:a2u('请选择')}" default="${tawRmTestcardForm.fromCountry}"/>
					<eoms:label styleClass="desc" key="tawRmTestcardForm.operator" />:
					<eoms:dict key="dict-plancontent" dictId="operator" beanId="selectXML"
						isQuery="false" selectId="fromOperator" defaultId="${tawRmTestcardForm.fromOperator}"/>
				</td>
			</tr>
			<tr id="localFrom">
				<td width="100" class="label">
			...
			<tr id="intelTo">
				...
				<td width="500" colspan="2">
					<eoms:label styleClass="desc" key="tawRmTestcardForm.country" />:
					<eoms:country name="toCountry"  prompt="${eoms:a2u('请选择')}" default="${tawRmTestcardForm.toCountry}"/>
					<eoms:label styleClass="desc" key="tawRmTestcardForm.operator" />:
					<eoms:dict key="dict-plancontent" dictId="operator" beanId="selectXML"
						isQuery="false" selectId="toOperator" defaultId="${tawRmTestcardForm.toOperator}"/>
				</td>
			</tr>
			<tr id="localTo">
				<td width="100" class="label">
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="supplyer" styleId="supplyer"
						styleClass="text medium"
						alt="allowBlank:false,vtext:'${eoms:a2u('请输入提供者')}'"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="iccid" styleId="iccid" styleClass="text medium" maxlength="25" alt="allowBlank:false,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入25位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="msisdn" styleId="msisdn"
						styleClass="text medium" maxlength="15"
						alt="allowBlank:false,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入15位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="imsi" styleId="imsi"
						styleClass="text medium" maxlength="20"
						alt="allowBlank:false,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入20位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="pin" styleId="pin"
						styleClass="text medium" maxlength="4"
						alt="allowBlank:false,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入4位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="puk" styleId="puk"
						styleClass="text medium" maxlength="8"
						alt="allowBlank:false,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入8位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:password property="bootPassword" styleId="bootPassword"
						styleClass="text medium" maxlength="6"
						alt="allowBlank:true,vtype:'number'"/>
						<font style="font-size:13px;color:#CC0000;"><strong>${eoms:a2u('请输入6位以内的数字')}</strong></font>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="operation" styleId="operation"
						styleClass="text medium"
						alt="allowBlank:false,vtext:'${eoms:a2u('请输入开通业务')}'"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="openAccountDate" styleId="openAccountDate"
						styleClass="text medium"
						alt="allowBlank:false,vtext:'${eoms:a2u('请输入开通日期')}'" 
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="logoutDate" styleId="logoutDate"
						styleClass="text medium"
						alt="allowBlank:false,vtext:'${eoms:a2u('请输入注销日期')}'" 
						onclick="popUpCalendar(this, this,'yyyy-mm-dd',-1,-1,false);" readonly="true"/>
				</td>
			</tr>
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="takeOverDate" styleId="takeOverDate"
						styleClass="text medium"
						alt="allowBlank:false,vtext:'${eoms:a2u('请输入接收日期')}'" 
						onclick="popUpCalendar(this, this,'yyyy-mm-dd',-1,-1,false);" readonly="true"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<eoms:dict key="dict-plancontent" dictId="useState" beanId="selectXML"
						isQuery="false" selectId="state" defaultId="${tawRmTestcardForm.state}"
						 alt="allowBlank:false,vtext:'${eoms:a2u('请选择使用状态')}'" />
				</td>
			</tr>
			...
			<tr>
				...
				<td width="500" colspan="2">
					<html:text property="oldNumber" styleId="oldNumber"
						styleClass="text medium" />
				</td>
			</tr>
		</table>
		...
		<table>
			<tr>
				<td>
					<html:submit styleClass="button" property="method.save" onclick="return confirmDate()">
