...
<html:form action="/complaint.do?method=performHide" styleId="theform" onsubmit="return ifcheckhide();">
    <bean:define id="url" value="complaint.do"/>
    <display:table name="taskList" cellspacing="0" cellpadding="0"
                   id="taskList" pagesize="${pageSize}" class="table taskList"
                   export="false" requestURI="complaint.do"
        ...
        <display:column sortable="true" headerClass="sortable" title="工单状态">
            <eoms:dict key="dict-sheet-common" dictId="sheetStatus" itemId="${taskList.status}"
                ...
                <td colspan="6">
                    ...
                    <table class="table" width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <logic:present name="FROMRELATIONLIST" scope="request">
                            <logic:iterate id="tawSheetRelation1" name="FROMRELATIONLIST"
                                           type="com.boco.eoms.sheet.tool.relation.webapp.form.TawSheetRelationForm">
                                <tr id="relation<bean:write name="tawSheetRelation1" property="currentId" scope="page" />"
                                    style="display:none">
                                    <td>
                                        <input type="checkbox" name="relationCheckSheet"
                                               id="<bean:write name="tawSheetRelation1" property="currentId" scope="page"/>"
                                               onclick="hidecheck(this)"/>
                                        <input type="hidden" name="relationId"
                                               value="<bean:write name="tawSheetRelation1" property="parentId" scope="page"/>"/>
                                        <input type="hidden" name="relationFlowName"
                                               value="<bean:write name="tawSheetRelation1" property="currentFlowName" scope="page"/>"/>
                                    </td>
                                    <td>
                                        <bean:write name="tawSheetRelation1" property="parentSheetId" scope="page"/>
                                    </td>
                                    <td>
                                        <a onclick=openSheet('<bean:write name="tawSheetRelation1"
                                                                          property="currentFlowName" scope="page"/>','
                                        <bean:write name="tawSheetRelation1" property="currentId" scope="page"/>')>
                                        <bean:write name="tawSheetRelation1" property="currentSheetId"
                                                    scope="page"/>
                                        </a>
                                    </td>
                                    <td><bean:write name="tawSheetRelation1" property="parentSheetId" scope="page"/>-->调用--><bean:write
                                            name="tawSheetRelation1" property="currentSheetId" scope="page"/></td>
                                </tr>
                            </logic:iterate>
                        </logic:present>
                        <!-- 调用我的的工单 -->
                        <logic:present name="TORELATIONLIST" scope="request">
                            <logic:iterate id="tawSheetRelation2" name="TORELATIONLIST"
                                           type="com.boco.eoms.sheet.tool.relation.webapp.form.TawSheetRelationForm">
                                <tr id="relation<bean:write name="tawSheetRelation2" property="parentId" scope="page"/>"
                                    style="display:none">
                                    <td>
                                        <input type="checkbox" name="relationCheckSheet"
                                               id="<bean:write name="tawSheetRelation2" property="parentId" scope="page"/>"
                                               onclick="hidecheck(this)"/>
                                        <input type="hidden" name="relationId"
                                               value="<bean:write name="tawSheetRelation2" property="currentId" scope="page"/>"/>
                                        <input type="hidden" name="relationFlowName"
                                               value="<bean:write name="tawSheetRelation2" property="parentFlowName" scope="page"/>"/>
                                    </td>
                                    <td>
                                        <bean:write name="tawSheetRelation2" property="currentSheetId" scope="page"/>
                                    </td>
                                    <td>
                                        <a onclick=openSheet('<bean:write name="tawSheetRelation2"
                                                                          property="parentFlowName" scope="page"/>','
                                        <bean:write name="tawSheetRelation2" property="parentSheetId"
                                                    scope="page"/>')>
                                        <bean:write name="tawSheetRelation2" property="parentSheetId" scope="page"/>
                                        </a>
                                    </td>
                                    <td><bean:write name="tawSheetRelation2" property="parentSheetId" scope="page"/>-->调用--><bean:write
                                            name="tawSheetRelation2" property="currentSheetId" scope="page"/></td>
                                </tr>
                            </logic:iterate>
                        </logic:present>
                    </table>
