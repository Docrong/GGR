...
<style type="text/css">
 a{ text-decoration: none;color:#3d7fb3;}
 a:hover{text-decoration: underline;color:#3d7fb3;}
</style>
...
<table class="formTable">
	...
	<tr>
		...
		<td class="content">
			<eoms:dict key="dict-kmmanager" dictId="difficulty" isQuery="false" alt="allowBlank:false,vtext:'请选择问题难度'"
	...
	<tr>
		...
		<td class="content" colspan="3">
		   <textarea name="question" cols="50" id="question" class="textarea max" alt="allowBlank:false,maxLength:200" >${kmExamQuestionsForm.question}</textarea>
	...
	<tr id="choiceArea">
		...
		<td class="content" colspan="3">
			...
			<c:forEach items="${choiceList}" var="item">			   
				<tr id="rowId<%=num%>">	
				<td><input type="button" name="del<%=num%>" class="btn" value="删除" onclick="DeleteChoiceDB('rowId<%=num%>','${item.choiceID}')"></td>
				<td><input type="checkbox" indexed="true"   id="checkall" name="checkall" value="${item.orderBy}"/></td>						
				    <td>${item.orderBy}</td>
					<td class="text" id="choice" width="90%">
					   <textarea name="choice<%=num%>" rows="2" cols="100" id="choice<%=num%>"  alt="allowBlank:false,maxLength:1000" >${item.content}</textarea>
