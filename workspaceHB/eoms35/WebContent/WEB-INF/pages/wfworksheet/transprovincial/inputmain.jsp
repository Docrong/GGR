...
<table id="sheet" class="formTable" >
		...
		<tr><td class="label">
			...
			<td>
				<input type="text"  class="text" name="manager" id="manager"  alt="allowBlank:false,maxLength:500,vtext:'请填入 发起方集客部客户经理 信息，最多输入 500 字符'" value="${sheetMain.manager}"/>
			</td>
 <td class="label">
				<!-- 客户经理联系电话 -->
			...
			<td>
				<input type="text"  class="text" name="managerNum" id="managerNum"  alt="allowBlank:false,maxLength:500,vtext:'请填入 客户经理联系电话 信息，最多输入 500 字符'" value="${sheetMain.managerNum}"/>
			</td>
		</tr>
 ...
 <td class="label">
			...
			<td>
				<input type="text"  class="text" name="groupNum" id="groupNum"  alt="allowBlank:false,maxLength:500,vtext:'请填入 集团编号 信息，最多输入 500 字符'" value="${sheetMain.groupNum}"/>
			</td>
		</tr>
		...
		<tr><td class="label">
			...
			<td>
				<input type="text"  class="text" name="groupName" id="groupName"  alt="allowBlank:false,maxLength:500,vtext:'请填入 集团名称 信息，最多输入 500 字符'" value="${sheetMain.groupName}"/>
			</td>
<td class="label">
				<!-- 集团级别 -->
				集团级别*
 ...
 <td class="label">
			...
			<td>
				<input type="text"  class="text" name="bandWidth" id="bandWidth"  alt="allowBlank:false,maxLength:500,vtext:'请填入 带宽 信息，最多输入 500 字符'" value="${sheetMain.bandWidth}"/>
			</td>
		</tr>
		...
		<tr><td class="label">
			...
			<td>
				<input type="text"  class="text" name="lineNum" id="lineNum"  alt="allowBlank:false,maxLength:500,vtext:'请填入 专线数量 信息，最多输入 500 字符'" value="${sheetMain.lineNum}"/>
			</td>
 <td class="label">
				<!-- 客户服务等级 -->
			        ...
			        <script type="text/javascript">
				         Ext.onReady(function(){
		                        ...
		                        factoryTree = new xbox({
		    	                    ...
		    	                    treeDataUrl:factoryTreeAction,treeRootId:'1010107',treeRootText:'处理地市',treeChkMode:'',treeChkType:'dict',
		    	                    showChkFldId:'${sheetPageName}showFactory',saveChkFldId:'${sheetPageName}atypearea',viewer:factoryViewer
		                       });
		                 });
                     </script>
                     <textarea class="textarea max" readonly="readonly" name="${sheetPageName}showFactory" style="height:50px" id="${sheetPageName}showFactory"  alt="allowBlank:true,maxLength:250,vtext:'请填入HLR资源类型，最多输入125个汉字'"><c:forTokens items="${sheetMain.atypearea}" delims="," var="atypearea" varStatus="status"><eoms:id2nameDB id="${atypearea}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
 ...
 <td class="label">
			...
			<td>
				<eoms:attachment name="sheetMain" property="resourceReport" scope="request" idField="resourceReport" appCode="transprovincial" /> 
...
<table id="sheet" class="formTable">
	    ...
	    <tr>
		    ...
		    <eoms:attachment name="sheetMain" property="sheetAccessories" 
		            scope="request" idField="sheetAccessories" appCode="transprovincial" alt="allowBlank:true"/> 				
		    </td>
	   </tr>			  
</table>
