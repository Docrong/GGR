...
<SCRIPT LANGUAGE=javascript>
    ...
    function sent_submit() {
        var sel_sprlen = document.forms[0].sTime.options.length - 1;
//针对特殊排班的处理
        if (<%=StaticMethod.getNodeName("DUTY.OtherAssign").equals(request.getAttribute("roomId"))?"true":"false"%> &&
        sel_sprlen > 1
    )
        {
...
<html:form method="post" action="/TawRmSysteminfo/save">
                ...
                <table>
                    <tr>
                        ...
                        <td class="label" width="10%">
                            <input type=button Class="button" name=add value=
                                <bean:message key="entry.TawRmSysteminfo.btn_add"/> onclick="javascript:add_Time();">
                            <p>
                                <input type=button Class="button" name=del value=
                                    <bean:message
                                            key="entry.TawRmSysteminfo.btn_del"/> onclick="javascript:del_Time();">
                        </td>
                        <td width="80%">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    ...
                                    <select name="sTime" size=8 class="select">
                                        ...
                                        %>
                                        <option value="<%=vector_exchangetime.elementAt(i)%>"><%=vector_exchangetime.elementAt(i)%>
                                        </option>
                                        <%
                                            }
                                        %>
    ...
    <input type="button" Class="button" name=save value=
        <bean:message key="entry.TawRmSysteminfo.btn_save"/> onclick="sent_submit();">&nbsp;&nbsp;
    <INPUT id="transmit" type="button" Class="button" value='<bean:message key="entry.TawUserRoom.btn_new"/>'
           name=button
           Onclick="window.location.href ='../TawUserRoom/new.do?roomId=<bean:write name="tawRmSysteminfoForm"
                                                                                    property="roomId"/>'">&nbsp;&nbsp;
    <!--<INPUT id="expert" type="button" Class="button" value='配置专家' name=button Onclick="window.location.href ='../TawUserRoom/expert.do?roomId=<bean:write
