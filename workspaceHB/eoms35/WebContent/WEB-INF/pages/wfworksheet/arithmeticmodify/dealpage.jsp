...
<c:url var="urlDeal" value="/sheet/arithmeticmodify/arithmeticmodify.do">
...
function initPage(){
	...
	if("${operateType}" !='4'&&"${taskName}"!="cc"){
	 v.preCommitUrl = "${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=performPreCommit";	
	}
 } 
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "DraftTask") {	
   		strUrl = '${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
	var config = {
		url:strUrl,
		callback : initPage
    ...
    var form = document.forms[0];
   	var ajaxForm = Ext.getDom(form);
		    ...
		    Ext.Ajax.request({
				   		form: ajaxForm,
						method:"post",
						url: "${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=performSaveInfo",
						success: function(){
				        	alert("保存成功！");
				 		}
				    });
	   	...
	   	if (confirm("确认保存模板吗？")) {
		    if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "arithmeticmodify.do?method=saveDealTemplate&operateType=${operateType}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "arithmeticmodify.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}&operateType=${operateType}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
    ...
    var ajaxForm = Ext.getDom(form);
   	var templateManage = "${type}";
	   	...
	   	if (templateManage == "templateManage") {
	   		form.action = "${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=saveTemplate&templateId=${templateId}&type=${type}";
	   		form.submit();
	   	} else {
		   	if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "arithmeticmodify.do?method=saveTemplate&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "arithmeticmodify.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
...
<div id="sheetform">
  <html:form action="/arithmeticmodify.do?method=newPerformDeal" styleId="theform">
   <input type="hidden" name="operateType" value="${operateType}"/>
	<table>
	  <tr>
	  <td width="100%" align="left">
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'RejectTask'&& taskName != 'DraftTask'&& taskName != 'cc'}">
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./arithmeticmodify.do?method=getDealTemplatesByUserId&sheetKey=${sheetMain.id}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskStatus=${taskStatus}&operateType=${operateType}&draft=${taskName}',null,'left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes')">
		...
		<input type="hidden" name="dictKey" value="dict-sheet-arithmeticmodify"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType1();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
    ...
    <c:when test="${taskName=='RejectTask'}">    
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType2();" styleId="method.save">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/arithmeticmodify/arithmeticmodify.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	   </div>	 
   	</c:when>
    <c:otherwise>   
	    <!-- buttons -->
