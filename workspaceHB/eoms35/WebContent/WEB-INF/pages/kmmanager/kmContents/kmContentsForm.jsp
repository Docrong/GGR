        ...
        <table class="formTable">
            ...
            <tr>
                ...
                <td class="content">
                    <html:select property="TABLE_ID" styleId="TABLE_ID"
                                 ...
                                 alt="allowBlank:false,vtext:'请选择知识库...'" value="${kmContentsForm.id}"
            ...
            <tr>
                ...
                <td class="content">
                    ...
                    <input type="hidden" id="CREATE_DEPT" name="CREATE_DEPT"
                        ...
                        <c:choose>
                            <c:when test="${item.colType == 2}">
                                ...
                                <textarea name="props(${item.colName})" cols="50" id="${item.colName}"
                                          class="textarea max"
                                          alt="allowBlank:false,vtext:'请输入${item.colChname}...'"></textarea>
                            </c:when>

                            <c:when test="${item.colType == 3}">
                                <!-- 数字类型 -->
                                <input type="text" name="props(${item.colName})" id="${item.colName}" value=""
                                       class="text medium"
                                       alt="allowBlank:false,vtext:'请输入${item.colChname}(正整数类型)...',vtype:'number'"/>
                            </c:when>

                            <c:when test="${item.colType == 4}">
                                <!-- 日期时间 -->
                                <input type="text" name="props(${item.colName})" size="20"
                                       onclick="popUpCalendar(this, this,null,null,null,true,-1);" readonly="true"
                                       class="text" alt="allowBlank:false,vtext:'请选择${item.colChname}...'"/>
                            </c:when>

                            <c:when test="${item.colType == 5}">
                                <!-- 单选字段 -->
                                <eoms:comboBox name="props(${item.colName})" id="${item.colName}"
                            ...
                            <c:when test="${item.colType == 7}">
                                ...
                                <c:choose>
                                    <c:when test="${not empty kmContentsForm.id}">
                                        <eoms:attachment name="" property="" scope="request" idField="${item.colName}"
                                                         appCode="kmmanager"/>
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <c:otherwise>
                                <input type="text" name="${item.colName}" value="" id="${item.colName}"
                                       class="text medium" alt="allowBlank:false,vtext:'${item.colChname}'"/>
                            </c:otherwise>
                        </c:choose>
                    </td>
                </tr>
