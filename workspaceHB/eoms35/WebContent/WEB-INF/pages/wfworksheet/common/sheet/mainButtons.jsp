...
<logic:notPresent name="templateManage">
    ...
    <logic:empty name="draft">
        <html:submit styleClass="btn" property="method.saveDraft" styleId="method.saveDraft"
                     onclick="v.passing=true;javascript:saveDraft()">
            <bean:message bundle="sheet" key="button.saveAsDraft"/>
        </html:submit>
        <!-- 引用模板和保存模板 -->
        <html:button styleClass="btn" property="method.save" styleId="method.save"
                     onclick="window.open('./${module}.do?method=newGetTemplatesByUserId&mainId=${mainId}',null,'left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes')">
    ...
    <logic:notEmpty name="draft">
        <!-- 保存 -->
        <input type="submit" class="submit" onclick="v.passing=true;javascript:saveInfo()"
               value="<bean:message bundle='sheet' key='button.save'/>">
        <!-- 返回 -->
        <input type="button" class="submit" onclick="history.go(-1);"
        ...
        <logic:present name="type">
            ...
            <table class="formTable">
                <tr>
                    ...
                    <td clspan="3">
                        <input type="text" class="text max" name="sheetTemplateName"
                               value="${sheetMain.sheetTemplateName}">
                    </td>
                </tr>
            </table>
            <br>
