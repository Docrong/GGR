...
<%@ page language="java"
	import="java.util.*,java.lang.*,org.apache.struts.util.*"%>
<head>

</head>
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" class="clssclbar">
	<html:form method="post" action="/Kmchangeduty/save">
			...
			<tr width="100%">
				<td width="100%" align="center" class="table_title">
					...
					<%
						...
						String transactflag = String.valueOf(request
								.getParameter("TRANSACTFLAG"));
						if (transactflag.trim().equals("true")) {
					%>
					<
					<font color="red"><bean:message
							key="TawRmChangeduty.transactsuccess" />
					</font>
					<%
					}
					%>
					...
					<table border="0" width="100%" cellspacing="1" cellpadding="1"
						...
						<tr class="tr_show">
							<td colspan=10 class="label">
								<bean:message key="TawRmChangeduty.sent_apply" />
							</td>
						</tr>
						<tr class="td_label">
							<td class="label">
							...
							<td class="label">
								<bean:message key="TawRmChangeduty.starttime" />
							</td>
							<td class="label">
							...
							<td class="label">
								<bean:message key="TawRmChangeduty.applyreason" />
							</td>
							<td class="label">
								<bean:message key="TawRmChangeduty.flag" />
							</td>
							<td class="label">
								<bean:message key="TawRmChangeduty.operate" />
							</td>
						</tr>
						...
						<tr class="tr_show">
							<td colspan=10 class="label">
								<bean:message key="TawRmChangeduty.alert_no_sent" />
							</td>
						</tr>
						<%
								} else {
						...
						<tr class="tr_show">
							...
							<td>
								<%=String.valueOf(getHander.elementAt(i))%>
							</td>
							<td>
								<%=String.valueOf(getWorkserial1.elementAt(i))%>
							</td>
							<td>
								<%=String.valueOf(getReceiver.elementAt(i))%>
							</td>
							<td>
								<%=String.valueOf(getWorkserial2.elementAt(i))%>
							</td>
							<td>
								<%=String.valueOf(getReason.elementAt(i))%>
							</td>
							<td>
								<%
											if (Integer.parseInt(String.valueOf(getFlag.elementAt(i))
											.trim()) == 0) {
								%>
								...
								<%
											...
											.trim()) == 1) {
								%>
								<bean:message key="TawRmChangeduty.flag_agree" />
								${eoms:a2u('管理员未处理')}
								<%
								}
								%>
							...
							<td>
								...
								<input type="button" name="button" class="button"
									value=<bean:message key="TawRmChangeduty.withdraw"/>
									Onclick="window.location.href ='${app}/kmmanager/Kmchangeduty/transact_apply.do?transact_id=<%=Integer.parseInt(String.valueOf(
										getId.elementAt(i)).trim())%>&roomId=<%=roomId%>&transact_flag=3'">
								<%
								}
								%>
								<%
								...
								%>
								<input type="button" name="button" class="button"
									value=<bean:message key="TawRmChangeduty.know"/>
									Onclick="window.location.href ='${app}/kmmanager/Kmchangeduty/transact_apply.do?transact_id=<%=Integer.parseInt(String.valueOf(
										getId.elementAt(i)).trim())%>&roomId=<%=roomId%>&transact_flag=3'">
								<%
								}
			...
			<tr>
				<td>
					<table border="0" width="100%" cellspacing="1" cellpadding="1"
						...
						<tr class="tr_show">
							<td colspan=10 class="label">
								<bean:message key="TawRmChangeduty.receive_apply" />
							</td>
						</tr>
						<tr class="td_label">
							<td class="label">
						...
						<%
							...
							receiveQueryVector = (Vector) request
									.getAttribute("RECEIVEQUERYVECTOR");
						...
						<tr class="tr_show">
							<td colspan=10 class="label">
								<bean:message key="TawRmChangeduty.alert_no_receive" />
							</td>
						</tr>
						<%
								} else {
						...
						<tr class="tr_show">
							...
							<td>
								...
								&nbsp;&nbsp;
								<input type="button" name="button" class="button"
									value=<bean:message key="TawRmChangeduty.agree"/>
									Onclick="window.location.href ='${app}/kmmanager/Kmchangeduty/transact_apply.do?transact_id=<%=Integer.parseInt(String.valueOf(
										getId.elementAt(i)).trim())%>&roomId=<%=roomId%>&transact_flag=1'">
								&nbsp;
								<input type="button" name="button" class="button"
									value=<bean:message key="TawRmChangeduty.disagree"/>
									Onclick="window.location.href ='${app}/kmmanager/Kmchangeduty/transact_apply.do?transact_id=<%=Integer.parseInt(String.valueOf(
										getId.elementAt(i)).trim())%>&roomId=<%=roomId%>&transact_flag=2'">
								<%
								}
								%>
								&nbsp;
