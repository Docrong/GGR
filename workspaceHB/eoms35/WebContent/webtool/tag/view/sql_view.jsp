...
<table width="100%" class="form_table">
    <tr>
        ...
        <td><%
            if (tag.getIsPageination().equals("false")) out.print("否");
...
<table width="100%" class="form_table">
    <tr>
        ...
        <td><%
            if (!(tag.getVerifyMethod().indexOf("listSize") < 0)) out.print("判断记录数");
            else if (!(tag.getVerifyMethod().indexOf("isEqual") < 0)) out.print("两值比对");
        %>
        </td>
    </tr>
    <tr>
        <th width="30%">校验失败是否继续执行下面标签
        </td>
        <td><%
            if (tag.getIsDisplayNext().equals("false")) out.print("终止");
            else if (tag.getIsDisplayNext().equals("true")) out.print("继续");
        %>
        </td>
    </tr>
    <tr>
        <th width="30%">校验正确显示内容
        </td>
