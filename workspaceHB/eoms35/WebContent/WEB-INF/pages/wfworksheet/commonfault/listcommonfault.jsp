...
<bean:define id="url" value="commonfault.do"/>
	...
	<display:table name="taskListAgent" cellspacing="0" cellpadding="0"
   	...
   	<%  if (source == null) { %>
		...
		<display:column  sortable="true"   sortName="sheetId" headerClass="sortable" title="工单流水号" >
	       <a href="${app}/sheet/commonfault/commonfault.do?method=showDetailPage&sheetKey=${taskListAgent.sheetKey}&taskId=${taskListAgent.id}&taskName=${taskListAgent.taskName}&operateRoleId=${taskListAgent.operateRoleId}&TKID=${taskListAgent.id}&piid=${taskListAgent.processId}&taskStatus=${taskListAgent.taskStatus}&preLinkId=${taskListAgent.preLinkId}&type=interface&interface=interface&userName=<%=userId%>">
	       ${taskListAgent.sheetId }	
	       </a>
		</display:column>
		...
		<display:column property="mainAlarmSolveDate" sortable="true"  sortName="main.mainAlarmSolveDate"
			...
			format="{0,date,yyyy-MM-dd HH:mm:ss}" />
		<display:column sortable="true"	headerClass="sortable" title="告警核查状态" sortName="task.checkStatus">
			<eoms:dict key="dict-sheet-common" dictId="checkStatus" itemId="${taskListAgent.mainCheckStatus}"  beanId="id2descriptionXML"/>
		</display:column>
		<display:column  sortable="true" headerClass="sortable" title="T1一键回复">
	...
	<c:if test="${showBatchAcceptBtn=='true'}">
		<input type="button" class="btn"  
			onclick="javascript:window.location='<html:rewrite page='/commonfault.do?method=showBatchAcceptSheetList&psize=20'/>'"  value="批量接单"/>&nbsp;
		<input type="button" class="btn"  
			onclick="javascript:window.location='<html:rewrite page='/commonfault.do?method=showCombineSheetList&psize=20&tt=1'/>'"  value="T1环节合并工单"/>&nbsp;
		<input type="button" class="btn"  
			onclick="javascript:window.location='<html:rewrite page='/commonfault.do?method=showCombineSheetList&psize=20&tt=2'/>'"  value="T2环节合并工单"/>&nbsp;
	</c:if>
