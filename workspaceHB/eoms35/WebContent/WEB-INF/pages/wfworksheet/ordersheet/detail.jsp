...
<script type="text/javascript">
 function query() {
 window.open("./ordersheets.do?method=relationSheet&ifReference=${ifReference}",null,"left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes");
 }
 function modify(){
 window.location.href='./ordersheets.do?method=edit&id=${ordersheetForm.id}&ifReference=${ifReference}';
 }
 function remove(){
...
<caption><div class="header center">定单详细信息</div></caption>
<html:form action="ordersheets.do?method=xsave" method="post" styleId="detail">
			<input type="hidden" name="orderBuisnessType" value="${orderBuisnessType}"/>
   ...
   <tr>
     ...
     <td class="content"><eoms:id2nameDB id="${ordersheetForm.urgentDegree}" beanId="ItawSystemDictTypeDao"/></td>  
   </tr>   
   <tr>
     <td class="label">定单业务类型</td>
     <td class="content"><eoms:id2nameDB id="${ordersheetForm.orderBuisnessType}" beanId="ItawSystemDictTypeDao"/></td>
   ...
   <tr>
    <td class="label">生成时间</td>
      <td class="content">${eoms:date2String(ordersheetForm.creatTime)}</td>   
      <td class="label">竣工时间</td>
      <td class="content">${eoms:date2String(ordersheetForm.endTime)}</td>
   </tr>
   <tr>
    <td class="label">完成期限</td>
       <td class="content">${eoms:date2String(ordersheetForm.completeLimit)}</td>  
    <td class="label">变更时间</td>
       <td class="content">${eoms:date2String(ordersheetForm.changeTime)}</td>
    
   <tr>
     <td class="label">撤销时间</td>
       <td class="content" colspan="3">${eoms:date2String(ordersheetForm.cancelTime)}</td>
   </tr>
 </table>
 <br>
 <fieldset>
...
<table class="formTable">
  <tr>
   ...
   <tr>
     ...
     <td class="content"><bean:write name="ordersheetForm" property="transfBusiness" scope="request"/> </td>   
   </tr>  
   <tr>
...
<fieldset>	
<legend>相关服务存量信息</legend>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLines&id=${ordersheetForm.id}&specialtyType=${ordersheetForm.specialtyType}" width="100%" height="300px"></iframe>
</fieldset>
<fieldset>	
<legend>相关工单信息</legend>
<iframe id="frame" src="${app}/businessupport/order/ordersheets.do?method=getRelationSheetById&id=${ordersheetForm.id}&specialtyType=${ordersheetForm.specialtyType}" width="100%" height="300px"></iframe>
</fieldset>
<table>
<tr>
<td>
