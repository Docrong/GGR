...
public class BusinessChangeCrmManagerImpl extends InterfaceServiceManageAbstract implements IInterfaceServiceManage{

	public String getLinkBeanId() {
		// TODO Auto-generated method stub
		return "iBusinessChangeLinkManager";
	}

	public String getMainBeanId() {
		// TODO Auto-generated method stub
		return "iBusinessChangeMainManager";
	}
	...
	public String getProcessTemplateName() {
		// TODO Auto-generated method stub
		return "BusinessChangeMainFlowProcess";
	}

	public String getSendUser(Map map) {
		String userId = StaticMethod.nullObject2String(XmlManage.getFile("/config/businessChange-crm.xml").getProperty("base.InterfaceUser"));
		return userId;
	}

	public String getTaskBeanId() {
		// TODO Auto-generated method stub
		return "ibusinesschangeTaskManager";
	}
	...
	public Map initMap(Map map, List attach, String type) throws Exception {
		try{
			...
			map = this.loadDefaultMap(map, "config/businessChange-crm.xml", type);
			...
			if(type.equals(InterfaceConstants.WORKSHEET_NEW)||type.equals(InterfaceConstants.WORKSHEET_RENEW)){
				String sendRoleId = StaticMethod.nullObject2String(XmlManage.getFile("/config/businessChange-crm.xml").getProperty("base.SendRoleId"));
...
//				map.put("extendKey2", Boolean.TRUE);
				...
				String bigRole = StaticMethod.nullObject2String(XmlManage.getFile("/config/businessChange-crm.xml").getProperty("base.AcceptGroupId"));
				String toDeptId = StaticMethod.nullObject2String(map.get("toDeptId"));
				
				WPSEngineServiceMethod sm = new WPSEngineServiceMethod();
				String subRoleId = "";
				TawSystemSubRole subRole = sm.getMatchRoles("BusinessChangeProcess", toDeptId, bigRole, map);
				if(subRole==null||subRole.getId()==null||subRole.getId().length()==0){
					System.out.println("未找到匹配角色，使用默认角色");
					subRoleId = XmlManage.getFile("/config/businessChange-crm.xml").getProperty("base.AcceptRoleId");
				}else{
				...
				map.put("businesstype1", serviceType);//业务类型
			}
	...
	public void finishNew(BaseMain main, Map interfaceMap) {
		...
		try {
			...
			IBusinessChangeTaskManager taskservice = (IBusinessChangeTaskManager) ApplicationContextHolder
			.getInstance().getBean("ibusinesschangeTaskManager");
...
//			String  templateName = (String) interfaceMap.get("sheetType");
			...
			IBusinessChangeLinkManager linkservice = (IBusinessChangeLinkManager)ApplicationContextHolder.getInstance().getBean("iBusinessChangeLinkManager");
//			确认受理
			BusinessChangeLink confirmBean = new BusinessChangeLink();
