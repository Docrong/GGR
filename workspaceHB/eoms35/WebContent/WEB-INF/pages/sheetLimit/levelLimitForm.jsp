	...
	<c:if test="${!empty columnMap}">
		...
		<tr>
			...
			<td>
				...
				<input type="hidden" name="<%=key %>" value="<%=StaticMethod.nullObject2String(defaultValueMap.get(key)) %>"/>
			</c:if>
		</tr>
		<%} %>
	</c:if>
		<tr>
			...
			<td class="content">
				...
				<html:text property="acceptLimit" styleId="acceptLimit"	styleClass="text medium" alt="allowBlank:false,maxLength:9,vtext:'请填入受理时限，最多输入9个字符'" value="${levelLimit.acceptLimit}" onchange="setStepTime(this.value);"/>
				</c:if>
		...
		<tr>
			...
			<td class="content">
			<c:if test="${empty addStep}">
        		<html:text property="replyLimit" styleId="replyLimit"	styleClass="text medium" alt="allowBlank:false,maxLength:9,vtext:'请填入处理时限，最多输入9个字符'" value="${levelLimit.replyLimit}"/>
        	</c:if>
        	<c:if test="${addStep=='true'}">
	...
	<%
		...
		for(int i=0;i<phaseIdList.size();i++){
			...
			<tr>
				...
				<td class="content">
				<%if(tmpint==0){ %>
					<input	type="text" readonly="readonly" class="text" name="sacceptLimit" styleClass="text medium" alt="allowBlank:false,maxLength:9,vtext:'请填入受理时限，最多输入9个字符'" value="${levelLimit.acceptLimit }"/>
				<%}else{ %>
					<input	type="text" class="text" name="sacceptLimit" styleClass="text medium" alt="allowBlank:false,maxLength:9,vtext:'请填入受理时限，最多输入9个字符'" value="<%=tmpStepLimit.getAcceptLimit()%>"/>
				<%} %>
				</td>
				<td class="content">
				<input	type="text" class="text" name="completeLimit" styleClass="text medium" alt="allowBlank:false,maxLength:9,vtext:'请填入处理时限，最多输入9个字符'" value="<%=tmpStepLimit.getCompleteLimit()%>"/>
				</td>
			</tr>
			<%
			tmpint++;
