 ...
 %> 
	...
	<tr>
		...
		<tr>
     		...
     		<td>
    			<input type="text" class="text" name="${sheetPageName}linkProjectKey" id="${sheetPageName}linkProjectKey" value="${sheetLink.linkProjectKey}" alt="allowBlank:false,maxLength:40,vtext:'请填入技术方案关键字，最多输入40个字符'"/>
			  </td>
     	</tr>
     	<tr>
     		<!-- 总技术方案说明 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkProjectExplain" id="${sheetPageName}linkProjectExplain" class="textarea max" alt="allowBlank:true,maxLength:255,vtext:'请填入总技术方案说明，最多输入125个字符'">${sheetLink.linkProjectExplain}</textarea>
			  </td>
		</tr>
     	<tr>
     		<!-- 风险评估 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkRiskEvaluate" id="${sheetPageName}linkRiskEvaluate" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入风险评估，最多输入125个字符'">${sheetLink.linkRiskEvaluate}</textarea>
			  </td>
     	</tr>
     	<tr>
     		<!-- 影响业务分析 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkOperationConstrue" id="${sheetPageName}linkOperationConstrue" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入影响业务分析，最多输入125个字符">${sheetLink.linkOperationConstrue}</textarea>
			  </td>
     	</tr>
	...
	<input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="<%=operateType%>" />
        ...
        <tr>
     		...
     		<td colspan="3">
     			<eoms:comboBox name="${sheetPageName}linkAuditingResult" id="${sheetPageName}linkAuditingResult" initDicId="1010907" defaultValue="${sheetLink.linkAuditingResult}" alt="allowBlank:false" onchange="ifAuditPass1(this.value);"/>
			  </td>
		</tr>
        <tr>
     		<!-- 审核意见 -->
     		...
     		<td colspan="3">
     			<textarea name="linkAuditingIdea" id="linkAuditingIdea" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入审核意见，最多输入125个字符'">${sheetLink.linkAuditingIdea}</textarea>
			  </td>
		</tr>
		 <tr>
		    <td class="label">
	...
	<tr>
        ...
        <tr>
     		...
     		<td colspan="3">
     			<eoms:comboBox name="${sheetPageName}linkPermitResult" id="${sheetPageName}linkPermitResult" initDicId="1010908" defaultValue="${sheetLink.linkPermitResult}" alt="allowBlank:false" onchange="ifAuditPass(this.value);"/>
			  </td>
		</tr>
        <tr>
     		<!-- 审批意见 -->
     		...
     		<td colspan="3">
     			<textarea name="linkPermitIdea" id="linkPermitIdea" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入审批意见，最多输入125个字符'">${sheetLink.linkPermitIdea}</textarea>
			  </td>
		</tr>
		<tr>
		    <td class="label">
      ...
      <input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="118" />
     	<tr>
            ...
            <td class="label"><bean:message bundle="netchange" key="netchange.linkIfStartCircuit"/>*</td>
			  <td class="content">
			  	<eoms:comboBox name="${sheetPageName}linkIfStartCircuit" id="${sheetPageName}linkIfStartCircuit" initDicId="10301" defaultValue="${sheetLink.linkIfStartCircuit}" alt="allowBlank:false" onchange="popOtherFlow('circuitdispatch',this.value);"/>
			  ...
			  <td>
			    <eoms:comboBox name="${sheetPageName}linkIfStartData" id="${sheetPageName}linkIfStartData" initDicId="10301" defaultValue="${sheetLink.linkIfStartData}" alt="allowBlank:false" onchange="popOtherFlow('netdata',this.value);"/>
				<html:button styleClass="btn" property="method.save" styleId="startNetData" onclick="openwin('startNetData')">
	         		网络数据管理工单
	     		</html:button>
			  </td>
			</tr>
			...
			<!-- 是否启动软件变更工单 -->
			  ...
			  <td colspan="3">
			  <eoms:comboBox name="${sheetPageName}linkIfStartSoft" id="${sheetPageName}linkIfStartSoft" initDicId="10301" defaultValue="${sheetLink.linkIfStartSoft}" alt="allowBlank:false" onchange="popOtherFlow('softchange',this.value);"/>
				<html:button styleClass="btn" property="method.save" styleId="startSoftChange" onclick="openwin('startSoftChange')">
	         		软件变更工单
	     		</html:button>
			  </td>
			</tr>
		         ...
		         <tr>
	  		      <td class="label">是否调用其它工单实施</td>
	  		       <td class="content max" colspan=3>
	  		        <eoms:comboBox name="${sheetPageName}ifInvokeAgin" id="${sheetPageName}ifInvokeAgin" alt="allowBlank:false" initDicId="10301" onchange="InvokeAgin(this.value);" />                
	  		      </td>
	  		    </tr>
	...
	<tr id="test1" style="display:none">
		...
		<tr id="test3" style="display:none">
			 <!-- 是否完全按照方案实施 -->
			  ...
			  <td colspan="3">
			  	<eoms:comboBox name="${sheetPageName}linkIfAccordingProject" id="${sheetPageName}linkIfAccordingProject" initDicId="10301" defaultValue="${sheetLink.linkIfAccordingProject}" alt="allowBlank:false"/>
			  </td>
			 </tr>
        <tr id="test4" style="display:none">
     		...
     		<td colspan="3">
     			<textarea name="linkColligateExcuteResult" id="linkColligateExcuteResult" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入综合实施结果，最多输入125个字符'">${sheetLink.linkColligateExcuteResult}</textarea>
			  </td>
		</tr>
	...
	<tr>
		...
		<tr>
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkTestSummarize" id="${sheetPageName}linkTestSummarize" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入综合测试情况概述，最多输入125个字符'">${sheetLink.linkTestSummarize}</textarea>
			  </td>
		</tr>
		<tr>
     		<!-- 后续工作安排 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkWorkPlan" id="${sheetPageName}linkWorkPlan" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入后续工作安排，最多输入125个字符'">${sheetLink.linkWorkPlan}</textarea>
			  </td>
		</tr>
		<tr>
     		<!-- 是否涉及工程交维 -->
		...
		<tr>
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkMaintenanceDes" id="${sheetPageName}linkMaintenanceDes" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入交维描述，最多输入125个字符'">${sheetLink.linkMaintenanceDes}</textarea>
			  </td>
		</tr>
		 <tr>
		    <td class="label">
     	...
     	<tr>
     		...
     		<td colspan="3">
     			<eoms:attachment name="sheetLink" property="nodeAccessories" 
              scope="request" idField="${sheetPageName}nodeAccessories" appCode="netchange" alt="allowBlank:false"/>
			  </td>
		</tr>
     <%} }
        ...
        <%}else if(parentProcessName.equals("iNetOptimizeMainManager")){%>
     	...
     	<tr>
		    ...
		    <td>
		      <eoms:comboBox name="${sheetPageName}mainIfDemonstrateCase" id="${sheetPageName}mainIfDemonstrateCase" initDicId="1010912" defaultValue="${sheetMain.mainIfDemonstrateCase}" alt="width:'500px',allowBlank:false" styleClass="select"/>
		    </td>
		  <td class="label"><bean:message bundle="netchange" key="netchange.mainCaseKeywords"/>*</td>
		  <td>
				<input type="text"  class="text" name="${sheetPageName}mainCaseKeywords" id="${sheetPageName}mainCaseKeywords" value="${sheetMain.mainCaseKeywords}"  alt="allowBlank:false,maxLength:255,vtext:'请填入案例关键字，最多输入255个字符'"/>
		  </td>			    
		 </tr>  				
				 <tr>
				    ...
				    <td colspan="3">
				      <eoms:comboBox name="${sheetPageName}holdStatisfied" id="${sheetPageName}holdStatisfied" defaultValue="${sheetMain.holdStatisfied != 0 ? sheetMain.holdStatisfied : 1030301}" initDicId="10303" defaultValue="${sheetMain.holdStatisfied}" alt="width:'500px',allowBlank:false" styleClass="select"/>
				    </td>
				 </tr>			  
				 <tr>
	       ...
	       <td class="label">
			...
			<td colspan="3">			
			 <input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="-15" />
		           <textarea name="${sheetPageName}remark" class="textarea max" id="${sheetPageName}remark" 
		        alt="allowBlank:false,width:500,maxLength:255,vtext:'请填入备注，最多输入125个字符'" alt="width:'500px'">${sheetLink.remark}</textarea>
		  </td>
		</tr>  
  <%} %> 
 ...
 <%} %> 
	...
	<fieldset id="link4">
	 <legend>
		...
		<eoms:chooser id="netchangeSendRole" type="role" roleId="250" flowId="045" config="{returnJSON:true,showLeader:true}"
	...
	<%} %>
 	 <%if(taskName.equals("AuditTask")&& operateType.equals("130")) {%>	
		...
		<div ID="permit4">
			<eoms:chooser id="netchangeSendRole" type="role" roleId="251" flowId="045" config="{returnJSON:true,showLeader:true}"
			   category="[{id:'${sheetPageName}dealPerformer',text:'派发',vtext:'请选择派发对象'},{id:'${sheetPageName}subAuditPerformer',childType:'user,subrole',limit:'none',text:'会审'},{id:'${sheetPageName}copyPerformer',childType:'user,subrole',limit:'none',text:'抄送'}]"/>	
		</div>
		</fieldset>	
