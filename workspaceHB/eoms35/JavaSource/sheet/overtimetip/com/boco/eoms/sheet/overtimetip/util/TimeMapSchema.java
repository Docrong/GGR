	...
	private static void loadXml() throws Exception{
		try {
			...
			try {
				...
				map.loadMapping(StaticMethod.getFileUrl("classpath:com/boco/eoms/sheet/overtimetip/util/TimeMapping.xml"));
				un.setMapping(map);
				// 设置不验证xml以加快速度
				...
				InputStreamReader isr = new InputStreamReader(StaticMethod.getFileInputStream("classpath:config/sheet-overtime-mapping.xml"),"utf-8");
			...
			e.printStackTrace();
			throw new Exception("读取配置文件'config/sheet-overtime-mapping.xml'出错");
		}
	}
