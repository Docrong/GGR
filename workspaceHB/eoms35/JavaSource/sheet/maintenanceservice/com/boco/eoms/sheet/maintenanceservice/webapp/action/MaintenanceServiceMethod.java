...
import com.boco.eoms.sheet.maintenanceservice.model.MaintenanceServiceMain;
import com.boco.eoms.sheet.maintenanceservice.service.IMaintenanceServiceLinkManager;
import com.boco.eoms.sheet.maintenanceservice.service.IMaintenanceServiceMainManager;
...
public class MaintenanceServiceMethod extends BaseSheet {
    ...
    public void dealFlowEngineMap(ActionMapping mapping, ActionForm form,
        ...
        IMaintenanceServiceMainManager mainservice = (IMaintenanceServiceMainManager) ApplicationContextHolder.getInstance().getBean("iMaintenanceServiceMainManager");

        Map main = (HashMap) sheetMap.get("main");
        Map operate = (HashMap) sheetMap.get("operate");
        Map link = (HashMap) sheetMap.get("link");
        if ("HoldTask".equals(phaseId)) {
            String sendUserId = StaticMethod.nullObject2String(main.get("sendUserId"));
            System.out.println("sendUserId=1==" + sendUserId);
            String sheetId = StaticMethod.nullObject2String(main.get("sheetId"));
            if ("".equals(sendUserId)) {
                MaintenanceServiceMain mainBean = (MaintenanceServiceMain) mainservice.getMainBySheetId(sheetId);
                sendUserId = mainBean.getSendUserId();
                Map mainMap = SheetBeanUtils.bean2MapWithNull(mainBean);
                sheetMap.put("main", mainMap);
                System.out.println("sendUserId=2==" + sendUserId);
            }
            operate.put("dealPerformer", sendUserId);
            operate.put("dealPerformerLeader", sendUserId);
    ...
    public void showInputDealPage(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        if (operateType.equals("4") && taskName.equals("Examine")) {
            String sheetKey = StaticMethod.nullObject2String(request.getParameter("sheetKey"));
            IMaintenanceServiceLinkManager service = (IMaintenanceServiceLinkManager) ApplicationContextHolder.getInstance().getBean("iMaintenanceServiceLinkManager");
            String condition = " mainId='" + sheetKey + "' and operateType=107";
            List linkList = service.getLinksBycondition(condition);
            if (linkList != null && linkList.size() > 0) {
                request.setAttribute("fPreTaskName", "RejectTask");
            } else {
                request.setAttribute("fPreTaskName", "Fill");
            }
        }
        //如果是移交后的任务被驳回
        if (operateType.equals("4")) {
    ...
    public void saveTemplate(ActionMapping mapping, ActionForm form,
        ...
        if (templateId.equals("")) {
            ...
            if ("".equals(sheetTemplateName)) {
                sheetTemplateName = oldTemplateName;
            }
