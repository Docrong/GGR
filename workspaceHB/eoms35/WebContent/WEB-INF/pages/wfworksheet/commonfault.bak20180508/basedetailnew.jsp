<table class="formTable">
       ...
       <tr>
			...
			<%if(taskName.equals("DraftHumTask")){%>
			<td colspan='3'><input type="text" class="text max" name="${sheetPageName}title" id="${sheetPageName}title" value="${sheetMain.title}" alt="allowBlank:false"/></td>
			<%} else {%>
		...
		<tr>
			...
			<td colspan="3"><bean:write name="sheetMain" property="sendTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="request"/></td>
  ...
  <c:if test="${!empty sheetMain.parentSheetId}">
	...
	<c:if test="${sheetMain.mainIfCheck=='2'}">
	   <tr>
		...
		<td colspan='3'>
		  <eoms:id2nameDB id="${sheetMain.mainCheckResult}" beanId="ItawSystemDictTypeDao"/>
		</td>
	  </tr>	
	   <tr>
		<td class="label">
		...
		<td colspan='3'>
		  <pre><bean:write name="sheetMain" property="mainCheckIdea" scope="request"/></pre>
		</td>
	  </tr>
	</c:if>		
 </table> 