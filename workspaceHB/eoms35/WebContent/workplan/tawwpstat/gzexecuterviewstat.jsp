    ...
    <tr>
        ...
        <td width="3%" height="16">
            <%
                if ("1".equals(tawwpMonthPlanVO.getStubFlag())) {
            %>
            <a href="../tawwpexecute/executeadds.do?monthplanid=<%=tawwpMonthPlanVO.getId()%>&flag=daily&userbystub=<%=tawwpMonthPlanVO.getUserByStub()%>&date=<%=i%>"><%=i%>
            </a>
            <%
            } else {
            %>
            <a href="../tawwpexecute/executeadds.do?monthplanid=<%=tawwpMonthPlanVO.getId()%>&flag=daily&date=<%=i%>"><%=i%>
            </a>
            <%
                }
            %>
...
<INPUT type=button value=" <bean:message key="gzexecuterviewstat.title.btnBack" />" name="button" Onclick="GoBack();"
       class="button">
