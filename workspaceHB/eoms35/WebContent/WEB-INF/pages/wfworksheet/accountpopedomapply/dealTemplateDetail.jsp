...
<html:form action="/accountpopedomapply.do" styleId="theform">
    ...
    <table class="listTable">
        <tr>
            ...
            <td colspan="3">
                <textarea class="textarea max" name="${sheetPageName}applyAttitude"
                          id="${sheetPageName}applyAttitude">${sheetLink.applyAttitude}</textarea>
            </td>
        </tr>
        <tr>
            <td class="label"><bean:message bundle="accountpopedomapply" key="accountpopedomapply.applyResult"/></td>
            <td colspan="3">
                <textarea class="textarea max" name="${sheetPageName}applyResult"
                          id="${sheetPageName}applyResult">${sheetLink.applyResult}</textarea>
            </td>
        </tr>
        <tr>
            <td class="label"><bean:message bundle="accountpopedomapply" key="accountpopedomapply.deResult"/></td>
            <td colspan="3">
                <textarea class="textarea max" name="${sheetPageName}deResult"
                          id="${sheetPageName}deResult">${sheetLink.deResult}</textarea>
            </td>
        </tr>
    </table>
    </table>
