...
FCKXHtml._AppendNode = function (A, B) {
    ...
    switch (B.nodeType) {
        case 1:
            ...
            if (FCKBrowserInfo.IsGecko && B.nextSibling && (B.hasAttribute('_moz_editor_bogus_node') || B.getAttribute('type') == '_moz')) return false;
...
FCKSelection.Save = function () {
    ...
    if (B) {
        C = B.createRange();
        if (C) {
            if (C.parentElement && FCKTools.GetElementDocument(C.parentElement()) != A) C = null; else if (C.item && FCKTools.GetElementDocument(C.item(0)) != A) C = null;
        }
    }
...
FCKTableHandler._UnmarkCells = function (A, B) {
    for (var i = 0; i < A.length; i++) {
        if (FCKBrowserInfo.IsIE) A[i].removeAttribute(B); else delete A[i][B];
    }
};
