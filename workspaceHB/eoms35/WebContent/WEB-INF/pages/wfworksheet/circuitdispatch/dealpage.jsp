 ...
 //request.setAttribute("operateType",operateType);
  String enableService = com.boco.eoms.base.util.StaticMethod.nullObject2String(com.boco.eoms.commons.util.xml.XmlManage
					.getFile("/config/circuitdispatch-util.xml").getProperty(
							"EnableService"));
 request.setAttribute("enableService",enableService);
%>
...
<c:url var="urlDeal" value="/sheet/circuitdispatch/circuitdispatch.do">
  ...
  <c:param name="processname" value="CircuitDispatchMainFlowProcess" /> 
</c:url> 
...
function initPage(){
	...
	if("${taskName}"!="cc"&&"${operateType}"!='4'&&"${operateType}"!='11'){
		v.preCommitUrl = "${app}/sheet/circuitdispatch/circuitdispatch.do?method=performPreCommit";	
		if("${taskName}"=="PermitTask"&&"${enableService}"=="true"){
			v.preCommitUrl = "";
		}
	}
 } 
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "DraftTask") {	
   		strUrl = '${app}/sheet/circuitdispatch/circuitdispatch.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
   ...
   function saveDealInfo(){
    ...
    Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "${app}/sheet/circuitdispatch/circuitdispatch.do?method=performSaveInfo",
				success: function(){
		        	alert("保存成功！");
		 		}
		    });
  ...
  function saveDealTemplate(type) { 
   	var form = document.forms[0];
   	var ajaxForm = Ext.getDom(form);
   	//$('${sheetPageName}operateType').value='${operateType}';
   	//alert($('${sheetPageName}operateType').value);
   	if(v.check()) {
    v.passing = false;
   	if (confirm("确认保存模板吗？")) {
	    if (type == "new"){
		   	Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "circuitdispatch.do?method=saveDealTemplate&operateType=${operateType}",
				success: function(){
		        	alert("保存模板成功！");
		 		}
		    });
	   		...
	   		Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "circuitdispatch.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}&operateType=${operateType}",
				success: function(){
		        	alert("保存模板成功！");
		 		}
		    });
    ...
    var ajaxForm = Ext.getDom(form);
     ...
     v.passing = false;
	   	...
	   	if (templateManage == "templateManage") {
	   		form.action = "${app}/sheet/circuitdispatch/circuitdispatch.do?method=saveTemplate&templateId=${templateId}&type=${type}";
	   		form.submit();
	   	} else {
		   	if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "circuitdispatch.do?method=saveTemplate&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "circuitdispatch.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
...
<div id="sheetform">
  <html:form action="/circuitdispatch.do?method=newPerformDeal" styleId="theform">		
  <!-- <input type="hidden" name="operateType" value="${operateType}"/> -->
	<table>
	  <tr>
	  <td width="100%" align="left">
	  <c:if test="${operateType != '' && operateType!= '115' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&&taskName!='DraftTask'&&taskName!='RejectTask'}">
		<html:button styleClass="btn" property="method.save" styleId="method.save" onclick="window.open('./circuitdispatch.do?method=getDealTemplatesByUserId&sheetKey=${sheetMain.id}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskStatus=${taskStatus}&operateType=${operateType}&draft=${taskName}',null,'left=300,top=150,height=400,width=500,scrollbars=yes,resizable=yes')">
		         <bean:message bundle="sheet" key="button.refereTemplate"/>
		 </html:button>
		 <c:choose> 
		<c:when test="${taskName=='DraftTask'||taskName=='RejectTask'}">
		...
		<input type="hidden" name="dealTemplateNameRule" value="title;mainOperateType"/>
		<c:if test="${taskName!='DraftTask'}">
		<input type="hidden" name="title" value="${sheetMain.title}"/>
		</c:if>		

		<input type="hidden" name="dictKey" value="dict-sheet-circuitdispatch"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/circuitdispatch/circuitdispatch.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftTask'}">
		...
		<html:submit styleClass="btn" property="method.save" onclick="javascript:changeType1();" styleId="submitone">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/circuitdispatch/circuitdispatch.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/circuitdispatch/circuitdispatch.do?method=showListsendundo&draft=1'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
    <c:when test="${taskName=='RejectTask'}">
		<div class="form-btns" id="btns">
    ...
    <c:when test="${taskName=='HoldTask'}">    
	    ...
	    <div class="form-btns" id="btns">
         ...
         <input type="submit" class="submit" name="method.save" id="method.save"  onclick="javascript:ifisCopy();" value="<bean:message bundle='sheet' key='button.done'/>" >	      
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;$('${sheetPageName}status').value = '0';this.form.action='${app}/sheet/circuitdispatch/circuitdispatch.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	       <%-- </c:if>--%>
    ...
    <c:otherwise>  
	    ...
	    <div class="form-btns" id="btns">
         <input type="submit" class="submit" name="method.save" id="method.save"  onclick="javascript:ifisCopy();" value="<bean:message bundle='sheet' key='button.done'/>" >	      
         <c:if test="${empty preLink.linkRejectFlag||preLink.linkRejectFlag==''}">
	      <html:button styleClass="btn" property="method.save" styleId="method.save"
		    onclick="v.passing=true;javascript:saveDealInfo();">
		          	<bean:message bundle='sheet' key='button.save'/>
		    	</html:button><%-- <c:if test="${taskName!='cc'}">--%>
