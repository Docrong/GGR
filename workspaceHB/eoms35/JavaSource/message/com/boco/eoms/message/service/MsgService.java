...
public interface MsgService {
     ...
     * 消息发送，根据服务订阅信息发送消息
     *
     * @param serviceId    消息系统中订阅的服务id
     * @param msg          发送内容
     * @param buzId        具体调用消息服务的业务流水号，如：作业计划id（主键），主要为取消消息发送
     * @param orgIds       发送人员或者部门的串 格式：1,admin#1,sunshengtai#2,254 （其中1代表人员  2代表部门 254代表部门id）
     * @param dispatchTime 业务发送时间点
     ...
     * @param msg          发送内容
     * @param buzId        具体调用消息服务的业务流水号，如：作业计划id（主键），主要为取消消息发送
     * @param orgIds       手机号以,分隔，如： 13510299287,13898979879,15934567876
     * @param dispatchTime 业务发送时间点
     * @return success, fail（成功与否）
     */
...
//			String mobiles);
     ...
     * @param subject        邮件主题
     * @param content        邮件内容
     * @param buizId         具体调用消息服务的业务流水号，如：作业计划id（主键），主要为取消消息发送
     * @param addresser      邮件的发件人 email格式
     * @param orgIds         发送人员或者部门的串 格式：1,admin#1,sunshengtai#2,254 （其中1代表人员  2代表部门 254代表部门id）
     * @param dispatchTime   业务发送时间点
     * @param accessoriesUrl 附件url地址
     * @return success, fail（成功与否）
     */
...
//			String emails);
     ...
     * @param msg       发送内容
     * @param buzId     具体调用消息服务的业务流水号，如：作业计划id（主键），主要为取消消息发送
     * @param orgIds    系统组织（用户，角色，部门）ids,id与typ成对出现，中间以逗号相隔，多条之间以井号出现，如：userId,1#roleId,2
     * @return success, fail（成功与否）
     */
     ...
     * @param allocTime    派单时间
     * @param finishTime   要求结束时间
     * @param content      语音内容
     * @param senderId     派单人userId
     * @param orgIds       接单人userId （orgIds 格式：1,admin#1,sunshengtai#2,151(其中1代表人，2代表部门)）
     * @return success, fail（成功与否）
     */
     ...
     * @param finishTime  要求结束时间
     * @param content     语音内容
     * @param telNum      工单接收人电话
     * @param telNum2     工单负责人电话（如果没有就用telNum工单接收人电话）
     * @param dispatchTel 派单人电话（如果没有就用telNum工单接收人电话）
     * @return success, fail（成功与否）
     */
     ...
     * @param serviceId      消息系统中订阅的服务id
     * @param buzId          具体调用消息服务的业务流水号，如：作业计划id（主键），主要为取消消息发送
     * @param orgIds         接收人（orgIds 格式：1,admin#1,sunshengtai#2,151#3,185(其中1代表人，2代表部门,3代表角色ID)）
     * @param dispatchTime   业务发送时间点
     * @param subject        彩信主题
     * @param mmsContentList 彩信对象集合
     * @return success, fail（成功与否）
     */
