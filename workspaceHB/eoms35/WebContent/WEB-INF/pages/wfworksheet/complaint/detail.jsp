		...
		<option value="${urlShowRejectBackPage}"><bean:message bundle="sheet" key="common.rejectBack"/></option> 
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
			 	  ...
			 	  <%if(ifsub.equals("") || ifsub.equals("false")){%>
					  ...
					  <!-- 流程步骤，移交，阶段回复，分派 -->
					  <option value="${urlShowTransmitDealPage}"><bean:message bundle="complaint" key="operateType.transmit1"/> </option>
					  <option value="${urlShowCompleteDealPage}"><bean:message bundle="complaint" key="operateType.dealComplete"/></option>
			...
			<!--		  <option value="${urlShowSendExaminePage}"><bean:message bundle="complaint" key="operateType.sendToExamine"/></option>    -->
			 	 ...
			 	 <%} else { %>
			 		...
			 		<!-- 不需要等待回复 -->
			 	  	<% if (ifwaitfor.equals("false")) {%>
			 			<option value="${urlShowDispatchPage}"><bean:message bundle="complaint" key="operateType.subTaskReply"/></option>
			 		<%}%>
			     <% } %>
			  <option value="${urlShowInputSplit}"><bean:message bundle="sheet" key="common.split"/></option>		  
		...
		<select id="dealSelector" onchange="javascript:eoms.util.appendPage('sheet-deal-content',this.value,true);">
		 ...
		 <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
		 	  ...
		 	  <%if(ifsub.equals("") || ifsub.equals("false")){%>
	             ...
	             <!--  <c:if test="${sheetMain.complaintType1 != '101062501' }"><option value="${urlShowAsleepOp}">工单休眠</option></c:if>  --><!-- add by weichao 20141223 -->		                   
	...
	<%}else if(taskName.equals("CheckingHumTask")){%>	
		<select id="dealSelector">
		  <%if(taskStatus.equals(Constants.TASK_STATUS_READY)){%>
		   ...
		   <option value="${urlShowRejectBackPage}"><bean:message bundle="sheet" key="common.rejectBack"/></option>
		  <%}else if(taskStatus.equals(Constants.TASK_STATUS_CLAIMED)){ %>
		   <option value="${urlShowCheckYesDealPage}"><bean:message bundle="complaint" key="operateType.checkYes"/> </option>
		   <option value="${urlShowCheckNoDealPage}"><bean:message bundle="complaint" key="operateType.checkNo"/></option>			  
  		  <%}%>
  		</select>
	<%}else if(taskName.equals("DraftHumTask")){%>	
		<select id="dealSelector">
		  <option value="${urlShowCompleteDealPage}"><bean:message bundle="complaint" key="operateType.draft"/></option>
		</select>
	<%}else if(taskName.equals("ByRejectHumTask")){%>	
		<select id="dealSelector">
		  <option value="${urlShowDraftDealPage}"><bean:message bundle="complaint" key="operateType.reSend"/></option>
		</select>		
	<%}else if(taskName.equals("DeferExamineHumTask")){%>	
		<select id="dealSelector">
		  <option value="${urlShowExamineYesDealPage}"><bean:message bundle="complaint" key="operateType.Examine"/></option>
		  <option value="${urlShowExamineNoDealPage}"><bean:message bundle="complaint" key="operateType.examineReject"/></option>
		</select>
	<%}else if(taskName.equals("HoldHumTask")){ %>
		<select id="dealSelector">
		  <option value="${urlShowHoldDealPage}"><bean:message bundle="complaint" key="operateType.hold"/></option>
		  <option value="${urlShowRejectDealPage}"><bean:message bundle="complaint" key="operateType.reject"/></option>
		</select>	
	<%}%>
