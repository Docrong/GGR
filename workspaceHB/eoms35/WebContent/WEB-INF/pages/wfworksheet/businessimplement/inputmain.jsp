 ...
 <legend>集团客户信息</legend> 
  <table class="formTable">
		     ...
		     <td class="content">
		     	...
		     	<eoms:comboBox   name="${sheetPageName}customLevel" id="${sheetPageName}customLevel"  initDicId="10405" styleClass="select-class" alt="allowBlank:false" defaultValue="${sheetMain.mainArgument}" />
		     </td>
		   </tr>
		   <tr>
		     <td class="label">集团客户编号*</td>
...
<iframe id="frame3" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=<%=orderSheetId %>&taskStatus=${taskStatus}&specialtyType=101230104&sheetType=businessImplementyy&taskName=LauguageTask&isShowLanguage=yes" width="100%" height="300px"></iframe>	

<%}else if(startType !=null && startType !=""){%>
<iframe id="frame4" src="${app}/businessupport/order/ordersheets.do?method=getSpecialLinesByType&orderId=<%=orderSheetId %>&taskStatus=${taskStatus}&specialtyType=<%=specialtyType %>&sheetType=businessImplementSms&taskName=TranferTask&isShowSms=yes" width="100%" height="300px"></iframe>	
<%}else{%>
		<tr>
			<td colspan="4">
