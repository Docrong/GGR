...
<%@ include file="/WEB-INF/pages/wfworksheet/netchange/baseinputmainhtmlnew.jsp"%>
		...
		<!-- <tr>
			...
			<!-- 网络分类 -->
			  ...
			  <td>
			  	 <eoms:comboBox name="${sheetPageName}mainNetSortOne" id="${sheetPageName}mainNetSortOne" 
			  	      sub="${sheetPageName}mainNetSortTwo" initDicId="1010104" defaultValue="${sheetMain.mainNetSortOne}" alt="allowBlank:false" onchange="specialtyType(this.value);selectLimit(this.value);"/>
			  </td>				
            ...
            <tr>
			  ...
			  <!-- 牵头专业 -->
			   <td class="label"><bean:message bundle="netchange" key="netchange.mainSpecialtyType"/></td>
				  <td>
				  <input type="text" class="text" name="${sheetPageName}mainSpecialtyType" id="${sheetPageName}mainSpecialtyType" value="${sheetMain.mainSpecialtyType}" alt="allowBlank:true,maxLength:40,vtext:'请填入牵头专业，最多输入40个字符"/>
				  </td>	  
			</tr>
			 <tr>    	
			              ...
			              <td colspan="3"> 	
	                            ...
	                            <textarea class="textarea max" readonly="true" name="mainAssortSpeciality2" style="height:50px" id="mainAssortSpeciality2"  alt="allowBlank:true"><eoms:invert appCode="netchange" sheetKey="${sheetMain.mainAssortSpeciality}" beanId="CacheBean" scope="page"/></textarea>
			...
			<tr>
			 <!-- 是否需要技术方案 -->
			  ...
			  <td>
			  	<eoms:comboBox name="${sheetPageName}mainIfRequireProject" id="${sheetPageName}mainIfRequireProject" initDicId="10301" defaultValue="${sheetMain.mainIfRequireProject}" alt="allowBlank:false" onchange="ifDoProjectDeal(this.value)"/>
			...
			<tr>
			  ...
			  <td>
			    <input type="text" class="text" name="${sheetPageName}mainExpectationCompleteTime" readonly="readonly" 
					id="${sheetPageName}mainExpectationCompleteTime" value="${eoms:date2String(sheetMain.mainExpectationCompleteTime)}" 
 ...
 割接原因 
			...
			<!-- 综合变更需求说明 -->
			  ...
			  <td colspan="3">
			  	<textarea name="${sheetPageName}mainChangeExplain" id="${sheetPageName}mainChangeExplain" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入综合变更需求说明，最多输入125个字符'">${sheetMain.mainChangeExplain}</textarea>		   
			  </td>			 
			</tr>				
	...
	<table id="sheetProjectCreate" class="formTable"  style="display:none">
		<tr>
     		...
     		<td>
    			<input type="text" class="text" name="${sheetPageName}linkProjectKey" id="${sheetPageName}linkProjectKey" value="${sheetMain.linkProjectKey}" alt="allowBlank:false,maxLength:40,vtext:'请填入技术方案关键字，最多输入40个字符'"/>
			  </td>
     	</tr>
     	<tr>
     		<!-- 总技术方案说明 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkProjectExplain" id="${sheetPageName}linkProjectExplain" class="textarea max" alt="allowBlank:true,maxLength:255,vtext:'请填入总技术方案说明，最多输入125个字符'">${sheetMain.linkProjectExplain}</textarea>
			  </td>
		</tr>
     	<tr>
     		<!-- 风险评估 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkRiskEvaluate" id="${sheetPageName}linkRiskEvaluate" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入风险评估，最多输入125个字符'">${sheetMain.linkRiskEvaluate}</textarea>
			  </td>
     	</tr>
     	<tr>
     		<!-- 影响业务分析 -->
     		...
     		<td colspan="3">
     			<textarea name="${sheetPageName}linkOperationConstrue" id="${sheetPageName}linkOperationConstrue" class="textarea max" alt="allowBlank:false,maxLength:255,vtext:'请填入影响业务分析，最多输入125个字符'">${sheetMain.linkOperationConstrue}</textarea>
			  </td>
     	</tr>
