...
<head><title>管理员WAP配置工具</title>
    ...
    <script type="text/javascript" src="<%=app%>/webtool/scripts/ajax/prototype.js">
        Try.these(
            function () {
                parent.webFXTreeHandler.all["${refreshId}"].parentNode.reload()
            }
        );
    </script>
</head>
...
<form name="form1" action="<%=app %>/webtool/wapnode" onSubmit="return checkInput()">
    ...
    <table width="100%">
        <tr>
            <td>修改系统信息</td>
            <td align="right"><a href="<%=app %>/webtool/card/add.jsp?wapNodeId=<%=wapNodeVO.getNodeId()%>">增加页面</a>
            </td>
        </tr>
    </table>
    <table class="form_table">
        ...
        <tr>
            <th>系统描述</th>
            <td><input name='description' type='text' value='<%=wapNodeVO.getDescription()%>'/><font
    ...
    <table class="list_table">
        <tr>
            <td colspan="4" class="nobg">页面列表</td>
        </tr>
        <tr>
            <th width="25%">页面名称
            </td>
