...
<head>
    ...
    <script language="JavaScript">
        ...
        function submitReject(userid, flowId, reportFileId) {
            ...
            currFlowId = flowId;
            type = "1";
            var loadStr = "<info userId='" + userid + "' type='reject1' auditInfo='" + auditInfo + "' flowId='" + flowId + "'/>";
            ReceiveStr(loadStr);
        }
        ...
        function submitPass(userid, flowId, reportFileId) {
            ...
            type = "0";
            var auditInfo = document.getElementById(reportFileId).value;
            var loadStr = "<info userId='" + userid + "' type='pass1' auditInfo='" + auditInfo + "' flowId='" + flowId + "'/>";
            ReceiveStr(loadStr);
        }
...
<logic:iterate id="reportFile" name="ReportList"
               type="com.boco.eoms.filemanager.form.ReportListBean">
    ...
    <table width="100%" class="formTable">
        <tr>
            ...
            <td>
                <bean:write name="reportFile" property="dealUserName" scope="page"/>
                [
                <bean:write name="reportFile" property="acceptDeptName" scope="page"/>
                ]
            </td>
            <td width="15%" class="label">
                <b>联系方式</b>
        ...
        <tr>
            ...
            <td
                    id="rejectArea_<bean:write name="reportFile" property="flowId" />">

                <logic:equal value="0" name="reportFile" property="status">
                <font color="red">
                    </logic:equal>
                    <b><bean:write name="reportFile" property="statusName"
                                   scope="page"/> </b>
                    <logic:equal value="0" name="reportFile" property="status">
                </font>
                </logic:equal>
            ...
            <logic:equal value="1" name="reportFile" property="status">
                ...
                <td width="15%">
                    <input type="button" name="btnVote" value="驳回报表" class="button"
                           onclick="if(window.confirm('是否要驳回\'<bean:write name="reportFile" property="acceptDeptName"
                                                                          scope="page"/>\'的报表？'))  submitReject('
                               <bean:write scope="session" name="sessionform" property="userid"/>','<bean:write
                                   name="reportFile" property="flowId"/>','<%="reportFile" + i%>');"/>
                </td>
                <td>
                    <input type="button" name="btnVote" value="通过报表" class="button"
                           onclick="if(window.confirm('是否要通过\'<bean:write name="reportFile" property="acceptDeptName"
                                                                          scope="page"/>\'的报表？'))  submitPass('
                               <bean:write scope="session" name="sessionform" property="userid"/>','<bean:write
                                   name="reportFile" property="flowId"/>','<%="reportFile" + i%>');"/>
                </td>
            </logic:equal>
            <logic:equal value="4" name="reportFile" property="status">
                <td class="label" width="15%">
                    <b>审核意见</b>
                </td>
                <td>
...
<%
    ...
    int count = StaticMethod.nullObject2int(request
            .getAttribute("size"));
    if (count > 1) {
%>
<table class="listTable" width="100%">
    <tr height="15">
        <td width="100%" align="right" nowrap valign="middle">
            <font size="2"><bean:write name="pagerHeader" scope="request"
                                       filter="false"/> <%!String key;%></font>
        </td>
        <!--ZF add-->
        <br/>
...
%>
<!--ZF add ===========================================================================================-->
<span id="combine_span" style="display: none;"> <html:form
        action="/ReportMgrAction.do?act=COMBINE" method="post"
        enctype="multipart/form-data">
		<input type="hidden" name="combinType"
               value="<bean:write name="SchemeMgrForm" property="combinType" />">
    <table width="100%" class="formTable">
			...
			<tr id="combine_table2_add1">
				<td>
					<logic:iterate id="fileList5" name="SchemeMgrForm"
                                   property="fileUrl">
                        <%
                            String fileInfo5 = fileList5.toString().trim();
                            if (fileInfo5 != null)
                                fileInfo5 = fileInfo5.trim();
    ...
    <table width="100%" cellspacing="1" cellpadding="1" class="formTable"
           align=center>
			...
			<logic:iterate id="reportFile2" name="ReportList"
                           type="com.boco.eoms.filemanager.form.ReportListBean">
                <logic:iterate id="fileList2" name="reportFile2" property="fileUrl">
                    ...
                    <tr id="combine_table2">
						...
						<td nowrap>
							...
							<input type="checkbox" name="fileList" value="<%=url2%>"
                                    <%if (reportFile2.getReject() == 1) { %> disabled="disabled"
                                    <%} %> />
						</td>
						<td nowrap>
							<bean:write name="reportFile2" property="dealUserName"
                                        scope="page"/>
							[
							<bean:write name="reportFile2" property="acceptDeptName"
                                        scope="page"/>
							]
						</td>
						<td nowrap>
    ...
    <table width="100%">
			<td>
				...
				<html:button property="btncombine" onclick="combineCheck()"
                             value="合并选中" styleClass="button"/>
			</td>
		</table>
</html:form> </span>
