...
import com.boco.eoms.sheet.resourceaffirm.service.IResourceAffirmLinkManager;
import com.boco.eoms.sheet.resourceaffirm.service.IResourceAffirmTaskManager;

public class ResourceAffirmCrmManagerImpl extends InterfaceServiceManageAbstract implements IInterfaceServiceManage {

	public String getLinkBeanId() {
		// TODO Auto-generated method stub
		return "iResourceAffirmLinkManager";
	}

	public String getMainBeanId() {
		// TODO Auto-generated method stub
		return "iResourceAffirmMainManager";
	}
	...
	public String getProcessTemplateName() {
		// TODO Auto-generated method stub
		return "ResourceAffirmMainFlowProcess";
	}

	public String getSendUser(Map map) {
		String userId = StaticMethod.nullObject2String(XmlManage.getFile("/config/resourceAffirm-crm.xml").getProperty("base.InterfaceUser"));
		return userId;
	}

	public String getTaskBeanId() {
		// TODO Auto-generated method stub
		return "iresourceaffirmTaskManager";
	}
	...
	public Map initMap(Map map, List attach, String type) throws Exception {
		try{
			...
			map = this.loadDefaultMap(map, "config/resourceAffirm-crm.xml", type);
			...
			if(type.equals(InterfaceConstants.WORKSHEET_NEW)||type.equals(InterfaceConstants.WORKSHEET_RENEW)){
				String sendRoleId = StaticMethod.nullObject2String(XmlManage.getFile("/config/resourceAffirm-crm.xml").getProperty("base.SendRoleId"));
...
//				map.put("extendKey2", Boolean.TRUE);
				...
				String bigRole = StaticMethod.nullObject2String(XmlManage.getFile("/config/resourceAffirm-crm.xml").getProperty("base.AcceptGroupId"));
				String toDeptId = StaticMethod.nullObject2String(map.get("toDeptId"));
				
				WPSEngineServiceMethod sm = new WPSEngineServiceMethod();
				String subRoleId = "";
				TawSystemSubRole subRole = sm.getMatchRoles("ResourceAffirmProcess", toDeptId, bigRole, map);
				if(subRole==null||subRole.getId()==null||subRole.getId().length()==0){
					System.out.println("未找到匹配角色，使用默认角色");
					subRoleId = XmlManage.getFile("/config/resourceAffirm-crm.xml").getProperty("base.AcceptRoleId");
				}else{
	...
	public void finishNew(BaseMain main, Map interfaceMap) {
		...
		try {
			...
			IResourceAffirmTaskManager taskservice = (IResourceAffirmTaskManager) ApplicationContextHolder
...
//			String  templateName = (String) interfaceMap.get("sheetType");
			...
			IResourceAffirmLinkManager linkservice = (IResourceAffirmLinkManager)ApplicationContextHolder.getInstance().getBean("iResourceAffirmLinkManager");
//			确认受理
			ResourceAffirmLink confirmBean = new ResourceAffirmLink();
