...
_p.getRowHtml = function () {
	...
	return "<div class=\"" + this.getRowClassName() + "\" style=\"padding-left:" +
		...
		this.getIconHtml() +
        this.getCheckboxHtml(this.getTree().checkboxmode)+ // add by mios 070518
		this.getLabelHtml() +
		//"</span>" +
		"</div>";
};
...
//add by mios 070518
_p.getCheckboxHtml = function(c){
		if(c=="true" || c=="single" || c=="multi"){
			this.setAction("#");
		}
		else{
			return "";
        ...
        if(c=="true" || c=="single"){
          return "<input onclick=\"javascript:docheck(this,'"+this.text+"','"+this.id+"','"+
          	t.showField+"','"+t.hiddField+"')\" type=\"checkbox\" class=\"webfx-tree-item-checkbox\">";
        }
        else if(c=="multi"){
          return "<input onclick=\"javascript:doMultiCheck(this,'"+this.text+"','"+this.id+"','"+
          	t.showField+"','"+t.hiddField+"')\" type=\"checkbox\" class=\"webfx-tree-item-checkbox\">";
        }
        else{
          return "";
 ...
 * 
 */
function docheck(el,text, id, showFieldId, hiddFieldId){
        var showField = document.getElementById(showFieldId);
        var hiddField = document.getElementById(hiddFieldId);
        if(!showField || !hiddField){
          alert("no save fields!");
          return;
        }
        if(el.checked){
        ...
        if(el!=cbCurEl){
          if(cbCurEl)
            cbCurEl.checked = false;
          cbCurEl = el;
        }
}

var cbMultiText = new Array();
var cbMultiId = new Array();

/* checkbox多选
 * 此模式下，没有src属性或src属性为空的，以及没有子节点的节点将可由checkbox框进行多选操作。
 * @return 返回的文本与id的两个字符串，都是由“,”号隔开的。
 */
function doMultiCheck(el,text, id, showFieldId, hiddFieldId){
        var showField = document.getElementById(showFieldId);
        var hiddField = document.getElementById(hiddFieldId);
        if(!showField || !hiddField){
          alert("没有找到指定的表单域。");
        ...
        if(el.checked){
          if (cbMultiId.indexOf(id)==-1) {
          	cbMultiText.push(text);
          	cbMultiId.push(id);
          }
        }
        else{
          if (cbMultiId.indexOf(id)>=0) {
          	cbMultiText.remove(text);
          	cbMultiId.remove(id);
          }
        }
        showField.value = cbMultiText.toString();
        hiddField.value = cbMultiId.toString();
}
...
_p.getLabelHtml = function () {
	...
	var target = this.getTree().target || this.getTarget();
	/* modified by mios >> */
...
_p._onmousedown = function (e) {
	...
	this.select();
	//window.status = "text:"+this.text+" type:"+this.type+" data:"+this.data;
...
_p._onclick = function (e) {
	...
	if (/webfx-tree-expand-icon/.test(el.className) && this.hasChildren()) {
        ...
        //checkbox add by mios 070518
	if (/webfx-tree-item-checkbox/.test(el.className)) {
		return true;
	}
	...
	if (typeof this.action == "function") {
		...
		return true;
		//window.open(this.action, this.getTree().target || "_self");
	}
	return false;
};
...
function WebFXTreeItem(sText, oAction, eParent, sIcon, sOpenIcon, sType, sData) {
	WebFXTreeAbstractNode.call(this, sText, oAction);
	if (sIcon) this.icon = sIcon;
	if (sOpenIcon) this.openIcon = sOpenIcon;
