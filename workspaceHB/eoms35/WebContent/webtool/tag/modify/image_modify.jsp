...
<form name="form1" action="<%=app %>/webtool/tag" onSubmit="return checkInput()">
    ...
    <table class="form_table">
        ...
        <tr>
            <th>图片名称：</th>
            <td>
                <input id="text0" name="alt" type="text" size="30" maxlength="50" value="<%=tag.getAlt()%>"><font
                    color="#FF0000">*</font>
                <!--
        ...
        <tr>
            ...
            <td>
                <select name="type" onchange="javascript:addUpload(form1.type.value)">
                    ...
                    <option value="line" <%if (tag.getType().equals("line")) out.println("selected"); %>> 网管数据线图
                    </option>
                    <option value="pole" <%if (tag.getType().equals("pole")) out.println("selected"); %>> 网管数据柱图
                    </option>
                </select>
            </td>
        </tr>
...
<%if (tag.getImageType().equals("auto")) { %>
    <table class="form_table"> 
      <tr>
        <th>现图片<img alt="没有上传图片" src="<%=request.getRealPath("/")+tag.getSrc()%>" width="<%=tag.getWidth()%>"
