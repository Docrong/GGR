...
<c:url var="urlDeal" value="/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do">
...
function initPage(){
	...
	if("${taskName}"!="cc"&&"${operateType}"!='4'&&"${operateType}"!='11'&&"${operateType}"!='55'){
		v.preCommitUrl = "${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=performPreCommit";
	}
}
...
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {	
   		strUrl = '${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
  ...
  function changeType2(){
   ...
   function saveDealInfo(){
    ...
    Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=performSaveInfo",
				success: function(){
		        	alert("保存成功！");
		 		}
		    });
   	...
   	if (confirm("确认保存模板吗？")) {
	    if (type == "new"){
		   	Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "commonfaultcorrigendum.do?method=saveDealTemplate&operateType=${operateType}",
				success: function(){
		        	alert("保存模板成功！");
		 		}
		    });
	   		...
	   		Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "commonfaultcorrigendum.do?method=saveDealTemplate&dealTemplateId=${dealTemplateId}&operateType=${operateType}",
				success: function(){
		        	alert("保存模板成功！");
		 		}
		    });
    ...
    var ajaxForm = Ext.getDom(form);
   	...
   	v.passing = false;
	   	...
	   	if (templateManage == "templateManage") {
	   		form.action = "${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=saveTemplate&templateId=${templateId}&type=${type}";
	   		form.submit();
	   	} else {
		   	if (type == "new"){
			   	Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "commonfaultcorrigendum.do?method=saveTemplate&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
		   		...
		   		Ext.Ajax.request({
			   		form: ajaxForm,
					method:"post",
					url: "commonfaultcorrigendum.do?method=saveTemplate&templateId=${dealTemplateId}&type=${type}",
					success: function(){
			        	alert("保存模板成功！");
			 		}
			    });
...
<div id="sheetform">
  <html:form action="/commonfaultcorrigendum.do?method=performDeal" styleId="theform">
	...
	<table>
	  ...
	  <c:if test="${operateType != '' && operateType != '18' && operateType != '-10'&& operateType != '4'&& operateType != '61'&& operateType != '46'&& taskName != 'RejectTask'&& taskName != 'DraftTask'}">
		...
		<input type="hidden" name="dictKey" value="dict-sheet-commonfaultcorrigendum"/>
   	  </c:if>
	</td>
	</tr>
	</table>
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">    
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftTask'}">    
		...
		<html:submit styleClass="btn" property="method.save"  styleId="submitone" onclick="javascript:changeType1();">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
   	<c:when test="${taskName=='RejectTask'}">    
		<div class="form-btns" id="btns">
		<html:submit styleClass="btn" property="method.save"  styleId="submitone" onclick="javascript:changeType3();">
		 <bean:message bundle="sheet" key="button.send"/>
          ...
          <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/commonfaultcorrigendum/commonfaultcorrigendum.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	    </div>	 
   	</c:when>
    <c:otherwise>  
	    <!-- buttons -->
