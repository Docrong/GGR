...
<caption><div class="header center">存量信息</div></caption>
...
<display:table name="ipspeciallineList" cellspacing="0" cellpadding="0"
    id="ipspecialline" pagesize="15" class="table ipspeciallineList"
     export="true" requestURI="/businessupport/order/ipspeciallines.do?method=showDetail" sort="list" > 
     	<display:column property="businessBandwidth" headerClass="sortable" title="业务带宽"  url="/businessupport/product/ipspeciallines.do?method=showDetail&flag=showDetail&ifReference=${ifReference}" paramId="id" paramProperty="id"/>
...
<tr class="buttonBar bottom">
     <input type="button"  styleClass="button"
            onclick="location.href='<c:url value="/businessupport/product/ipspeciallines.do?method=xedit&type=add&ifReference=${ifReference}"/>'" 
            value="添加"/>
  <logic:present name="back" scope="request">
     <input type="button"  styleClass="button"
            onclick="location.href='<c:url value="/businessupport/product/ipspeciallines.do?method=xquery&ifReference=${ifReference}"/>'" 
            value="返回"/>
  </logic:present>
</tr>
</html:form>
