...
<%@ page
	import="com.boco.eoms.base.util.StaticMethod,com.boco.eoms.duty.util.DutyMgrLocator"%>
<%
...
<HTML>
	...
	<BODY
		onLoad="javascript:TANGER_OCX_OpenDoc2('<%=basePath+"/"+request.getParameter("path")%>')">
		<%

		%>
		<FORM id="myForm" METHOD="POST" ENCTYPE="multipart/form-data"
			ACTION="uploadDetail.jsp?filePath=<%=path%>">
				...
				<tr>
					<td align="left">
						<input TYPE=hidden id="filename" name="filename" MAXLENGTH=50
							size=50 value="<%=request.getParameter("path")%>">
						<INPUT type=BUTTON VALUE=${eoms:a2u("保存")} class="button"
							onclick="TANGER_OCX_SaveEditToServer('<%=request.getParameter("path")%>');">
						<font color="red">${eoms:a2u("如果需要保存您填写的内容，请点击左侧的[保存]按钮")} </font>
					</td>
				</tr>
			</TABLE>
		</form>
		...
		<object id="TANGER_OCX"
			...
			codebase="OfficeControl.ocx#version=3,0,0,7" width="100%"
			height="100%">
