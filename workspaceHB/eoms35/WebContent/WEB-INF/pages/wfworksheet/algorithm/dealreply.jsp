...
<html:form action="/algorithm.do?method=batchPerformDeal" styleId="theform">
<%@ include file="/WEB-INF/pages/wfworksheet/algorithm/baseinputlinkhtmlnew.jsp"%>
     ...
     <input type="hidden" name="${sheetPageName}processTemplateName" id="${sheetPageName}processTemplateName" value="AlgorithmMainFlowProcess" />
     ...
     <input type="hidden" name="${sheetPageName}toOrgRoleId" id="${sheetPageName}toOrgRoleId" value="${toTask.taskOwner}"/> 
     <table  class="formTable">
     <tr>
        <td class="label"><bean:message bundle="sheet" key="phase.pleaseSelect" /></br>
        <input type="checkbox" name="${sheetPageName}checkall" id="${sheetPageName}checkall" value="${toTask.taskOwner}" onclick="selfcheckall();eoms.util.checkAll(this, '${sheetPageName}deal')" >
           全选</br>
        <input type="checkbox" name="${sheetPageName}checkbackall" id="${sheetPageName}checkbackall" value="${toTask.taskOwner}" onclick="selfcheckbackall();selfx()" >
           反选
      ...
      <logic:iterate id="toTask" name="mapList" type="java.util.HashMap">  
        <tr>
          <td>
              <input type="checkbox" name="${sheetPageName}deal" id="${sheetPageName}deal" value="${toTask.operateUserId}" onclick="check()">
              <eoms:id2nameDB id="${toTask.operateUserId}" beanId="tawSystemDeptDao" /><eoms:id2nameDB id="${toTask.operateUserId}" beanId="tawSystemUserDao" />&nbsp;
              <input type="hidden" name="${sheetPageName}toid" id="${sheetPageName}toid" value="${toTask.id}" />
          </td>
           <td>
              <eoms:id2nameDB id="${toTask.operateDeptId}" beanId="tawSystemDeptDao" /><eoms:id2nameDB id="${toTask.operateDeptId}" beanId="tawSystemUserDao" />&nbsp;
          </td>
          <td>
