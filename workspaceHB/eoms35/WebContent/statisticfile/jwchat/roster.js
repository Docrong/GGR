...
function RosterSendMessage(jid) {
    var user = this.getUserByJID(jid);
    var wName = makeWindowName(user.jid);
    var userid = jid.substring(0, jid.indexOf("@"));
    //alert(userid);
    var url = '../tawUserAddressBook/tawUserAddressBooks.do?method=sendSM&userId=' + userid + '&userName=' + user.name;
    window.open(url, '', 'channelMode, menubar=no, toolbar=no, location=no, directories=no, status=no, resizable=no, scrollbars=no, width=450, height=210, fullscreen=no');
...
function Roster(items, targetW) {
    ...
    this.sendMessage = RosterSendMessage;
    ...
    for (var i = 0; i < this.groups.length; i++) {
        ...
        if (groupEl) { // update group
            ...
            var onclickHandler = function (group) {
                ...
                var toggler = function () {
                    //parent.top.roster.toggleGrp(groupName); 
                    parent.parent.roster.toggleGrp(groupName);
                };
                return toggler;
            }
...
function RosterUserClicked(el, jid) {
    ...
    if (this.name == 'GroupchatRoster') {
        ...
        return parent.parent.user.roster.openChat(jid);
    }

    //var user = parent.top.roster.getUserByJID(parent.top.cutResource(jid));
    var user = parent.parent.roster.getUserByJID(parent.parent.cutResource(jid));

    if (user && typeof (user.type) != 'undefined' && user.type == 'groupchat')
    //return parent.top.openGroupchat(jid);
        return parent.parent.openGroupchat(jid);

    //if (!parent.top.isGateway(jid))
    //	return parent.top.roster.openChat(jid);
    if (!parent.parent.isGateway(jid))
        return parent.parent.roster.openChat(jid);
}
