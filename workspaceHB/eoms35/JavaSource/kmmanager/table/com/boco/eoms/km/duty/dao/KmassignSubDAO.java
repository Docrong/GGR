...
public class KmassignSubDAO extends DAO  {

  public KmassignSubDAO(com.boco.eoms.db.util.ConnectionPool ds) {
    super(ds);
  }
  ...
  public Vector getDutymanVec(int workserial) throws SQLException {
    ...
    try {
      ...
      String sql = "SELECT dutyman,id FROM km_assign_sub WHERE workserial=? order by id";
  ...
  public String getDutymanStr(int workserial) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT a.dutyman,b.username FROM km_assign_sub a,taw_system_user b WHERE workserial=? and b.userid=a.dutyman";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, workserial);
      rs = pstmt.executeQuery();
      while (rs.next()) {
  ...
  public void update_changeduty(int workserial,String dutyman_from,String dutyman_to) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "UPDATE km_assign_sub SET dutyman=? WHERE workserial=? and dutyman=? ";
      pstmt = conn.prepareStatement(sql);
  ...
  public void delete_room_date(int roomId,String dutydate) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "DELETE FROM km_assign_sub WHERE workserial in (SELECT id FROM km_assignwork where room_id=? and dutydate=?)";
      pstmt = conn.prepareStatement(sql);
  ...
  public void insert(int workserial,String dutyman) throws SQLException {
    String sql;
    sql = "INSERT INTO km_assign_sub ( workserial, dutyman) VALUES (?, ?)";
    com.boco.eoms.db.util.BocoConnection conn = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try {
  ...
  public void insert(TawRmAssignSub tawRmAssignSub) throws SQLException {
    String sql;
    sql = "INSERT INTO km_assign_sub (id, workserial, dutyman, notes) VALUES (?, ?, ?, ?)";
    com.boco.eoms.db.util.BocoConnection conn = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try {
  ...
  public void update(TawRmAssignSub tawRmAssignSub) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "UPDATE km_assign_sub SET workserial=?, dutyman=?, notes=? WHERE id=?";
      pstmt = conn.prepareStatement(sql);
  ...
  public void delete(int id) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "DELETE FROM km_assign_sub WHERE id=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, id);
      pstmt.executeUpdate();
      close(pstmt);
  ...
  public TawRmAssignSub retrieve(int id) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT * FROM km_assign_sub WHERE id=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, id);
      rs = pstmt.executeQuery();
      if (rs.next()) {
  ...
  public List list() throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT workserial, dutyman, notes, id FROM km_assign_sub";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      while(rs.next()) {
        tawRmAssignSub = new TawRmAssignSub();
  ...
  public List getUserIdListByWorkSerial(String workSerial) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT dutyman FROM km_assign_sub where workserial='"+workSerial+"' group by dutyman";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      while(rs.next()) {
  ...
  public boolean getUserIdEqualsWorkSerial(String workSerial,String userid) throws SQLException {
	   boolean bool = false;
	    ...
	    try {
	      conn = ds.getConnection();
	      String sql = "SELECT count(*) as cn FROM km_assign_sub where workserial='"+workSerial+"'  and dutyman = '"+userid+"'";
	      pstmt = conn.prepareStatement(sql);
	      rs = pstmt.executeQuery();
	      while(rs.next()) {
	    	 int i = rs.getInt("cn");
