...
<caption>
    <div class="header center">传输专线列表</div>
</caption>
<html:form action="transferspeciallines.do?method=xsearch" method="post" styleId="transferspeciallineForm">
    ...
    <display:table name="transferspeciallineList" cellspacing="0" cellpadding="0"
                   id="transferspecialline" pagesize="15" requestURI="transferspeciallines.do?method=xquery"
                   class="table transferspeciallineList"
                   export="false" sort="list">
        <display:column property="cityA" headerClass="sortable" title="城市A"
                        url="/businessupport/product/transferspeciallines.do?method=showDetail&flag=showDetail&back=true"
    ...
    <tr class="buttonBar bottom">
        <input type="button" styleClass="button"
               onclick="location.href='<c:url
                       value="/businessupport/product/transferspeciallines.do?method=xedit&type=add"/>'"
               value="添加"/>
        <logic:present name="back" scope="request">
            <input type="button" styleClass="button"
                   onclick="location.href='<c:url
                           value="/businessupport/product/transferspeciallines.do?method=xquery"/>'"
                   value="返回"/>
        </logic:present>
