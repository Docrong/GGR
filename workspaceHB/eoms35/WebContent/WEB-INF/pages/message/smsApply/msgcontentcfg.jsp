		...
		<tr>
			...
			<td>
 			<eoms:comboBox name="flowName" id="flowName" 
        	      initDicId="1017702"  alt="allowBlank:false" defaultValue="${flowId}" styleClass="select-class" />				
			</td>
			<td class="label">
				提醒类型：
			</td>
			<td>
 			<eoms:comboBox name="noticeType" id="noticeType" 
        	      initDicId="1017701"  alt="allowBlank:false" defaultValue="${noticeType}" styleClass="select-class" />				
			</td>
		</tr>
		<tr>
			<td colspan="4">
...
<html:form action="/smsApplys.do?method=saveMsgContent" method="post" styleId="">
	<table >
	  ...
	  <%}%>
		<td width="200">
			<input type="checkbox" name="<%=subList.get(0)%>" id="<%=subList.get(0)%>" value="<%=subList.get(0)%>" onclick="changeContent(this)"><%=subList.get(1)%>
		</td>
	  <%if(i%5==4){ %>
	  </tr>
	  <%} %>
	...
	<br>
	<textarea class="textarea max" name="chContent" id="chContent" size="200" width="200" onmousedown=>${content}</textarea>	
