	    ...
	    <%if(taskName.equals("MakeTask")){ 
     	 ...
     	 <input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="<%=operateType%>" />	
	             ...
	             <tr>
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkSecurityInproveAdvice"/>*</td>
		              <td colspan="3"> 	
    				  <textarea class="textarea max" name="${sheetPageName}linkSecurityInproveAdvice" value="${sheetLink.linkSecurityInproveAdvice}" id="${sheetPageName}linkSecurityInproveAdvice" alt="allowBlank:false,maxLength:500,vtext:'请填入信息，最多输入250字'">${sheetLink.linkSecurityInproveAdvice}</textarea>
                    </td>
		          </tr>
	              <tr>
  						<td class="label"><bean:message bundle="securitydeal" key="securitydeal.linkSecurityInproveProgram"/></td>
		   				 <td  colspan='3'>
		   		          <eoms:attachment name="sheetLink" property="nodeAccessories" 
            		          scope="request" idField="${sheetPageName}nodeAccessories" appCode="securitydeal" alt="allowBlank:true"/> 
		             </td>
		          </tr>
		     <%}} %>
	        ...
	        <input type="hidden" name="${sheetPageName}operateType" id="${sheetPageName}operateType" value="<%=operateType %>" />
	          ...
	          <tr>
  						<td class="label"><bean:message bundle="securitydeal" key="securitydeal.linkAuditResult"/>*</td>
		   				 <td  colspan='3'>
		   		          <textarea class="textarea max" name="${sheetPageName}linkAuditResult" id="${sheetPageName}linkAuditResult" alt="allowBlank:false,maxLength:500,vtext:'请填入信息，最多输入250字'">${sheetLink.linkAuditResult}</textarea>
		                 </td>
		      </tr>
	     ...
	     <%} } %> 
	      <%if(taskName.equals("PerformTask")&&(operateType.equals("92")|| operateType.equals("11"))){%>
              ...
              <tr>
  						<td class="label"><bean:message bundle="securitydeal" key="securitydeal.linkPerformResult"/>*</td>
		   				 <td  colspan='3'>
		   		          <textarea class="textarea max" name="${sheetPageName}linkPerformResult" id="${sheetPageName}linkPerformResult" alt="allowBlank:false,maxLength:500,vtext:'请填入信息，最多输入250字'">${sheetLink.linkPerformResult}</textarea>
		      ...
		      <tr >
		           <!-- 是否启动网络变更配置工单 -->
		            <td  class="label"><bean:message bundle="securitydeal" key="securitydeal.linkIfStartChangeProcess"/>*</td>
            	      ...
            	      initDicId="1011302"  alt="allowBlank:false" styleClass="select-class" defaultValue="${sheetLink.linkIfStartChangeProcess}" onchange="popOtherFlow(this.value);"/>
                     <html:button styleClass="btn" style="display:none" property="method.save" styleId="startCircuit" onclick="openwin('101130401')">电路调度工单</html:button>
                     <html:button styleClass="btn" style="display:none" property="method.save" styleId="startSoftChange" onclick="openwin('101130402')">软件变更工单</html:button>
                     <html:button styleClass="btn" style="display:none" property="method.save" styleId="startNetChange" onclick="openwin('101130404')">网络综合调整工单</html:button>
                     <html:button styleClass="btn" style="display:none" property="method.save" styleId="startNetData" onclick="openwin('101130403')">网络数据管理工单</html:button>
                    </td>
		      </tr>
		  ...
		  <tr>
		    ...
		    <td colspan="3">
		      <textarea name="${sheetPageName}endResult" alt="allowBlank:false,maxLength:500,vtext:'请填入信息，最多输入250字'" id="${sheetPageName}endResult"  class="textarea max">${sheetMain.endResult}</textarea>
		    </td>
		  </tr>    
	     <%} }%>
	      <!--流程中的字段域 结束-->
