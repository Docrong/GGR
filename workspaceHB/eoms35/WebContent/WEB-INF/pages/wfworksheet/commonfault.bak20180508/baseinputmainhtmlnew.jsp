...
<table class="formTable">
    ...
    <tr>
        <td><bean:message bundle="sheet" key="mainForm.sendRoleName"/>*</td>
        <td>
        ...
        <td><bean:message bundle="commonfault" key="commonfault.mainIsPack"/>*</td>
        <td>
            <eoms:comboBox name="${sheetPageName}mainIsPack" id="${sheetPageName}mainIsPack"
                           initDicId="10301" defaultValue="${sheetMain.mainIsPack}" alt="allowBlank:true"
                           onchange="changeMode(this.value);"/>
            <c:if test="${taskName=='DraftHumTask'}">
            <script language="javascript>
		   var packFlag=document.getElementById(" mainIsPack
            ");
            packFlag.disabled="disabled";
            </script>
            </c:if>
            <
            /td>
            ...
            < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.sendContact"/> * < /td>
                < td
            colspan = "3" >
                < input
            type = "text"
            class
            = "text"
            name = "${sheetPageName}sendContact"
            id = "${sheetPageName}sendContact"
            value = "${sheetMain.sendContact}"
            alt = "allowBlank:false" / >
                < input
            type = "hidden"
            name = "${sheetPageName}sendTime"
            id = "${sheetPageName}sendTime"
            value = "${eoms:date2String(sheetMain.sendTime)}" / >
                ...
                < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.endUserName"/> < /td>
                < td >
                < input
            type = "hidden"
            name = "${sheetPageName}endUserId"
            value = "${endUserId}" / >
                < input
            type = "hidden"
            name = "${sheetPageName}showDept"
            id = "${sheetPageName}showDept"
            value = "<eoms:id2nameDB id="${sheetMain.toDeptId}" beanId="tawSystemDeptDao"/>" / >
                < input
            type = "hidden"
            name = "${sheetPageName}toDeptId"
            id = "${sheetPageName}toDeptId"
            value = "${sheetMain.toDeptId}" / >
                <eoms:id2nameDB id="${endUserId}" beanId="tawSystemUserDao"/> & nbsp;
            <
            /td>
            < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.endDeptName"/> < /td>
                < td >
                < input
            type = "hidden"
            name = "${sheetPageName}endDeptId"
            value = "${endDeptId}" / >
                <eoms:id2nameDB id="${endDeptId}" beanId="tawSystemDeptDao"/> & nbsp;
            <
            /td>
            < /tr>
            ...
            < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.endRoleName"/> < /td>
                < td
            colspan = "3" >
                < input
            type = "hidden"
            name = "${sheetPageName}endRoleId"
            value = "${endRoleId}" / >
                <eoms:id2nameDB id="${endRoleId}" beanId="tawSystemSubRoleDao"/> & nbsp;
            <
            /td>
            < /tr>
            ...
            < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.holdStatisfied"/> < /td>
                < td
            colspan = "3" >
                ...
                < td
            class
            = "label" > <bean:message bundle="sheet" key="mainForm.endResult"/> < /td>
                < td
            colspan = "3" >
                < textarea
            name = "${sheetPageName}endResult"
            id = "${sheetPageName}endResult"
            alt = "width:'500px',allowBlank:false" > < /textarea>
                < /td>
                < /tr>
            ...
            name = "${sheetPageName}endUserId"
            value = "${endUserId}" / >
                <eoms:id2nameDB id="${sheetMain.endUserId}" beanId="tawSystemUserDao"/> & nbsp;
            <
            /td>
            < td
            class
