...
<logic:present name="HISTORY" scope="request">
      <logic:iterate id="baselink" name="HISTORY" type="com.boco.eoms.sheet.safeaudit.model.SafeAuditLink">  
       <%         
        jNum += 1;       
      ...
      <div class="history-item"><!-- add space to hack ie-->&nbsp;
       <div class="history-item-title">
         ...
         <bean:write name="baselink" property="operateTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="page"/>&nbsp; 
  	    ...
  	    <eoms:dict key="dict-sheet-safeaudit" dictId="activeTemplateId" itemId="${baselink.activeTemplateId}" beanId="id2descriptionXML" />
  	     <bean:message bundle="safeaudit" key="safeaudit.operateType"/>:
  	     <eoms:dict key="dict-sheet-safeaudit" dictId="mainOperateType" itemId="${baselink.operateType}" beanId="id2descriptionXML" />
         ...
         <bean:message bundle="sheet" key="common.copy"/>	     
  		 ...
  		 <table class="history-item-table" width="100%" cellpadding="0" cellspacing="0">
               ...
               <%if(baselink.getNodeAcceptLimit()!= null && ! baselink.getNodeAcceptLimit().equals("")
                ...
                <%} %>
		  	   <!-- 提交材料 -->
		  	 <%if(baselink.getActiveTemplateId()!=null&& baselink.getActiveTemplateId().equals("EvaluateMaterialHumTask")&&(baselink.getOperateType().intValue()==90|| baselink.getOperateType().intValue()==11)){%> 	

	           <tr>
		  
                    <td  class="label"><bean:message bundle="safeaudit" key="safeaudit.linkPresentDescribe"/></td>
		              <td colspan="3"> 	
    				  <pre>${baselink.linkPresentDescribe}</pre>
                    </td>
		        </tr> 
		           <tr>
                    ...
                    <td  class="label"><bean:message bundle="safeaudit" key="safeaudit.linkPresentReport"/></td>
		              <td colspan="3"> 	
    				  <pre>${baselink.linkPresentReport}</pre>
                    </td>
		          
		          </tr> 
		                   		 
		  	 <%}%>
		  	 <%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("ApprovingHumTask")&& (baselink.getOperateType().intValue()==921||baselink.getOperateType().intValue()==922|| baselink.getOperateType().intValue()==55)){%> 	 
		  	 <tr>
		            <td  class="label"><bean:message bundle="safeaudit" key="safeaudit.linkCheckIdeas"/></td>
		              <td class="column-content" colspan="3"> 	
    				 <eoms:id2nameDB id="${baselink.linkCheckIdeas}" beanId="ItawSystemDictTypeDao"/>
                    </td>
             </tr>
             <tr>
                    <td  class="label"><bean:message bundle="safeaudit" key="safeaudit.linkCheckResult"/></td>
		              <td class="column-content" colspan="3"> 	
    				  <pre><bean:write name="baselink" property="linkCheckResult" scope="page"/></pre>
                    </td> 
		      </tr> 
		  	 <%}%>
		  	  <%if(baselink.getActiveTemplateId()!=null&& baselink.getActiveTemplateId().equals("EditReportHumTask")&&(baselink.getOperateType().intValue()==91|| baselink.getOperateType().intValue()==11)){%> 	 

		  	 <tr>
		            <td  class="label"><bean:message bundle="safeaudit" key="safeaudit.linkAuditReport"/></td>
		   				 <td  colspan='3'>
		   				 <eoms:attachment  name="baselink" property="linkAuditReport"  scope="page"
		   				 idField="${sheetPageName}linkAuditReport" appCode="safeaudit" 
		   				 viewFlag="Y"/>
		                </td>
             </tr>
            <%}%>
...
<script type="text/javascript">
 ...
 function copy(id){
    ...
    Ext.Ajax.request({
		method:"get",
		url: "safeaudit.do?method=getJsonLink&id="+id+"&beanName=SafeAudit",
		success: function(x){
