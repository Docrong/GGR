			  ...
			  <td class="content">
			    <input type="text" class="text" name="${sheetPageName}sheetAcceptLimit" readonly="readonly" 
					id="${sheetPageName}sheetAcceptLimit" value="${eoms:date2String(sheetMain.sheetAcceptLimit)}" 
					 alt="vtype:'lessThen',link:'${sheetPageName}sheetCompleteLimit',vtext:'受理时限不能晚于处理时限',allowBlank:false" onclick="popUpCalendar(this, this)"/> 
            ...
            <tr>
			  <td class="label"><bean:message bundle="netOptimize" key="netOptimize.mainOptimizeDemand"/>*</td>
			   <td class="content" colspan="3">
			     <textarea name="${sheetPageName}mainOptimizeDemand" id="${sheetPageName}mainOptimizeDemand" class="textarea max"  alt="allowBlank:false,maxLength:200,vtext:'请输入专题优化需求，最大长度为200个汉字！'">${sheetMain.mainOptimizeDemand}</textarea>			   
			  </td>				
		  ...
		  <tr>
			...
			<td class="content" colspan="3">					
			   <eoms:attachment name="sheetMain" property="mainOptmizeScheme" 
		            scope="request" alt="allowBlank:false" idField="${sheetPageName}mainOptmizeScheme" appCode="netOptimize" /> 				
		    </td>
		  </tr>										
		</c:if>
   ...
   <c:if test="${status!=1}">	  
	<fieldset id="link1">
	 <legend>
	 	 ...
	 	 <eoms:chooser id="test" type="role" roleId="361" flowId="046" config="{returnJSON:true,showLeader:true,mainPanelTitle:'可选择的子角色'}"
