...
<form name=editsysform action="" method="post">
    <table bgcolor=#dddddd width='100%' cellspacing="1" cellpadding="0" border="0">
        <tr bgcolor=#c6c6c6>
            ...
            <td align=center><font size=2 color=#000000>³¤¶È(½öÊÊÓÃvarchar¡¢nchar)</font></td>
            <td align=center><font size=2 color=#000000>ÊÇ·ñ¿ÉÎª¿Õ</font></td>
            <td align=center><font size=2 color=#000000>ÊÇ·ñ²Î¿¼ÁÐ</font></td>
            <td align=center><font size=2 color=#000000>ÊÇ·ñÔÚÁÐ±íÖÐÏÔÊ¾</font></td>
            <td align=center><font size=2 color=#000000>ÊÇ·ñÖ»¶ÁÁÐÁÐ</font></td>
        </tr>
        <tr bgcolor=#eeeeee>
            <td align=center><font size=2 color=#000000></font></td>
            <td align=center><input type=text name=fieldname value='<%=SysColIndex.getCc_name()%>'><input type=hidden
                                                                                                          name=cc_code
                                                                                                          value='<%=SysColIndex.getCc_code()%>'>
            </td>
                ...
                <%
                    if (!SysOpt.tabIsData(Integer.parseInt(tabid))) {
                        if (SysColIndex.getCi_nul_flag() == 1)
                            out.println("<select name=nulflag><option value=0 selected>ÊÇ</option><option value=1>·ñ</option>");
                        else
                            out.println("<select name=nulflag><option value=0>ÊÇ</option><option value=1 selected>·ñ</option>");
                    } else
                        out.println("<input type=hidden name=nulflag value='0'><select name='' disabled><option selected>ÊÇ</option>");
                %>

                </select></td>
            <td align=center><select name=refflag><%
                if (SysColIndex.getCi_ref_flag() == 1)
                    out.println("<option value=1 selected>ÊÇ</option><option value=0>·ñ</option>");
                else
                    out.println("<option value=1>ÊÇ</option><option value=0 selected>·ñ</option>");
            %></select></td>
                ...
                <table>
                    <tr>
                        <td width=60 align=center><a href="javascript:formsubmit()"><img
                                src="../images/button_submit.gif" border=0></a></td>
                        <td width=20></td>
                        <td width=60 align=center><a href='javascript:window.close()'><font size=2>¹Ø±Õ</font></a></td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>
    <input type="hidden" name="tabid" value='<%=tabid%>'><input type="hidden" name="id" value='<%=id%>'><input
        type="hidden" name="ot" value='2'></input>
</form>
<Script Language=JavaScript>
    function formsubmit() {
        if (allTrim(editsysform.fieldname.value) < 1) {
            alert('ÄúÃ»ÓÐÊäÈë×Ö¶ÎÃûÐÅÏ¢£¡ºËÊµºóÖØÊÔ');
        } else if (editsysform.fieldtype.value.indexOf("(") != -1) {
            if (!IsNumber(editsysform.len.value) || allTrim(editsysform.len.value) < 1)
                alert('ÄúµÄ×Ö¶Î³¤¶ÈÃ»ÓÐÌîÐ´£¬»òÌîÐ´ÓÐ´íÎó£¡');
