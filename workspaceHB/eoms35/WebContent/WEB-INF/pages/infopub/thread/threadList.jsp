...
<html:form action="/thread.do" method="post" styleId="threadForm">
	...
	<display:table name="threadList" cellspacing="0" cellpadding="0" id="threadList" pagesize="${pageSize }" class="table threadList" export="true" requestURI="${app}/workbench/infopub/thread.do?method=list&forumsId=${forumsId }" sort="list"
		...
		<display:column property="title" sortable="true" headerClass="sortable" titleKey="infopub.threadForm.title" paramId="id" paramProperty="id" href="${app}/workbench/infopub/thread.do?method=detail" />
		
		<display:column sortable="true" headerClass="sortable" titleKey="infopub.threadForm.status" href="${app}/workbench/infopub/threadAuditHistory.do?method=device" paramId="threadId" paramProperty="id" >
			<eoms:dict key="dict-workbench-infopub" dictId="auditStatus" itemId="${threadList.status}" beanId="id2nameXML" />
		</display:column>
		<display:column property="threadCount" sortable="true" headerClass="sortable" titleKey="infopub.threadForm.num" />
		<display:column property="createrName" sortable="true" headerClass="sortable" titleKey="infopub.threadForm.createrId" />

		<display:column property="createTime" sortable="true" headerClass="sortable" titleKey="infopub.threadForm.createTime" format="{0,date,yyyy-MM-dd HH:mm:ss}" />

		<display:column property="editTime" sortable="true" headerClass="sortable" titleKey="infopub.threadForm.editTime" format="{0,date,yyyy-MM-dd HH:mm:ss}" />
