...
<fmt:bundle basename="resourceBundle">
    ...
    <c:if test='${requestScope.delicious == null}'>
        <table border='0' cellpadding='0' cellspacing='0'>
            ...
            <tr>
                <td class='portlet-table-td-left'>
                    If you are not a member of del.icio.us, please click <a href="http://del.icio.us/" target="_blank">http://del.icio.us/</a>
                    to register.
                </td>
            </tr>
        </table>
                ...
                <tr>
                    <td class='portlet-table-td-left'>
                        <a href='javascript:void(0);' name="all"
                           onClick="javascript:viewDelicious('<%= responseId %>',this.name);">All</a>
                        <c:forEach var="tag" items="${requestScope.tags}">
                            <a href='javascript:void(0);' name="<c:out value='${tag.tag}'/>"
                               onClick="javascript:viewDelicious('<%= responseId %>',this.name);"><c:out
                                    value="${tag.tag}"/></a>
                        </c:forEach>
                    </td>
                </tr>
                ...
                <c:forEach var="delicious" items="${requestScope.delicious}">
                    <tr>
                        <td class='portlet-table-td-left' width='80%'>
                            <a href='<c:out value="${delicious.value}"/>' target='_blank'><c:out
                                    value="${delicious.name}"/></a>
                        </td>
                        <td class='portlet-table-td-right'>
                            <input type="image" src="portal/light/images/edit.gif"
                                   name="<c:out value='${delicious.name}'/>"
                                   onClick="document.pressed='edit';document.parameter=this.name;document.mode='EDIT';"/>
                            <input type="image" src="portal/light/images/deleteLink.gif"
                                   name="<c:out value='${delicious.value}'/>"
