...
<script type="text/javascript">
    ...
    function displayTR(sel) {
        //alert(sel.value);
        //alert('${eoms:a2u("")}');
        if (1 == sel.value || 0 == sel.value)
            document.getElementById("reportIndex").value = sel.value;
        if (sel.value == 0) {
            eoms.form.disableArea("trSelectdept", true);
            eoms.form.enableArea("tr1");
            document.getElementById("findListForward").value = "statisticsheetlist";
            //var listforward = document.getElementById("findListForward").value;
            //alert(listforward);
            ...
            document.getElementById("findListForward").value = "statisticsheetlist2";
            //var listforward = document.getElementById("findListForward").value;
            //alert(listforward);
...
//alert(${"statspecial"}.value);
    ...
    function setDeptType(sel) {
        //var sel = document.getElementById("reportIndex");
    ...
    function checkSubmit() {
        var reportIndex = document.getElementById("reportIndex1").value;
        //alert(reportIndex);
        //var tempreportIndex = reportIndex;
        if (reportIndex == 0)
            return true;
        else {
            var selectDeptType = document.getElementById("selectDeptType").value;
            //alert(reportIndex);
            //alert(selectDeptType);
            if ("senddept" == selectDeptType) {
                var senddept = document.getElementById("sendDeptId").value;
                //alert(senddept=="");
                if (senddept == "") {
                    alert("${eoms:a2u("请选择派单部门")}");
                    return false;
                }
                document.getElementById("reportIndex").value = 2;
                //alert(document.getElementById("reportIndex").value);
                return true;
            } else if ("revdept" == selectDeptType) {
                var recdept = document.getElementById("revDeptId").value;
                //alert(recdept=="");
                if (recdept == "") {
                    alert("${eoms:a2u("请选择受理部门")}");
                    return false;
                }
                document.getElementById("reportIndex").value = 3;
                return true;
            } else {
                var senddept = document.getElementById("sendDeptId").value;
                var recdept = document.getElementById("revDeptId").value;
                //alert("both");
                if (senddept == "" || recdept == "") {
                    alert("${eoms:a2u("请选择派单部门和受理部门")}");
                    return false;
                }
                document.getElementById("reportIndex").value = 1;
                return true;
            }
        }
        return false;
...
<html:form action="/stat.do?method=performStatistic" styleId="theform">
    ...
    <table class="formTable">
        ...
        <tr>
            ...
            <td width="80%">
                <select name="reportIndex1" onchange="displayTR(this)">
                    <%
        ...
        <tr id="trSelectdept">
            ...
            <td id="selboth">
                ...
                <input type="text" name="sendDeptName1" value="" id="sendDeptName1" class="text" readonly="readonly"
                       alt="allowBlank:true">

                <input type="button" value="${eoms:a2u('受理部门')}" id="userTreeBtn3" class="btn"/>
                <input name="revDeptName1" value="" id="revDeptName1" class="text" readonly="readonly"
                       alt="allowBlank:true">
            </td>
        </tr>
    ...
    <html:submit styleClass="btn" property="method.save" styleId="method.save" onclick="return checkSubmit()">
