		...
		<tr>
		    ...
		    <td colspan="3">
				...
				<eoms:comboBox name="${sheetPageName}mainResourceType" id="${sheetPageName}mainResourceType" 
            	      initDicId="1013402"  alt="allowBlank:false" styleClass="select-class" defaultValue="${sheetMain.mainResourceType}"
...
<tr>	
<!-- 分类为HLR的字典值 -->			
<tbody id='notpassreasion' style="display:<c:choose><c:when test="${sheetMain.mainResourceType == '101340201'}">block</c:when><c:otherwise>none</c:otherwise></c:choose>">
			        ...
			        <script type="text/javascript">
				         Ext.onReady(function(){
		                        ...
		                        factoryTree = new xbox({
		    	                    btnId:'${sheetPageName}showFactory',dlgId:'dlg3',
		    	                    treeDataUrl:factoryTreeAction,treeRootId:'1013401',treeRootText:'HLR资源类型',treeChkMode:'',treeChkType:'dict',
		    	                    showChkFldId:'${sheetPageName}showFactory',saveChkFldId:'${sheetPageName}mainHLRResource',viewer:factoryViewer
		                       });
		                 });
                     </script>
                     <textarea class="textarea max" readonly="readonly" name="${sheetPageName}showFactory" style="height:50px" id="${sheetPageName}showFactory"  alt="allowBlank:true,maxLength:250,vtext:'请填入HLR资源类型，最多输入125个汉字'"><c:forTokens items="${sheetMain.mainHLRResource}" delims="," var="mainHLRResource" varStatus="status"><eoms:id2nameDB id="${mainHLRResource}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
                        <input type="hidden" name="${sheetPageName}mainHLRResource" id="${sheetPageName}mainHLRResource" value="${sheetMain.mainHLRResource}"/>			  
      </td>
</tbody>
<!-- 分类为MSC的字典值 -->
<tbody id='passreasion' style="display:<c:choose><c:when test="${sheetMain.mainResourceType == '101340202'}">block</c:when><c:otherwise>none</c:otherwise></c:choose>">
			         ...
			         Ext.onReady(function(){
	                  //viewer
				           ...
				           factoryViewer1.refresh();
    	                    ...
    	                    treeDataUrl:factoryTreeAction,treeRootId:'1013403',treeRootText:'MSC、GMSC资源类型',treeChkMode:'',treeChkType:'dict',
    	                    showChkFldId:'${sheetPageName}showFactory2',saveChkFldId:'${sheetPageName}mainMSCResource',viewer:factoryViewer1
	                       });
	                       });
                     </script>
                     <textarea class="textarea max" readonly="readonly" name="${sheetPageName}showFactory2" style="height:50px" id="${sheetPageName}showFactory2"  alt="allowBlank:true,maxLength:250,vtext:'请填入MSC、GMSC资源类型，最多输入125个汉字'"><c:forTokens items="${sheetMain.mainMSCResource}" delims="," var="mainMSCResource" varStatus="status"><eoms:id2nameDB id="${mainMSCResource}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
                        <input type="hidden" name="${sheetPageName}mainMSCResource" id="${sheetPageName}mainMSCResource" value="${sheetMain.mainMSCResource}"/>			  
                    </td>
</tbody>
</tr>               
...
<tr>
   <td class="label"><bean:message bundle="numberapply" key="numberApplyMain.mainContentDescribe"/>*</td>
	<td colspan="3"> 	
    	<textarea class="textarea max" name="${sheetPageName}mainContentDescribe" id="${sheetPageName}mainContentDescribe" alt="allowBlank:false,maxLength:500,vtext:'请输入申请内容描述，最大长度为255个汉字'">${sheetMain.mainContentDescribe}</textarea>
    </td>
</tr>
<script type="text/javascript">
