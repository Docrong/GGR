        ...
        <c:choose>
            <c:when test="${operateType=='61'}">
                <div class="form-btns" id="btns">
                    <input type="submit" class="submit"
                           onclick="this.form.action='${app}/sheet/commonfault/commonfault.do?method=performClaimTask'"
            ...
            <c:when test="${taskName=='DraftHumTask'}">
                <div class="form-btns" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/commonfault/commonfault.do?method=performSaveInfoAndExit&draft=1'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/commonfault/commonfault.do?method=showListsendundo&draft=1'"
            ...
            <c:when test="${taskName=='BackHumTask'}">
                <div class="form-btns" id="btns">
                    ...
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/complaint/complaint.do?method=performSaveInfoAndExit'"
                           value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                    <input type="submit" class="submit"
                           onclick="v.passing=true;this.form.action='${app}/sheet/complaint/complaint.do?method=showListsendundo'"
                           value="<bean:message bundle='sheet' key='button.back'/>"/>
                </div>
            </c:when>
            <c:when test="${taskName == 'HoldHumTask'}">
                ...
                <logic:notEmpty name="batch">
                    <div class="form-btns hide" id="btns">
                        <input type="submit" class="submit" name="method.save" id="method.save"
                               onclick="javascript:batchSubmithold();"
                               value="<bean:message bundle='sheet' key='button.done'/>">
                    </div>
                </logic:notEmpty>
            </c:when>
            ...
            <c:otherwise>
                ...
                <c:if test="${taskName == 'FirstExcuteHumTask' || taskName == 'SecondExcuteHumTask' || taskName=='ThirdExcuteHumTask' || taskName=='subTask'}">
                    <input type="button" title="knowledge" value="预处理" onclick="searchK();" class="btn">
                </c:if>
                <logic:empty name="batch">
                    <div class="form-btns hide" id="btns">
                        <input type="submit" class="submit" name="method.save" id="method.save"
                        ...
                        <input type="submit" class="submit"
                               onclick="v.passing=true;this.form.action='${app}/sheet/commonfault/commonfault.do?method=performSaveInfoAndExit'"
                               value="<bean:message bundle='sheet' key='button.saveback'/>"/>
                            <%-- </c:if>--%>
                        <input type="submit" class="submit"
                               onclick="v.passing=true;this.form.action='${app}/sheet/commonfault/commonfault.do?method=showListsendundo'"
                               value="<bean:message bundle='sheet' key='button.back'/>"/>
                    </div>
                </logic:empty>
                <logic:notEmpty name="batch">
        ...
        <script type="text/javascript">
            ...
            v.custom = function () {
                if ('${sheetMain.mainSendMode}' != '101030502' && ${operateType} == 46) {//如果是告警派过来的工单，并且在处理完成时需要校验
                    //if(deptid!='12201'){
                    if ($("mainAlarmSolveDate1").value == '' && $('${sheetPageName}nodeAccessories').value == '') {
                        alert('告警消除时间为空，不允许处理完成,如果要强制回复请上传附件！');
                        return false;
                    } else {
            ...
            function searchK() {
                ...
                window.showModalDialog("${app}/sheet/telnetSocket/telnetSocket.do?method=init&neid=-1453970567", "", "");
                Ext.Ajax.request({
                    url: "${app}/sheet/telnetSocket/telnetSocket.do?method=close",
                    method: "POST"
                });
            }
