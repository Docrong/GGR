...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/TR/WD-xsl" xmlns:txsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="node[@nodeType='node']">
        <li>
            ...
            <xsl:attribute name="state">
                <xsl:eval>state()</xsl:eval>
            </xsl:attribute>
            <IMG WIDTH="16" topmargin="0">
                <xsl:attribute name="SRC"><xsl:eval>GetPath()</xsl:eval>/XTreeIcon/
                    <xsl:eval>icon()</xsl:eval>
                </xsl:attribute>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <IMG WIDTH="14" topmargin="0">
                        <xsl:attribute name="SRC"><xsl:eval>GetPath()</xsl:eval>/bs.gif
                        </xsl:attribute>
                    </IMG>
                </xsl:otherwise>
            </xsl:choose>
            ...
            <ul>
                <xsl:attribute name="style">display: <xsl:eval>Exp()</xsl:eval>;
                </xsl:attribute>
                <xsl:apply-templates order-by="@apptype;@name" select="*"></xsl:apply-templates>
            </ul>
        </li>
    </xsl:template>
    <xsl:template match="node[@clickstate='onuse']">
        <li>
            ...
            <span class="clsUnavailable" onmouseover="over(this)" onmousedown="down(this)" onmouseout="out(this)"
                  onmouseup="this.className='clsCurrentHasFocus'">
                <xsl:attribute name="onclick">NodeInfo(this)</xsl:attribute>
                <xsl:choose>
                    <xsl:when test=".[@title!=''] ">
    ...
    <xsl:template match="node[@nodeType='leaf']">
        <li>
            ...
            <IMG WIDTH="16">
                <xsl:attribute name="SRC"><xsl:eval>GetPath()</xsl:eval>/XTreeIcon/leaf.gif
                </xsl:attribute>
            </IMG>
            <xsl:choose>
                <xsl:when test="context()[@src $ne$ '']">
    ...
    <xsl:template match="node[@apptype='template']">
        <li>
            ...
            <xsl:attribute name="state">closed</xsl:attribute>
            <IMG WIDTH="16">
                <xsl:attribute name="SRC"><xsl:eval>GetPath()</xsl:eval>/XTreeIcon/files.gif
                </xsl:attribute>
            </IMG>
            <span onmouseover="over(this)" onmousedown="down(this)" onmouseout="out(this)"
                  onmouseup="this.className='clsCurrentHasFocus'">
    ...
    <xsl:template match="node[@nodeType='root']">
        <ul ID="Root" STYLE="display:block;margin=3">
            <li>
                ...
                <ul>
                    <xsl:attribute name="style">display: block;</xsl:attribute>
                    <xsl:apply-templates order-by="@order" select="*"></xsl:apply-templates>
                </ul>
            </li>
        </ul>
    </xsl:template>
