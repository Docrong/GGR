...
function check() {
	...
	if (templateId == "") {
		...
		if (taskName != null && taskName == "DraftHumTask") {
			window.opener.location.href = "${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+templateId ;
		} else {
			window.opener.location.href = "${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=referenceTemplate&templateId="+templateId ;
		}
...
<bean:define id="url" value="businessdredgebroad.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		export="false" requestURI="businessdredgebroad.do" 
		...
		<display:column sortable="true" headerClass="sortable" title="${templateName}">
			<a href="${app}/sheet/businessdredgebroad/businessdredgebroad.do?method=editTemplateInfo&templateId=${process.id}&type=${templateManage}">
