...
public class BusinessChangeInterfaceManagerImpl extends WfInterfaceOperateManagerAbstract implements IWfInterfaceOperateManager{
	...
	public boolean sendFlowInterfaceData(BaseMain main, BaseLink link, String interfaceType, String methodType, String serviceType) {
		try{
			...
			if (taskName.equals("ExcuteHumTask") && operateType.equals("4")) {// 驳回
					...
					String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(map, StaticMethod.getFilePathForUrl("classpath:config/businessChange-crm.xml"), nodeName);
					result = CrmLoaderHB.withdrawWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSCHANGE, new Integer(serviceType).intValue(),
							main.getParentCorrelation(), opDetail,attachRef);
			} else if (operateType.equals("9")) {// 阶段回复
				nodeName = "notifyWorkSheet";
				String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(map, StaticMethod.getFilePathForUrl("classpath:config/businessChange-crm.xml"), nodeName);
				result = CrmLoaderHB.notifyWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSCHANGE, new Integer(serviceType).intValue(),
						main.getParentCorrelation(), opDetail,attachRef);
			}else if (operateType.equals("205")) {// 回复
				nodeName = "replyWorkSheet";	
				String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(map, StaticMethod.getFilePathForUrl("classpath:config/businessChange-crm.xml"), nodeName);
				result = CrmLoaderHB.replyWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSCHANGE, new Integer(serviceType).intValue(),
						main.getParentCorrelation(), opDetail,attachRef);
			}else if (taskName.equals("ExcuteHumTask") && operateType.equals("61")) {// 受理
				nodeName = "confirmWorkSheet";
				String opDetail = InterfaceUtilProperties.getInstance().getXmlFromMap(map, StaticMethod.getFilePathForUrl("classpath:config/businessChange-crm.xml"), nodeName);
				result = CrmLoaderHB.confirmWorkSheet(InterfaceConstants.SERVIVETYPE_BUSINESSCHANGE, new Integer(serviceType).intValue(),
						main.getParentCorrelation(), opDetail,attachRef);
			}
