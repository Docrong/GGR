...
<td class="label">
			        ...
			        <script type="text/javascript">
				         Ext.onReady(function(){
		                        ...
		                        factoryTree = new xbox({
		    	                    ...
		    	                    treeDataUrl:factoryTreeAction,treeRootId:'1010107',treeRootText:'Z端地市',treeChkMode:'',treeChkType:'dict',
		    	                    showChkFldId:'${sheetPageName}showFactory',saveChkFldId:'${sheetPageName}ztypearea',viewer:factoryViewer
		                       });
		                 });
                     </script>
                     <textarea class="textarea max" readonly="readonly" name="${sheetPageName}showFactory" style="height:50px" id="${sheetPageName}showFactory"  alt="allowBlank:true,maxLength:250,vtext:'请填入HLR资源类型，最多输入125个汉字'"><c:forTokens items="${sheetMain.ztypearea}" delims="," var="ztypearea" varStatus="status"><eoms:id2nameDB id="${ztypearea}" beanId="ItawSystemDictTypeDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens></textarea>              
 ...
 <td class="label">
			...
			<td class="content">
				<eoms:attachment name="sheetMain" property="resourceReport" scope="request" idField="resourceReport" appCode="citycustom" /> 
